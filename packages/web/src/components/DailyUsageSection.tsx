    ...
    <section className="console-list-card rounded-2xl shadow-[0_12px_30px_rgba(43,33,26,0.08)] p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="px-3 py-1.5 text-xs rounded-lg bg-[var(--console-button-emphasis)] text-[var(--cafe-surface)] hover:bg-[var(--console-button-emphasis-hover)] disabled:opacity-50"
        >
          {loading ? '加载中...' : '刷新'}
        </button>
      </div>
      ...
      {grandTotal && grandTotal.invocations > 0 && (
        <div className="border-t-2 border-[var(--console-border-soft)] pt-2 flex items-center gap-2 text-xs">
          <span className="flex-1 font-semibold text-cafe-secondary">7 日合计 {grandTotal.invocations} 次</span>
          <span className="shrink-0 tabular-nums font-semibold text-cafe-secondary">
            总 {formatTokens(grandTotal.inputTokens + grandTotal.outputTokens)}
          </span>
          <span className="w-16 shrink-0 text-right tabular-nums text-cafe-secondary">
            入 {formatTokens(grandTotal.inputTokens)}
          </span>
          <span className="w-16 shrink-0 text-right tabular-nums text-cafe-secondary">
            出 {formatTokens(grandTotal.outputTokens)}
          </span>
          {grandTotal.costUsd > 0 && (
            <span className="w-14 shrink-0 text-right tabular-nums text-conn-amber-text font-semibold">
              ${grandTotal.costUsd.toFixed(2)}
            </span>
          )}
        </div>
      )}
