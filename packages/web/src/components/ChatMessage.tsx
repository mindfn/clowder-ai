...
import { detectRoutePrefix, getThreadHref, pushThreadRouteWithHistory } from './ThreadSidebar/thread-navigation';
      ...
      <div className="max-w-[85%] md:max-w-[75%] min-w-0">
          ...
          <div className="mb-1 flex flex-col gap-1 min-w-0">
            ...
            {message.extra?.crossPost &&
              (() => {
                ...
                return (
                  <a
                    href={getThreadHref(sourceId, detectRoutePrefix())}
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
        ...
        <div
          className={`border px-4 py-3 transition-transform hover:-translate-y-0.5 overflow-hidden ${
            catStyle ? `${catStyle.radius} ${catStyle.font ?? ''}` : 'bg-cafe-surface border-[var(--console-border-soft)] rounded-2xl'
