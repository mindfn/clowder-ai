  ...
  return (
    <section className={`rounded-[20px] border p-[18px] transition-colors ${toneClass}`} {...rest}>
      <div className="space-y-1">
        <h4 className="text-[17px] font-bold text-cafe-black">{title}</h4>
        {description ? <p className="text-[14px] leading-6 text-cafe-secondary">{description}</p> : null}
      </div>
  ...
  return (
    <FieldShell label={label} required={required} tone={tone}>
      <input
        ...
        className={`w-full rounded-[10px] border px-3.5 py-2 text-[14px] leading-5 text-cafe-black placeholder:text-cafe-muted outline-none transition focus:ring-2 ${inputColors}`}
        inputMode={inputMode}
  ...
  return (
    <FieldShell label={label} tone={tone}>
      <textarea
        ...
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className={`min-h-[92px] w-full rounded-[10px] border px-3.5 py-2 text-[14px] leading-5 text-cafe-black outline-none transition focus:ring-2 ${inputColors}`}
        placeholder={placeholder}
      />
    </FieldShell>
  );
  ...
  return (
    <FieldShell label={label} required={required} tone={tone}>
      <select
        ...
        disabled={disabled}
        required={required}
        className={`w-full rounded-[10px] border px-3.5 py-2 text-[14px] leading-5 text-cafe-black outline-none transition focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 ${inputColors}`}
      >
        {options.map((option) => (
...
export function PersistenceBanner() {
  return (
    <div data-notice-tone="warning" className="system-notice-bar rounded-[16px] px-4 py-3">
      <p className="text-[13px] font-bold text-cafe-accent">运行时持久化</p>
