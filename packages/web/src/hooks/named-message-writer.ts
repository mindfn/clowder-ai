...
import type { ChatMessage, ChatMessageMetadata, TokenUsage, ToolEvent } from '@/stores/chat-types';
 ...
 * creates it under the server's id. A background thread counts one unread when the message
 * first gains something to show.
 */
function writeStreamed(target: NamedMessageTarget, write: (store: NamedMessageStore) => void, source: StoreSource) {
  ...
  if (hadBody || target.threadId === current.currentThreadId) return;
  const after = findNamed(current, target);
  if (after && hasAssistantBody(after)) current.incrementUnread(target.threadId);
}
