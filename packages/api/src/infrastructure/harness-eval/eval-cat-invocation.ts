...
const DOMAIN_INSTRUCTIONS: Partial<Record<string, string>> = {
  ...
  'eval:capability-evolution':
    'Enter the eval:capability-evolution domain thread. Resolve the F311 Program trajectory only from F299 inv:<id>, then inspect canonical owner refs and join keys for at least two heterogeneous signal/decision surfaces. Require owner-backed evidence roles, named-consumer consumption proof, optimizer exposure proof, and an independent sealed or time-fresh promotion holdout. Missing owner truth is typed insufficient; never infer success from silence, copy owner payloads, or create an F311 scheduler/query ledger. Use the F192 event/quota/time registration and hand evidence refs to the next evaluation stage.',
