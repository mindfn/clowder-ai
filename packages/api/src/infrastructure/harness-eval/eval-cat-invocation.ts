...
const DOMAIN_INSTRUCTIONS: Partial<Record<string, string>> = {
  ...
  'eval:harness-ledger':
    'Enter the eval:harness-ledger domain thread. Treat invocation tracing as immutable evidence, never as a precomputed verdict. Objective semantic sweep packets create sparse structured hints: review only their frozen episodes and call cat_cafe_submit_semantic_sweep with the exact jobId. Unit semantic evaluation packets are the actual metric judgment: start with the supplied high-priority traces, progressively call cat_cafe_retrieve_unit_evaluation_traces with the exact nextCursor when more evidence is needed, then call cat_cafe_submit_unit_evaluation. A structured counterexample changes retrieval priority but never admits or excludes raw traces and never outranks the raw source. Use matched only when sweep evidence supports the supplied coordinate; use irrelevant when it clearly matches none; use unscorable when bounded context cannot support a sweep classification. Do not invent invocation ids, Objective ids, Metric ids, unit refs, counts, denominators, rates, or retrieval provenance. Counterexample metrics count distinct incidents and may trigger by threshold without a denominator. Both flows are asynchronous and must never delay or change the original invocation outcome. Guard rejection events remain supplementary structured evidence, not the packet verdict. Produce a verdict handoff packet only after persisted MetricResult evidence supports fix/build/keep_observe/delete_sunset.',
  'eval:anchor-first':
...
const PUBLISH_VERDICT_INSTRUCTIONS_FRESHNESS = `${PUBLISH_VERDICT_PACKET_INSTRUCTIONS}${FRESHNESS_PUBLISH_SELECTOR_INSTRUCTIONS}${PUBLISH_VERDICT_ARTIFACT_RESULT_INSTRUCTIONS}`;
