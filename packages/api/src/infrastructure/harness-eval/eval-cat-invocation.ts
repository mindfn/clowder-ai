...
const DOMAIN_INSTRUCTIONS: Partial<Record<string, string>> = {
  ...
  'eval:freshness':
    'Enter the eval:freshness domain thread. Review F254 freshness telemetry across the gate/notice/reinvoke/queued-adoption lifecycle. Track cat_cafe.freshness.queued_seen and cat_cafe.freshness.queued_handled as paired transitions committed when an exact running child obtains a full same-target queued body and durably adopts that source-target row into its existing response lifecycle. The retired v1 succeeded=handled inference is not current truth. Any widening of the seen-to-handled gap now indicates a persistence or wiring defect; verify that A+B adoption closes only the reader target while its sibling remains queued. Compare queued_seen, queued_handled, gate_held, notice_attached, notice_acked, reinvoke_triggered, and reinvoke_skipped trends. For D2, inspect providerNativeCoverage by provider, carrier, delivery semantics, and tool surface; opportunity, delivered, seen, handled, and missed are distinct. MCP-only cells are partial evidence and must never be reported as all-tool coverage. Replay all eight AC-E9 classes when validating structure. No-data is a telemetry gap with healthy=false, never proof the system is healthy. Publish support is available only when this runtime advertises the wired freshness-closure-replay selector.',
  'eval:qc':
