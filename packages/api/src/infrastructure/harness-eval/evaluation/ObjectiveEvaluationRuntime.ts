...
import { EvaluationScheduler, evaluateCounterSnapshot, evaluateRateSnapshot } from './EvaluationScheduler.js';
...
export class ObjectiveEvaluationRuntime {
  ...
  async scheduleMetric(ownerUserId: string, objectiveId: string, metricId: string, now: number): Promise<void> {
    ...
    if (definition.metric.evaluator.kind !== 'code') return;
    if (definition.metric.kind === 'counter') {
      await this.results.append(evaluateCounterSnapshot(scheduled.snapshot, definition.metric, now));
    } else if (definition.metric.kind === 'rate') {
      await this.results.append(evaluateRateSnapshot(scheduled.snapshot, definition.metric, now));
    }
  }
}
