...
export async function sessionChainRoutes(app: FastifyInstance, opts: SessionChainRouteOptions): Promise<void> {
  ...
  app.post<{
    ...
    if (candidate.kind === 'error') {
      return reply.status(candidate.status).send(candidate.body);
    }
    const { session } = candidate;

    // The idle check and store transition are separated by awaits. The tracker
    // guard prevents a local invocation from starting in that window and taking
    // ownership of the still-active session record.
    const sealGuard = invocationTracker?.guardSessionSeal
      ? invocationTracker.guardSessionSeal(session.threadId, session.catId)
      : {
          acquired: !invocationTracker?.has(session.threadId, session.catId),
          release: () => {},
        };
    if (!sealGuard.acquired) {
      return reply.status(409).send({
        error: '请先停止该 Agent，再封存会话',
        code: 'SESSION_ACTIVE_INVOCATION',
        catId: session.catId,
      });
    }

    let seal: Awaited<ReturnType<ISessionSealer['requestSeal']>>;
    try {
    ...
    if (!sealed || sealed.status !== 'sealed') {
      return reply.status(503).send({
        error: 'Session sealing has not completed yet',
        code: 'SESSION_SEAL_PENDING',
      });
    }
    return reply.send({
