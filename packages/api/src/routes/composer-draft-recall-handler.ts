...
async function commitRecall(
  ...
  try {
    ...
    if (result.kind === 'recalled' || result.kind === 'already_recalled') {
      const withdrawalCommitted = await context.opts.invocationQueue.commitClaimedMessageWithdrawal(
        context.threadId,
        entryIds,
      );
      if (!withdrawalCommitted) {
        ...
        for (const entry of carrier.entries) context.opts.queueProcessor?.registerCallerDispatchQueueWithdrawal(entry);
      }
      return result;
    }
