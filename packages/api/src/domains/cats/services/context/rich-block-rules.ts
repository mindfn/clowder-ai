...
- **video**（视频展示 — MediaHub 生成/分析结果）
  - 视频生成结果展示、视频分析报告
  - \`url\` 必填（视频文件 URL，支持 /api/ 路径或 https:// ）
  - \`title\`/\`poster\` 可选
...
- video: \`url\` 必填（/api/ 或 https:// 视频路径），\`title\`/\`poster\` 可选
...
支持: card / diff / checklist / media_gallery / audio / interactive / video / html_widget。
