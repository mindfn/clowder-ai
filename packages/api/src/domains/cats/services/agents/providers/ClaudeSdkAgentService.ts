...
function settleProviderOwnedTurnInputs(
  ...
  if (queuedTurnCount !== undefined && snapshotUpperBound !== undefined) {
    // @anthropic-ai/claude-agent-sdk@0.3.267 sdk.d.ts defines
    // queued_turn_count as the provider command-queue snapshot at result time.
    // The echoed input identity also bounds which accepted sends that snapshot
    // can describe. Inputs accepted after that identity survive even if the SDK
    // has already prefetched them from our iterator before we consume the older
    // result event.
...
export class ClaudeSdkAgentService implements AgentService {
  ...
  async *invoke(prompt: string, options?: AgentServiceOptions): AsyncIterable<AgentMessage> {
    ...
    const locallyQueuedTurnInputIds = new Set<string>();
    const providerOwnedTurnInputIds = new Set<string>();
    const turnInputOrdinals = new Map<string, number>();
    let nextTurnInputOrdinal = 0;
    let activeDispatches = 0;
    let terminalWaitingForDispatch = false;

    const pushTurnInput = (message: IdentifiedSdkUserMessage): boolean => {
      locallyQueuedTurnInputIds.add(message.uuid);
      turnInputOrdinals.set(message.uuid, ++nextTurnInputOrdinal);
      const accepted = input.push(message, () => {
        locallyQueuedTurnInputIds.delete(message.uuid);
        providerOwnedTurnInputIds.add(message.uuid);
      });
      if (accepted) {
        terminalWaitingForDispatch = false;
      } else {
        locallyQueuedTurnInputIds.delete(message.uuid);
        turnInputOrdinals.delete(message.uuid);
      }
      return accepted;
    };
    ...
    const closeAfterTerminalIfIdle = (): boolean => {
      if (
        !terminalWaitingForDispatch ||
        locallyQueuedTurnInputIds.size !== 0 ||
        providerOwnedTurnInputIds.size !== 0 ||
        activeDispatches !== 0
      ) {
        return false;
      }
    ...
    try {
      ...
      for await (const event of query as AsyncIterable<SDKMessage>) {
        ...
        if (isResultTerminal) {
          ...
          terminalWaitingForDispatch = settleProviderOwnedTurnInputs(providerOwnedTurnInputIds, turnInputOrdinals, raw);
