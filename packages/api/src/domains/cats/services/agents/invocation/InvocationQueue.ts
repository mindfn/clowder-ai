...
export class InvocationQueue {
  ...
  private async claimLedgerEntry(entry: QueueEntry, selectedTargetCatId?: string): Promise<QueueEntry | null> {
    ...
    const bindTargetCatId = entry.targetCats.length === 0 ? selectedTargetCatId : undefined;
    ...
    const cached = this.findEntry(projected.threadId, projected.userId, projected.id);
    if (!cached) return null;
    // Legacy readers have no `claimed` state. Present the reversible claim as
    // processing while ledgerClaimIds retains the exact restore authority.
    cached.status = 'processing';
    cached.processingStartedAt = claimedAt;
    this.ledgerClaimIds.set(cached.id, claimId);
    return InvocationQueue.cloneEntry(cached);
  }

  private cacheLedgerClaim(entries: readonly QueueLedgerEntry[], claimId: string, claimedAt: number): QueueEntry[] {
    const projected = this.cacheLedgerEntries(entries);
    for (const entry of projected) {
      const cached = this.findEntry(entry.threadId, entry.userId, entry.id);
      if (!cached) continue;
      // QueueEntry is a compatibility projection while consumers move to the
      // scalar ledger row. Claimed is intentionally hidden from ordinary
      // dequeue selectors as processing; ledgerClaimIds keeps restore authority.
      cached.status = 'processing';
      cached.processingStartedAt = claimedAt;
      this.ledgerClaimIds.set(cached.id, claimId);
    }
    return projected.map((entry) => this.getEntrySnapshot(entry.threadId, entry.userId, entry.id) ?? entry);
  }
  ...
  async claimExactSteerEntryDurable(
    ...
    const assignsTargetlessConversation = entry.kind === 'conversation_input' && entry.targetCats.length === 0;
    if (
    ...
    return { outcome: 'claimed', entries: this.cacheLedgerClaim(claimed.entries, claimId, claimedAt), targetCatId };
  }
  ...
  async markProcessingDurable(
    ...
    if (best.targetCats.length > 0 && !best.targetCats.includes(selectedTargetCatId)) return null;
  ...
  async markProcessingGroupAcrossUsersDurable(
    ...
    if (
      selected.some(
        (entry) =>
          !entry ||
          entry.status !== 'queued' ||
          entry.userId !== best.userId ||
          (entry.targetCats.length > 0 && !entry.targetCats.includes(selectedTargetCatId)),
      )
    ) {
      return null;
    }
    const claimId = randomUUID();
    const claimedAt = Date.now();
    const bindsTargetlessGroup = selected.every((entry) => entry?.targetCats.length === 0);
    ...
    if (claimed.outcome !== 'claimed') return null;
    const projected = this.cacheLedgerClaim(claimed.entries, claimId, claimedAt);
  ...
  async markProcessingByIdDurable(threadId: string, entryId: string, targetCatId: string): Promise<QueueEntry | null> {
    ...
    if (!entry || entry.status !== 'queued') return null;
    if (entry.targetCats.length > 0 && !entry.targetCats.includes(targetCatId)) return null;
  ...
  async commitClaimedProcessing(threadId: string, entryIds: readonly string[], at = Date.now()): Promise<boolean> {
    for (const entryId of entryIds) {
      const claimId = this.ledgerClaimIds.get(entryId);
      if (!claimId) continue;
      const committed = await this.ledgerStore.commit(threadId, entryId, claimId, 'processing', at);
  ...
  async removeProcessedAcrossUsersDurable(threadId: string, entryId: string): Promise<QueueEntry | null> {
    const snapshot = this.findEntryAcrossUsers(threadId, entryId);
    if (!snapshot || snapshot.status !== 'processing') return null;
    const claimId = this.ledgerClaimIds.get(entryId);
    if (claimId) {
      const processing = await this.ledgerStore.commit(threadId, entryId, claimId, 'processing', Date.now());
      if (processing.outcome !== 'updated') return null;
      this.ledgerClaimIds.delete(entryId);
    }
    const terminal = await this.ledgerStore.commit(threadId, entryId, '', 'terminal', Date.now());
  ...
  listScopes(): Array<{ threadId: string; userId: string }> {
    ...
    for (const queue of this.queues.values()) {
      const first = queue[0];
      if (first) scopes.push({ threadId: first.threadId, userId: first.userId });
    }
    return scopes;
  }
  ...
  /** Mark one legacy in-memory entry processing. */
  private markEntryProcessing(entry: QueueEntry, selectedTargetCatId?: string): QueueEntry | null {
    const targetCatId =
      selectedTargetCatId ?? entry.targetCats.find((catId) => isOrdinaryQueueTargetEligible(entry, catId));
  ...
  markProcessing(
    ...
    return this.markEntryProcessing(best, selectedTargetCatId);
  }
  ...
  markProcessingAcrossUsers(
    ...
    return this.markEntryProcessing(best, bestTargetCatId);
  }
  ...
  private resolveProcessingGroupAcrossUsers(
    threadId: string,
    entryId: string,
  ): { queue: QueueEntry[]; members: Array<{ candidate: QueueEntry; index: number }> } | null {
    for (const queue of this.queues.values()) {
      if (!this.queueMatchesThread(queue, threadId)) continue;
      const entry = queue.find((candidate) => candidate.status === 'processing' && candidate.id === entryId);
      if (!entry) continue;
      if (entry.prestartRetirement) {
      ...
      return { queue, members: [{ candidate: entry, index: queue.indexOf(entry) }] };
    }
    return null;
  }
  ...
  hasPendingForCat(
    ...
    for (const q of this.queues.values()) {
      ...
      for (const e of q) {
        ...
        if (
          opts?.continuationKey !== undefined &&
          e.continuationKey !== opts.continuationKey &&
          e.idempotencyKey !== opts.continuationKey
        )
          continue;
  ...
  markProcessingById(
    ...
    for (const q of this.queues.values()) {
      ...
      if (entry) {
        ...
        return this.markEntryProcessing(entry, selectedTargetCatId) !== null;
      }
    }
    return false;
  }
  ...
  collectCompatibleConversationPrefix(
    ...
    if (
      ...
      (resolution?.resolvedTargets ?? head.targetCats).length === 0 ||
