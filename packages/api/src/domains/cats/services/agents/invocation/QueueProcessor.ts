...
export class QueueProcessor {
  ...
  private async executeEntry(
    ...
    try {
      ...
      for await (const msg of router.routeExecution(
        ...
        },
        {
          ...
          onLifecycleInvocationStarted: async (
            ...
            const interruptLifecycleResponse = async (reason: string): Promise<void> => {
            ...
            // ADR-043 D4: History is the receiver and must exist before the
            // exact target leaves Queue. A crash after this point replays the
            // source×target dispatchRef and only cleans the residual claim.
            let queueTargetsRetired = false;
            try {
              queueTargetsRetired = await queue.commitClaimedProcessing(
                input.threadId,
                admissionEntries.map((candidate) => candidate.id),
                input.startedAt,
              );
            } catch (err) {
            ...
            if (!queueTargetsRetired) {
              await interruptLifecycleResponse('queue_target_retirement_pending');
              throw new Error(
                `Lifecycle receiver persisted but Queue target retirement did not commit: ${input.invocationId}`,
              );
            }
      ...
      if (!processingReservationReplaced) {
        try {
          if (lifecycleTransferStarted && !lifecycleReceiverPersisted) {
      ...
      for (const bid of processingReservationReplaced || (lifecycleTransferStarted && !lifecycleQueueTargetsRetired)
        ? []
        : batchedEntryIds) {
