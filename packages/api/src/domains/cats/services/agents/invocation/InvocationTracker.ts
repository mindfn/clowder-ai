...
export class InvocationTracker {
  ...
  /** Exact execution fence for active-run admission and control operations. */
  ...
  getCanceledSlotIdentity(threadId: string, catId: string): { executionId: string; userId: string } | undefined {
    const inv = this.active.get(this.slotKey(threadId, catId));
    if (!inv || inv.state !== 'canceled' || !inv.executionId) return undefined;
    return { executionId: inv.executionId, userId: inv.userId };
  }
