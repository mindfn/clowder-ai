...
export interface ThreadReadState {
  ...
  /** Canonical visibility-domain anchor retained even when the primary v1 message is pruned. */
...
export interface IThreadReadStateStore {
  ...
  /**
   * #1304: Explicit catch-up repair for an unresolvable legacy cursor.
   * CAS: replace only when the stored primary cursor still equals expectedCursor.
   */
  repairReadCursor?(
    userId: string,
    threadId: string,
