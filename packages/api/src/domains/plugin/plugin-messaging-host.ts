...
function identitySource(
  ...
  if (origin?.kind === 'external') {
    const connectorContribution = (manifest.contributions ?? []).find(
      (contribution) => contribution.type === 'connector' && contribution.id === origin.connectorId,
    );
    if (!connectorContribution || connectorContribution.type !== 'connector') {
      throw new MessagingError('PERMISSION', `connector ${origin.connectorId} is not declared by this plugin`);
    }
    connector = connectorContribution.id;
    identity = declared.get(connectorContribution.identityRef);
  } else {
    const all = [...declared.values()];
    if (all.length !== 1) {
      throw new MessagingError('VALIDATION', 'plugin-authored messages require exactly one declared identity');
    }
    identity = all[0];
  ...
  return {
    ...
    icon: identity.icon ?? 'message',
    ...(origin?.kind === 'external' && origin.sourceAddress !== undefined
      ? { meta: { externalChatId: origin.sourceAddress.chatId } }
      : {}),
...
export function createPluginMessagingHost(input: PluginMessagingHostDeps): PluginMessagingHost {
  return {
    ...
    async send(value) {
      ...
      const source = identitySource(input.manifest, origin, sender);
      const scope = {
        canSend: true,
