...
export const ENV_VARS: EnvDefinition[] = [
  ...
  },
  {
    ...
    defaultValue: '0',
    description:
      '消息过期时间（秒）。默认 0 = 永久保留（与 RedisMessageStore 的 DEFAULT_TTL_SECONDS=0 一致）。注意：过期的 Redis 消息不影响已索引的 evidence_passages（Phase I 保证永久性）。',
    category: 'storage',
    sensitive: false,
    runtimeEditable: false,
    control: 'number',
    hubVisible: false,
    deprecated:
      '已迁移到 .cat-cafe/user-preferences.json 的 retentionConfig 字段（F770），运行时修改、对新写入立即生效、无需重启；此 env 仅作只读回退，不自动迁移——用户选择一次预设后由 JSON 接管',
  },
  {
    name: 'THREAD_TTL_SECONDS',
    defaultValue: '0',
    description: '对话过期时间（秒）。默认 0 = 永久保留',
    category: 'storage',
    sensitive: false,
    runtimeEditable: false,
    control: 'number',
  ...
  },
  {
    ...
    defaultValue: '0',
    description: '任务过期时间（秒）。默认 0 = 永久保留',
    category: 'storage',
    sensitive: false,
    runtimeEditable: false,
    control: 'number',
  ...
  },
  {
    ...
    defaultValue: '0',
    description: '摘要过期时间（秒）。默认 0 = 永久保留',
    category: 'storage',
    sensitive: false,
    runtimeEditable: false,
    control: 'number',
  ...
  },
  {
    ...
    defaultValue: '',
    description: 'Backlog 过期时间（秒）。默认 0 = 永久保留',
    category: 'storage',
    sensitive: false,
    runtimeEditable: false,
    control: 'number',
  ...
  },
  {
    ...
    defaultValue: '',
    description: '草稿过期时间（秒）。默认 0 = 永久保留',
    category: 'storage',
    sensitive: false,
    runtimeEditable: false,
    control: 'number',
    hubVisible: false,
    deprecated:
      '草稿 TTL 是自动保存功能常量（300 秒，RedisDraftStore），从来不是保留策略；代码从不读取此 env——F770 将草稿整体移出保留预设',
  },
  {
    name: 'DATA_DIR',
  ...
  },
  {
    ...
    defaultValue: '~/.cat-cafe',
    description:
      '猫猫平台状态目录（默认 ~/.cat-cafe）：成员 profile、agent key sidecar、TTS 听读状态、CLI 进程记账等。' +
      '账号/凭据不在这里（由 CAT_CAFE_GLOBAL_CONFIG_ROOT 决定）。与 DATA_DIR（repo 数据根：缓存/会话/上传等业务数据）区分，两者互不影响',
    category: 'cli',
    sensitive: false,
    runtimeEditable: true,
...

  {
    ...
    hubVisible: false,
    deprecated:
      '已迁移到 .cat-cafe/user-preferences.json 的 themeConfig 字段（F770），运行时读写、改后无需重启；此 env 值仅作首次读取的只读回退',
  },
