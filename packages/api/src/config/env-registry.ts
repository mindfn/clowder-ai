...
export const ENV_VARS: EnvDefinition[] = [
  ...
  },
  {
    ...
    defaultValue: '0',
    placeholder: '604800 = 7天（示例）',
    description:
      '消息过期时间（秒）。默认 0 = 永久保留（与 RedisMessageStore 的 DEFAULT_TTL_SECONDS=0 一致）。设为 604800 即 7天。注意：过期的 Redis 消息不影响已索引的 evidence_passages（Phase I 保证永久性）。',
    category: 'storage',
    sensitive: false,
    runtimeEditable: true,
    label: '消息过期时间',
