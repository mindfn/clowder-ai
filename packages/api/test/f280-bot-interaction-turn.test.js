...
describe('F280 4b — bot interaction turns', () => {
  ...
  /*
   * F280 section 4b: a stranger's summon is not our round.
   *
   * The turn folder used to open a round for ANY `@bot review` on the PR. A maintainer asking
   * codex for a second opinion therefore created pending state on OUR tracker, and thirty
   * minutes later A28 announced that a request this thread never made had gone unanswered.
   */
  test("A28: someone else's summon opens no round, so no timeout is invented for it", async () => {
    let now = TRIGGER_MS + 1_000;
    const harness = await createHarness({ when: AUTHOR_SUBSCRIPTION, now: () => now });
    await harness.poll({
      conversation: [triggerComment({ id: 22, user: { login: 'Maintainer', type: 'User' } })],
    });
    // It is still bot conversation — the maintainer's summon is delivered like any other
    // external comment. Only the ROUND is not ours.
    assert.equal(harness.delivered().length, 1);

    now = TRIGGER_MS + BOT_TURN_TIMEOUT_MS * 2;
    await harness.poll();
    assert.deepEqual(
      harness.delivered().filter((content) => /never answered/.test(content)),
      [],
      'a round we never opened cannot time out on us',
    );
  });

  /*
   * "I cannot tell whether this was ours" is NOT "this was ours". Both unresolvable boundaries —
   * GitHub gave us no author (deleted / ghost account), or our own identity will not resolve —
   * reach the folder as the same missing `self`, and must produce the same answer: no round.
   * An invented round reports a timeout for a request nobody sent; a missing one costs the owner
   * one "it never came back" notice.
   */
  test('an unattributable summon opens no round; our own opens one', () => {
    const summon = (self) =>
    ...
    assert.equal(summon(undefined).botTurn, undefined, 'unresolved identity must not open a round');
    assert.equal(summon(false).botTurn, undefined);
    assert.equal(summon(true).botTurn?.phase, 'open');
    // The NAME is decided by mention, not by ownership: an unattributable summon is still part
    // of the bot conversation, so a maintainer can still mute that whole round.
    assert.equal(summon(undefined).type, 'pr_bot_interaction');
  });

  /*
   * F280 section 4b: registration does NOT look for open rounds, and F177 does not borrow one.
   *
   * It used to do both: the baseline reader scanned conversation history for a summon that
   * looked like ours, asked an EYES coverage verifier whether the bot had accepted, and stamped
   * the round with the registering `invocationId` so the routing guard could clean-stop on it
   * the same turn. The scan reads HISTORY, so the stamp landed on a summon an EARLIER turn
   * wrote, and a brand-new invocation inherited an exit it never earned. Rounds now have one
   * origin — the normalized stream, after the cursor, from the owner's own summon — and the
   * routing exit fails closed rather than borrowing from tracking.
   */
  describe('registration never opens a round, and no round is an exit', () => {
    const readerDeps = {
    ...
    test('our own summon sitting in history does not become a pending round', async () => {
      // The retired seam is passed in on PURPOSE. A test that simply omits it would stay green
      // if someone re-added the probe tomorrow; this one asserts the reader asks nobody about
      // coverage and stamps nobody's invocation, even when handed everything it used to use.
      let verifierCalls = 0;
      ...
      assert.equal(verifierCalls, 0, 'registration must not consult an EYES coverage verifier');
      assert.equal(snapshot.baseline.botTurns, undefined, 'history is not a pending round');
      // The frontier still froze past that comment, exactly like every other source: the summon
      // is HISTORY to this registration, and a later poll must not replay it as new.
      assert.equal(snapshot.baseline.review.conversationCommentCursor, 21);
    });

    test('an open round releases nobody — not even the invocation that registered', async () => {
      const task = {
        id: 'task-1394',
        kind: 'pr_tracking',
        status: 'doing',
      ...
      // The round belongs to the tracking OWNER, so it cannot say which INVOCATION an event is
      // coming back to. Both answers must be the same one, or the difference is a fiction.
      for (const invocationId of ['inv-owner', 'inv-foreign']) {
        assert.deepEqual(await exitFor(invocationId), { kind: 'reject', reason: 'predicate_missing' });
      }
    });
  });
