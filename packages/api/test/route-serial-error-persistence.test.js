...
describe('route-serial error persistence (F5 reload)', () => {
  ...
  it('serial: terminalizes a failed A2A response without creating a predecessor report carrier', async () => {
    ...
    for await (const _ of routeSerial(deps, ['bengal'], 'please handle this', 'user1', 'thread1', options)) {
      void _;
    }

    assert.ok(lifecycleStore);
    const response = lifecycleStore.getById(responseMessageId);
    assert.equal(response?.lifecycle?.kind, 'response');
    assert.equal(response?.lifecycle?.status, 'failed');
    assert.equal(response?.content, 'model unavailable');
    assert.equal(
      lifecycleStore
        .getByThread('thread1', 100, 'user1')
        .some((message) => message.lifecycle?.kind === 'input' && message.content.includes('model unavailable')),
      false,
      'ordinary failure must not synthesize a second predecessor carrier',
    );
  });
