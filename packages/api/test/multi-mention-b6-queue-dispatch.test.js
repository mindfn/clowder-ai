...
describe('B6: multi_mention queue dispatch', () => {
  ...
  test('a new multi-mention source is not deduped by another pending message for the same cat', async () => {
    ...
    assert.equal(mockQueueProcessor.getHooks().size, 1);
    assert.equal(
      invocationQueue.list('thread-1', 'user-1').filter((entry) => entry.targets.includes('codex')).length,
      2,
      'dedupe is source×target, never merely target cat identity',
    );
  });
...
describe('B6: QueueProcessor entryCompleteHook integration', () => {
  test('executeEntry fires registered hook with response text', async () => {
    ...
    const stubDeps = {
      ...
      router: {
        ...
        async *routeExecution(userId, _c, threadId, _m, targetCats, _intent, options) {
          await options.onLifecycleInvocationStarted({
            threadId,
            userId,
            catId: targetCats[0],
            invocationId: 'turn-hook-single',
  ...
  test('executeEntry partitions one source row into target-specific completion hooks', async () => {
    ...
    const stubDeps = {
      ...
      router: {
        resolveExplicitTargets: async (requestedCatIds) => [...requestedCatIds],
        resolveConversationTargetsAtAdmission: async (requestedCatIds) => [...requestedCatIds],
        async *routeExecution(userId, _content, threadId, _messageId, targetCats, _intent, options) {
          const [catId] = targetCats;
          await options.onLifecycleInvocationStarted({
            threadId,
            userId,
            catId,
            invocationId: `turn-hook-${catId}`,
            parentInvocationId: options.parentInvocationId,
            startedAt: Date.now(),
          });
  ...
  test('dispatches one exact cloud child, returns its typed failure notice, and retires the Queue carrier', async () => {
    ...
    const enqueue = () =>
      queue.enqueueExistingMessageDurable(messageStore, sourceMessage.id, {
