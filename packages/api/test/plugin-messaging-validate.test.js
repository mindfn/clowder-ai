...
let validate;
/** @type {typeof import('../dist/domains/messaging/contract/types.js')} */
let types;

before(async () => {
  validate = await import('../dist/domains/messaging/contract/validate.js');
  types = await import('../dist/domains/messaging/contract/types.js');
});
...
describe('validateAppendInput', () => {
  ...
  test('MessagingError carries code and name', () => {
    const err = new types.MessagingError('VALIDATION', 'boom');
