...
describe('QueueProcessor over the source-row pending Queue', () => {
  ...
  it('keeps the History receiver when Queue retirement loses its acknowledgement', async () => {
    const harness = createHarness();
    const admitted = await admitMessage(harness);
    const originalCommit = harness.queue.commitClaimedProcessing.bind(harness.queue);
    let injected = false;
    harness.queue.commitClaimedProcessing = mock.fn(async (...args) => {
      if (!injected) {
        injected = true;
  ...
  it('keeps a FIFO prefix as separate prompt messages instead of concatenating bodies', async () => {
    ...
    const harness = createHarness({
      routeExecution: async function* (...args) {
        calls.push(args);
        await startLifecycle(args, 'turn-fifo-prefix');
