...
describe('#1392 AC-6b — baseline reader seeds the conversation frontier for a conversation-comment waiter', () => {
  ...
  test('a non-review waiter (pr_head_changed only) does NOT fetch the review frontier and gets no review baseline', async () => {
    const { calls, deps } = makeDeps({ conversation: [{ id: 250 }] });
    const snap = await readGitHubWaitBaseline(
      { repoFullName: 'owner/repo', prNumber: 7, when: [{ kind: 'pr_head_changed' }] },
      deps,
    );
    assert.equal(calls.fetchConversationComments, 0, 'a head-only waiter must not fetch review data');
    assert.equal(snap.baseline.review, undefined);
  });
});
