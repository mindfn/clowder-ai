...
describe('Callback Routes', () => {
  ...
  /*
   * F280 2.5b: re-registration may hold or rewind a frontier, never advance it. Registration no
   * longer probes for open rounds, so a round exists only where the poll stream put it — in the
   * LIVE baseline. Merging a fresh snapshot over it is therefore not a way to "refresh" the
   * round, it is the only way to lose one: the cat that summoned a bot and then changed its
   * subscription could never be told the bot went silent.
   */
  test('re-registering a live tracker holds the whole baseline, including a round the poll opened', async () => {
    const polled = {
      'chatgpt-codex-connector[bot]': { triggerId: 4936000123, openedAt: 1_788_000_000_000, headSha: 'head-live' },
    };
    // The stub MOVES on the second call. A stub that returned the same frontiers twice would
    // pass whichever baseline survived, and prove nothing about holding.
    let call = 0;
    const app = await createApp({
      fetchPrWaitBaseline: async () => {
        call += 1;
        return {
          baseline: {
            ...
            headSha: 'head-live',
            review: {
              inlineCommentCursor: call === 1 ? 99 : 500,
              conversationCommentCursor: call === 1 ? 99 : 500,
              decisionCursor: call === 1 ? 99 : 500,
            },
    ...
    assert.equal(first.statusCode, 200);
    assert.equal(
      JSON.parse(first.body).await.baseline.botTurns,
      undefined,
      'registration must not invent a round; only the poll stream opens one',
    );

    // What the poll does when the cat summons a bot: it folds the round into the LIVE state.
    const tracked = await taskStore.getBySubject('pr:zts212653/cat-cafe#410');
    const live = tracked.automationState;
    const seeded = await taskStore.replaceAutomationStateIfGeneration(tracked.id, {
      expectedGeneration: live.await.generation,
      expectedUpdatedAt: tracked.updatedAt,
      automationState: {
        ...live,
        await: { ...live.await, baseline: { ...live.await.baseline, botTurns: polled } },
      },
    });
    assert.ok(seeded, 'test setup: the polled round must be installed');
    ...
    assert.equal(baseline.review.conversationCommentCursor, 99, 'the old frontier must be held, not advanced');
    assert.deepEqual(baseline.botTurns, polled, 'a live round must survive re-registration');
    await app.close();
  });
