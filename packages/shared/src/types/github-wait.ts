...
export interface WaitOutcomeV1 {
  ...
  /**
   * R4 review-loop brake: this outcome must be DELIVERED but must not auto-wake its owner.
   *
   * sol R32: the decision belongs to the outcome, not to whichever signal happens to be in the
   * caller's hand. It used to live only in the rendered `nextStep` prose, so a re-published
   * outcome silently lost it — a transient connector failure could cancel the very pause the
   * brake exists to enforce. Structured, because reading it back out of the prose is the
   * guessing section 2.4 forbids.
   */
