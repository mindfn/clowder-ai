...
describe('F280 register_issue_tracking public contract', () => {
  ...
  it('#1392: forwards issue_comment_added authorLogins and autoRenew, omitting an unset expiry', async () => {
    const originalFetch = globalThis.fetch;
    const originalEnv = { ...process.env };
    let requestBody;
    process.env.CAT_CAFE_API_URL = 'http://127.0.0.1:1';
    ...
    try {
      ...
      await handleRegisterIssueTracking({
        repoFullName: 'zts212653/cat-cafe',
        issueNumber: 1227,
        when: [{ kind: 'issue_comment_added', authorLogins: ['maintainer'] }],
        nextStep: 'Reply to the maintainer.',
        autoRenew: false,
      });
      assert.deepEqual(requestBody, {
        repoFullName: 'zts212653/cat-cafe',
      ...
      assert.equal(Object.hasOwn(requestBody, 'expiresAt'), false, 'an unset expiry must not cross the boundary');
    } finally {
      globalThis.fetch = originalFetch;
      for (const key of Object.keys(process.env)) if (!(key in originalEnv)) delete process.env[key];
      Object.assign(process.env, originalEnv);
