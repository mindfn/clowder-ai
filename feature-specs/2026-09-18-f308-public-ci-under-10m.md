...
**Acceptance:** All selected files execute exactly once. A test enters the one global serial lane only through explicit evidence that it uses a cross-VM remote endpoint, shared account, or shared quota. Every other file enters one count-balanced pool across six isolated VMs, one fresh process and one file at a time per VM. Target-CI distributable lanes execute in a Linux network namespace whose only enabled interface is loopback, then set `no_new_privs` and empty every capability set before tests start, so unknown Node/native/shell IP-egress paths fail at the kernel boundary and child processes cannot re-enter the parent namespace. The bounded preload guard additionally rejects its recognized non-loopback and privilege-escape surfaces before I/O, including Bash `/dev/tcp` and `/dev/udp` redirections. Local command doubles require an explicit exact executable declaration under the temporary directory. A measured critical path over 600,000 ms fails CI. AC-D6 remains open until three same-selection target artifacts establish p50/p95.
...
The revised boundary is behavioral instead of lexical. The shared lane is an explicit registry of proved cross-VM resources; it is empty for the current suite. Each distributable target-CI command runs below a kernel network boundary: a new Linux network namespace enables loopback for local test servers but has no external interface or route, then restores the unprivileged runner identity with `no_new_privs` set and inheritable, permitted, effective, bounding, and ambient capability sets empty. The launcher verifies those kernel facts and proves both passwordless `sudo` and `nsenter` fail before starting the shard. This covers unknown Node/native/shell IP-egress APIs without guessing source tokens or leaving a route back to the parent namespace. The bounded preload guard remains defense in depth and gives an earlier typed failure for recognized `fetch`, WebSocket, HTTP(S), TCP/TLS, command, and child-process paths; it also rejects non-loopback or dynamic Bash `/dev/tcp` and `/dev/udp` targets plus explicit privilege-escape commands before spawn. A network-command-named test double is allowed only when the test explicitly declares the exact executable and it resolves beneath the system temporary directory. Public shard jobs additionally receive no service credentials, use read-only repository permission, do not persist checkout credentials, and restrict Git transports to local files.
...
6. distributable shard commands enter a new Linux network namespace, enable only loopback, irreversibly drop root with `no_new_privs` plus empty capability sets, and prove `sudo`/`nsenter` cannot escape before executing tests;
...
4. The workflow contract checker locks the lane matrix, loopback-only network namespace, `no_new_privs`/capability-drop barrier with Linux escape probes, read-only/no-persisted-credentials boundary, exact execution environment, and 10-minute gate.
