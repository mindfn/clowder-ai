...
### LL-103: 交接里写"归属结论"会过期成误导——传判据，不传缓存结论

- 状态：validated
- 更新时间：2026-09-07

- 坑：#1394 的一份 session 交接在 gotchas 里写了「CI 现在的红不是你的，已有干净对照」。这条结论在写下的那一刻是真的，但分支继续演进后，至少两条红确实是接手方自己引入的（一次 Biome `noDuplicateObjectKeys`，一次 hold 生命周期测试）。fresh-context 的接手方两次都先按"又是老 flake"处理，绕了两圈才回到核查。
- 根因：把一次**带日期的观测**写成了**无时效的常识**。接手方没有独立证据，缓存结论直接充当先验；而先验的作用恰恰是**抑制本该发生的核查动作**。留一条会过期的结论比不留结论更危险——不留结论时，接手方至少会去查。
- 触发条件：交接笔记 / 长期文档里出现"X 不是你的问题 / X 是已知 flake / X 可忽略"这类**归属判定**；且分支或环境仍在演进；且接手方是 fresh context（没有能力凭记忆质疑）。
- 修复：删除该结论，换成归属判定方法（见防护）。
- 防护：
  1. **交接只写"怎么判定归属"，不写"归属是谁"。** 必须写结论时，同一行给出 exact SHA + 失效条件（"仅对 `<sha>` 成立，HEAD 前进即作废"）。
  2. **拉 job log 核到具体测试名。** 聚合 check（`Test (Public)` 之类）只是壳，真失败在 shard 里；只看聚合名无法判断归属。
  3. **跑全量套件，把失败文件集与冻结对照 diff。** 手挑几个套件全绿，对不在清单里的文件什么都没说明；对照必须是同一命令在已知基点上跑出的**文件集**，不是一个数字。
- 来源锚点：PR #1394（zts212653/clowder-ai，review 中）；pre-squash 轮次 commit `ff26269bc` 的 commit body 记录了第一条自引入红的完整因果（该 SHA 会随 squash 失效，PR 号才是稳定锚点——本条自身即按本条规则标注）
- 原理：**结论有时效，判据没有。** 交接的价值是让接手方更快获得判断力，不是替他做出判断；替他做出的判断，会在他最没有能力质疑的时候生效。

- 关联：LL-001（提炼前先做时效性验证）| LL-006（没有新鲜验证证据不得宣称完成）| LL-007（交接缺 Why）| LL-029（不能只看 spec checkbox）

### LL-104: 同一形状的缺陷第二次出现，要加的是约束不是注意力

- 状态：validated
- 更新时间：2026-09-07

- 坑：#1394 中同一形状被 reviewer 连抓五轮（R29–R33）：一条规则在一个 consumer 里修好了，兄弟 consumer 没有。前四轮的修复动作都是"在当轮被指出的那一处也补一份"，给出的防护是"下一轮我会更仔细"。第五轮才改成共享投影 + 必填字段。
- 根因：把**可枚举的遗漏**当成注意力问题。第 N 个 consumer 是否被覆盖，不由记忆保证，由类型或测试保证。加注意力的修复必然复发，因为它没有改变"漏掉时系统会不会告诉你"。
- 触发条件：同类缺陷第二次出现；修复动作是"在另一处也补一份"；给出的防护是"下次记得 / 更仔细 / 加个注释"。
- 修复：把决定从"各 adapter 各自记得"改成**从单一来源投影一次**，并让承载该决定的字段在**每个可能承载它的返回类型上必填**——漏掉从此是编译错误。改成必填当场就多暴露了一个作者没数出来的承载点。
- 防护：第二次同形状时停手，先回答一个问题：**"能不能让漏掉变成编译错误或测试红？"** 能——先做这件事，再修当轮那一处。不能——必须写出全部承载点清单并逐个断言，且把"为什么不能机械保证"写进 commit body。禁止以"下次会记得"结案。
- 来源锚点：PR #1394（zts212653/clowder-ai，review 中）；pre-squash 轮次 commit `926174fc4` commit body（"Fifth time in this PR that a rule I fixed in one consumer was missing from its siblings"）
- 原理：**注意力不可复利，约束可以。** 第二次同形状是坐标系信号，不是勤奋度信号——它说明"正确"当前依赖人的记忆，而记忆是这条链上唯一没有测试覆盖的环节。

- 关联：LL-019（过度修复反模式）| LL-020（补丁数量是方向信号 N > 3）| LL-021（根因追溯深度）| LL-105（约束怎么加才真的生效）

### LL-105: 对象展开会绕过 TypeScript 多余属性检查——加字段必须先改类型

- 状态：validated
- 更新时间：2026-09-07

- 坑：给若干 router result 补一个新字段，实现写成 `{ ...base, newField }`。`tsc` 全程沉默，字段在运行时确实存在，但类型上从未声明——下游 consumer 读不到它。"字段已经加上了"是假的。
- 根因：excess property check 只作用于**对象字面量直接赋值给带类型的目标**；spread 产生的属性不参与该检查。于是"加字段"这个动作在 spread 写法下**没有任何编译期反馈**，加对加错一样安静。
- 触发条件：用 spread 给已有类型的对象补字段；尤其是跨模块 / union / 投影类型，作者无法一眼看全所有承载点。
- 修复：先在共享类型上声明字段，且声明为**必填**，让每个返回点被迫自己回答；实现随后跟上。必填立刻暴露了一个此前没被数到的承载点。
- 防护：
  1. 新增跨模块字段时**先改类型、再改实现**；用 `tsc` 的红当作"承载点清单"，不靠人工枚举。
  2. **必填优先于可选**：可选字段的遗漏永远不会报错，等于把这条规则重新交还给记忆（见 LL-104）。
  3. 不要用"运行时打印出来了"证明字段已接通——那只证明写入端，不证明类型面，也不证明读取端。
- 来源锚点：PR #1394（zts212653/clowder-ai，review 中）`packages/api/src/domains/github-signals/WaitWakeDisposition.ts`；pre-squash 轮次 commit `926174fc4` commit body
- 原理：**类型检查只在你给它检查点的地方帮你。** spread 是一个静默的逃逸口——它不报错不代表它同意，只代表你没问。

- 关联：LL-104（同形状要加约束）| LL-024（状态字段多点写入会复发蜘蛛网）

### LL-106: 缺证据时的默认值按"哪侧损失不可逆"定方向——同一条流水线上两个默认可以相反

- 状态：draft
- 更新时间：2026-09-07

- 坑：同一次观察缺少求值证据时，两个下游动作各需要一个默认：是否推进游标、是否唤醒 owner。作者第一版为了"一致"给了同向默认，并写了一句"absence is not permission"的注释——而代码实际在证据缺失时放行唤醒。注释与代码互相矛盾，提交前才被作者自己抓到。
- 根因：把"默认值方向"当成风格一致性问题（两个都关 / 两个都开看起来更整齐），而没有按每一侧的**损失可逆性**分别求解。
- 触发条件：一次判定的结果被多个动作消费；证据可能缺失；作者倾向给所有分支同一个 fail-closed / fail-open 方向。
- 修复：分别定向，并把不对称写进注释——缺证据**不得推进游标**（游标前进会永久销毁反馈，不可逆），但缺证据**不得扣住唤醒**（扣住会销毁 owner 唯一的通知，比多一次噪音更严重）。注释改为陈述这个不对称，而不是断言一个并不存在的对称。
- 防护：给缺失证据定默认时，对每个消费动作单独问一句"**这一侧做错了，下一次观察能不能自愈？**"——不能自愈的那一侧才是要拒绝的方向。禁止用"保持一致"作为默认值方向的理由。注释必须陈述实际不对称：写反的注释比没有注释更贵。
- 来源锚点：PR #1394（zts212653/clowder-ai，review 中）；pre-squash 轮次 commit `926174fc4` commit body（"The two defaults point opposite ways on purpose"）
- 与 LL-100 的关系：LL-100 讲护栏的**爆炸半径**不能超过它保护的风险域；本条讲同一判定下**每个消费动作的默认方向要各自求解**。前者约束广度，后者约束方向，不是同一条。

- 关联：LL-100（fail-closed 要按 claim 分层）| LL-019（过度修复反模式）

### LL-107: 按位置改文件、按尾部读检查器、push 与 gate 并发——三种"看起来没问题"

- 状态：validated
- 更新时间：2026-09-07

- 坑：同一个 session 里三次被同一类错误咬到：(1) 一次 `str.replace` 静默 no-op，文件根本没改，后续步骤全部建立在幻觉上；(2) 按**行号**给多个 stub 补字段，而行号已被同文件先前插入的测试顶移，字段落进了新测试里造成重复键，Biome 红；(3) 在同一条命令里 `push` 和 `pnpm check`，push 因此没有等 gate，红是在 CI 上才被发现的。
- 根因：三次都是用一个**不保证覆盖 / 不保证完成的观察**替代了确定性证据——位置替代匹配、输出尾部替代完整输出、并发替代顺序。三种写法的共同点是**失败时保持沉默**。
- 触发条件：批量改同类 stub 或多处同名字段；检查器输出较长时用 `tail` / `head` 截读；把 push 与本地 gate 写进同一条命令或同一批后台任务。
- 修复：改文件一律用**精确字符串匹配 + 断言匹配计数**；检查器输出**完整读取**；`pnpm check` 绿了**之后**才 push。
- 防护：
  1. 任何"改 N 处"的编辑，把 N 写进命令并让"不等于 N"直接失败；不用行号定位正在变化的文件。
  2. 不 `tail` 检查器 / 测试输出——被截掉的恰恰是你没预期到的那一条。
  3. gate 与 push 分成两步，中间自己看一眼结论；顺序本身就是证据。
- 来源锚点：PR #1394（zts212653/clowder-ai，review 中）；pre-squash 轮次 commit `ff26269bc` commit body（"Exact-string edits with an asserted match count are the only ones that tell me when the file is not what I think it is" / "Gate first, then push"）
...
### LL-109: 通知订阅不是执行权威——两者共用一个 threadId 字段，语义完全相反

- 更新时间：2026-09-20

- 坑：为修跨 thread 误投，我提议把 `PrTrackingStore`（`(repoFullName + prNumber) → { catId, threadId, userId }`）接进 custody，用它解析 review 的 holder thread。评审直接否掉：它的契约原文是 "route **notifications** to the correct cat/thread"，注册 `why` 写的是 "**Notify** this thread about external GitHub activity"，而且同 subject 重注册会**直接覆盖 `threadId`**（`TaskStore.ts:133`）。
- 根因：两个语义完全不同的概念**共用同一个字段名和同一个形状**——"事件通知投到哪"和"谁有权执行、执行应落在哪"都是一个 `threadId`。形状相同让它看起来可以复用；但订阅是**可覆盖、可迁移、最后写入者赢**的，执行权威必须是**有 generation、有争用语义、不可被旁人改写**的。接上去等于让最后一次 `register_pr_tracking` 改写 review holder。
- 触发条件：任何"我需要一个 X→thread 的映射，而系统里刚好有一个 X→thread 的表"的时刻。**字段形状匹配不是所有权匹配。**
- 防护：
  1. 复用一个映射前先读它的**写入者是谁、覆盖语义是什么**，不是只读它的类型签名。
  2. 判据一句话：**这张表被覆盖时，谁会受伤？** 订阅被覆盖只是通知换地方；执行权威被覆盖是责任丢失。二者不能共用同一个真相源。
  3. 在文档里同时"列为锚点"又"下文否决"是矛盾不是细微差别——两处必须同时改。
- 来源锚点：`docs/architecture/cross-thread-protocol.md`（迁移步骤 1′ 的撤回）；`packages/api/src/infrastructure/email/PrTrackingStore.ts:2-5`；`packages/api/src/routes/callbacks.ts:5595`
- 原理：**同形不同权。** 两个结构一样的映射，可以一个是缓存、一个是契约。

- 关联：LL-108（真相在对象里不在计数器里）

---

### LL-110: 幂等不等于资格——CAS 解决的是竞态，不是"谁才是对的那个"

- 更新时间：2026-09-20

- 坑：设计跨 thread 邀请协议时，我写了"邀请单次、幂等消费"，以为这样就安全了。评审指出：幂等只保证**不会被消费两次**，完全没回答**当同一个 `catId` 有多个同等合格的在线 endpoint（平行 invocation）时，谁才是正确的接受者**。按幂等实现，答案是"先到的那个"——**用到达顺序裁决正确性**。
- 根因：把**并发控制**当成了**语义裁决**。CAS / 幂等键 / generation 解决的是"同一个操作不被重复应用"；它们对"这个操作本来就该由谁执行"没有任何意见。一旦把前者当后者，系统会在歧义处静默地选一个，而且看起来完全正常——没有报错、没有冲突、状态一致。
- 触发条件：同一身份存在多个活跃执行体（平行 invocation、多设备、多 worker）时的任何"领取 / 认领 / 接受"动作。
- 防护：
  1. 歧义必须**显式建模**：解析出 0 个或 >1 个合格 endpoint 一律 **fail closed**，返回 `ambiguous_endpoint`，升级到显式选择或人工批准。
  2. **到达顺序永远不是裁决依据**。如果实现里"先到先得"是正确行为，必须写明为什么顺序在这里具有语义。
  3. 把"谁可以领取"（eligibility selector）和"领取后成为谁"（admitted role）拆成两个字段——合成一个会让资格判断无处安放。
- 来源锚点：`docs/architecture/cross-thread-protocol.md` 邀请协议节（`ambiguous_endpoint`）
- 原理：**原子性保证"只发生一次"，不保证"发生在对的人身上"。**

- 关联：LL-106（缺证据时按不可逆损失定默认方向）

---

### LL-111: 旧路由不是正确性真值——拿新模型去对齐旧guess，等于把guess追认成标准

- 更新时间：2026-09-20

- 坑：设计 shadow plane 时我打算"推导 endpoint vs 实际 target"做比对，用差异率衡量新模型准不准。评审拦下：`actual` 恰恰是这次调查认定为**不可靠的猜测**——整份报告的结论就是"调用方填的那个 threadId 没有任何东西校验过"。拿它当参照系，等于把被否证的东西重新立为标准。
- 根因：缺标注时，人会本能地抓**手头唯一的既有值**当基线。但既有值的可信度正是被调查的对象；用它评分会让新旧系统"越一致越好看"，而一致恰恰意味着新系统复制了旧缺陷。
- 触发条件：任何 shadow / dual-write / 影子比对上线；任何"没有 ground truth，但有历史数据"的评估。
- 防护：
  1. `derived !== actual` 只能叫 **disagreement**，不能叫 error。命名上就把判断权留空。
  2. **正确性只能由独立标注计算**（operator 判定 / typed incident label），不能由与旧路径的一致性计算。
  3. 覆盖率与正确率**分开记账**：`null` 计入 coverage，不计入 mismatch；历史流量大面积 null 只说明回填不足，不说明设计准确。
  4. 只测 **prospective cohort**（新机制在投递前就已存在的那部分），不回溯打分。
- 来源锚点：`docs/architecture/cross-thread-protocol.md` 迁移步骤 2（shadow plane 指标口径）
- 原理：**被调查的东西不能同时当尺子。**

- 关联：LL-103（传判据不传缓存结论）| LL-109（同形不同权）

---

### LL-112: 授权生命周期不能挡在"报告受阻"的必经路上——自动化失效应降级自动化，不该冻结沟通

- 更新时间：2026-09-20

- 坑：为根治跨 thread 误投，我设计了"关系即授权"——必须存在 active `ThreadRelation` 才允许投递，卖点是"投错在结构上无法表达"。五轮评审、四次重写之后，上游 maintainer 直接否掉方向，并给出 shipped 先例：`cat-cafe#4255` *"retire local review custody"*（2026-09-03 合入，删约 8,088 行）。那套被删的东西，**和我提的是同一个形状**（lease / generation / replacement / issuer-route authority / reentry / verdict settlement / recovery），只是范围更窄。真实事故是：reviewer 已经做完 exact-HEAD review、产出三条 blocking finding，**却投不出去**，一路撞 `local_review_verdict_identity_unavailable → lease_not_active → replacement/generation → issuer-route authority`；最后是改用普通 typed A2A 消息才送到。修复方式是**做减法**，不是继续完善。
- 根因：我把"授权"放在了**沟通**的必经路径上。授权状态有四种坏法——缺失、过期、分歧、恢复失败——任何一种都会把"猫想说话"变成"猫不能说话"。而且**最坏的情况恰好在最需要的时候**：`escalation` 是系统坏掉时最该送出去的消息，而被卡住的猫正是最可能没有 active 关系的猫。**让"错误"无法表达，同时也让"急需"无法表达。**
- 触发条件：任何"必须先满足 X 才能发消息 / 才能上报 / 才能求助"的设计。尤其是当你被"结构上不可能出错"这个说法吸引的时候——它诱人，因为它听起来是根治。
- 防护：
  1. **判据一句话**：这条规则失效时，猫是"少了一个自动化便利"，还是"说不出话"？后者一律不做。
  2. 证据与授权只在**接纳真实 custody / 执行有后果或不可逆动作**时消耗，**不作为沟通、协调、求助、报告发现的许可**。
  3. 误投这类问题走 **discovery UX / 显式寻址 / 接收侧约束 / 诊断与警告**，不走**发送准入白名单**。
  4. 关系/图数据可以做可观测性、推荐、provenance、警告——但"非阻塞"必须是**不变量**，不能是"强制之前的临时阶段"。
  5. 同形先例要主动找：本次的致命证据是一个**已经合入的删除 PR**。设计前问一句"这个形状我们是不是已经删过一次"。
- 来源锚点：`zts212653/clowder-ai#1490`（maintainer correction）；`docs/architecture/cross-thread-protocol.md`（已标 SUPERSEDED）；本地佐证 `lease_not_active` 横跨 17 个源文件 + 三个 `Legacy*LocalReview*` shim
- 原理：**自动化失效应当降级自动化，不该冻结沟通。**

- 关联：LL-110（幂等≠资格）| LL-111（旧路由不是真值）

---

### LL-113: 先例核实不到时，验"论证实质"而不是停在"无法确认"

- 更新时间：2026-09-20

- 坑：LL-112 那次否决，对方援引 `cat-cafe#4255` 作为决定性依据。我去查，`zts212653/cat-cafe` **对本账号不可解析**——PR 看不到。此时有两条歧路：一是"无法核实"就照单全收（凭权威），二是"无法核实"就挂起不动（凭形式）。两条都错。
- 根因：把"核实先例"窄化成了"打开那个链接"。但论证的力量不在 PR 编号上，在**那个失败模式是否真实存在**。而失败模式是可以在自己能读到的代码里查的。
- 触发条件：外部方援引你无权限访问的 artifact（私有 repo / 内部文档 / 他人 thread）作为论据。
- 防护：
  1. 把 claim 拆成**不可验证的坐标**（PR 号、行数、日期）和**可验证的实质**（错误码、代码形状、残留 shim），分别记账。
  2. 实质能在本地查证时，**查证实质**并写明"坐标未核实、实质已佐证"——不要用"无法确认"当挡箭牌，也不要让不可核实的坐标给结论背书。
  3. 找**第一手亲历**：本次最强的佐证是我自己刚刚撞上的两个 409（`managed_hold_disposition_source_mismatch` / `a2a_dispatch_disposition_source_missing`）——我连自己的球都处置不掉，正是对方描述的那类冻结。
  4. 写进文档时**分层标注可信度**，不要把三类证据（不可核实 / 代码佐证 / 亲历）混成一句"已核实"。
- 来源锚点：`docs/architecture/cross-thread-protocol.md` 顶部 banner 的 "Verification status (honest)" 段
- 原理：**核实的对象是论证，不是引用。**

- 关联：LL-103（传判据不传缓存结论）| LL-112
