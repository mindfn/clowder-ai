...
关于这层改造还欠一个正面回答：它是整套设计里成本最高、也最可能收不拢的一步——agent 需要的能力宽度和受控出口天然互相挤压，这正是今天主流 agent 运行时都是自由 shell 的原因。如果它最终收不拢，本文的机制就从"事前保证"整体降级为"事后审计"：账本仍能重建谁做了什么，但排他、原子翻转、在途清单的账本推导都不再成立。改造的形态、代价与能力损失，是我们下一阶段必须回答、本文无法预支答案的问题。
