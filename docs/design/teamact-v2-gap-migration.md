...
provenance: >
  ...
  v2：对齐 paradigm v3（sol r14 APPROVE @ 2a3a08578）——概念系迁移
  （Offer/Claim/Attempt/HumanGate/三分量 token → Assignment/Grant/Run/
  approval gate/四段凭据），差距矩阵重挂 I1–I6 与 O1–O4，新增悬置处置
  事务差距（G19）与判据自检（§0.6）。
---
...
三轮讨论的核心结论：**现有机制大多不是要推倒的补丁，而是范式中正确概念的早期实现**。定向 @ 移交是 Assignment offered 态的会话级表达、"我来做"等责任声明是 accept（建立 Assignment）的推断式实现、F233 事件溯源是 ledger 的模式先例、F224 续接协调器是 Run 链的先行者。改造是**把推断升级为账本事务、把 thread 粒度收窄为 per-recipient、把散落机制接到统一账本**——不是 big bang 重写。

## 0.5 实测失效记录（范式 §1.3 五类失效的具体数据）
...
- 当前没有 authoritative 的 per-message × per-recipient `enqueued → delivered → seen → processed`，更没有把消息 processed 与 WorkUnit assigned / fulfilled 分开（传输 ACK ≠ 语义 context.ack，范式 §5.1）。
...
| G2 | offer/accept 账本事务 + Assignment/Grant 两关系（§2.2, §2.3；I1） | 言语行为 + F233 事后观测推断 | 无账本事务 CAS、无 per-scope Grant、无版本化；"两只猫都以为对方在做"无结构防护 | **Foundation** |
| G3 | per-recipient obligation membership（§5.1 传输/语义分离的义务侧） | seenCursor **已 per-cat**（`userId+catId+threadId` 分区），但义务判定从 raw thread cursor 向后扫描 | 缺 per-message obligation membership：单调 cursor 无法表达稀疏义务集合（I1b（§0.5）） | **Foundation** |
| G4 | hydration per-invocation-scope（§5.1） | 上下文注入按 thread 取材，不查义务归属 | 缺同一 membership projection 的消费侧（I1a（§0.5）） | **Foundation** |
| G5 | coordination ledger（§2.3 账本；I4） | F233 有职责归属观测 event log（模式已验证），协调域账本不存在 | G1-G4 的存储基座 | **Foundation** |
| G6 | Run 链 + lineage（§5.2） | F224 续接协调器已接线，无 WorkUnit/Assignment/Run 关联 | session 断片仍产生职责归属歧义 | Next |
| G7 | approval gate 配置 + offered/assigned 双层 SLA（§6） | 无；F233 诊断确认 operator 是唯一缺少职责超时保护的执行者 | 人的待办无认领态、无 SLA、无探测 | Next |
| G8 | 四段凭据 fencing + effect 准入串行化（§5.2；I2） | 无任何 fencing | 职责转移/取消竞态零防护（跨执行者职责转移已有真实场景，如额度中断后的职责转移）；缺 effect receipt 认证根 | Next |
| G9 | 两阶段 transfer 事务（§3.1：Core/Prepared digest 链、AND 授权集、sourceState、在途副作用 manifest；I3） | 职责转移靠会话约定（cross-cat-handoff skill） | 无事务身份、无授权绑定、无在途副作用清单；接收方"确认" = 会话回复，非绑定 digest 的 context.ack | Next |
| G10 | 静默执行失联探测（Run started + 心跳，§5.2；I6 探测半边） | F233 **已有 task 级 probe + wake（Phase B 已落地）** | 缺 Run heartbeat/lease 与 provider silent-death 检测（I5：探测粒度在责任项，不在实际执行）；心跳观测位点未与账本同串行化域（G19 suspend CAS 的前提，§3.4） | Next |
| G11 | resolve(complete) 产出不可变坐标 + 验证独立绑定（§3.3；I5） | review 实践有 hash 惯例（本范式历轮 review 即例），无结构化 | TOCTOU 靠 reviewer 自律 | Later |
| G12 | join / fan-in barrier（扇出拓扑的工程缺件；范式定位见 §1.4，不属交接闭环） | F086 状态机无正式 barrier | fan-out review 结果聚合靠人工 | Later |
| G13 | 投影分层（I4 全程落账的消费侧：审计序 / 因果树 / 执行泳道） | 单一壁钟时间线 UI | I2 实测痛点（§0.5） | Later |
| G14 | 验证独立形式化（I5：verifier ≠ responsibleActor，同源回避） | 家规文本 + 流程自律（平台层还受共享 GitHub 账号限制） | 无结构化校验 | Later |
| G15 | 消息投递协议（§5.1：传输 ACK 链 + 显式语义 context.ack 分离） | 主动 push 已会排队/启动目标 invocation 并把 envelope 送入执行；有 `clientMessageId` 幂等先例、消息级 queued/delivered/canceled、per-cat 连续 cursor | 缺 per-message × per-recipient membership 与 enqueued/delivered/seen/processed ACK；**传输 processed 与语义 context.ack 未分离**；hydration 仍按 thread 扫描；缺统一 pull discovery loop（与 G3/G4 同根，**并入 S1/S2**） | **Foundation** |
...
| G20 | 义务巡检循环 reconciler（§5.4；I6 的运行时载体） | **纯事件驱动 + 参与者自觉**：invocation 由消息触发、回合尽即眠；接球提醒 watchdog 是消息形态启发式（不读账本义务）；`hold_ball` 是执行者自设闹钟；§0.5 I4/I5（人工悬置、静默失联靠手动恢复）共同缺失的是**运行时驱动与兜底层**（各自的状态与机制已由 G6/G7/G10 分别定义，缺的是持续检查并推进它们的循环）——co-creator 定性："系统整体被动式，非有状态循环" | 缺账本驱动的常驻对账循环：desired（Assignment/SLA/处置时限）vs observed（心跳位点/处置落账）→ 差异触发 I6 态别处置；无 authority 约束（不能替行动/替批准/分派）待立为硬边界 | **Foundation** |
...
- 三视图：arrival-order 审计 / conversation 因果树（`replyTo` 已有，只做渲染）/ WorkUnit-Run 执行泳道。
...
11. **义务巡检 reconciler 权威化**（G20；依赖：**第 1 项**权威 Assignment/Grant——desired 读取的 SLA/处置时限字段随 Assignment 账本化，**shadow 数据不得驱动处置**（总原则）；**第 2 项**心跳账本化——observed 位点；涉 human 审批的巡检路径另依赖**第 5 项**；suspend/恢复分支另依赖**第 10 项**）：S0 起 dry-run 对照，达标后**逐处置类别转正**——先低风险类（催办 / 唤醒探测 / **机械重放式 re-offer**：候选集、路由规则与 policy 版本须已预声明，reconciler 不选人不扩集，需选择候选即降为 proposal），后 disposition proposal 类（suspend/恢复 transfer：**reconciler 只创建 proposal 与 detection evidence，不签任何关系授权**——授权由 policy 指定主体签署、commit 走既有 CAS 三段式，§5.4）；无 authority 硬边界入运行时校验（reconciler 凭据不含任何 WorkUnit 的 execute/decide/approve scope）；自身可靠性契约随转正落地（durable 水位保证**不漏扫**、允许重扫；**全部处置**经 durable outbox + 幂等消费，处置身份 `{workUnit, violationKind, observedVersion, dispositionKind, escalationStage}`；at-least-once + 账本 CAS 重复安全；自身生命迹象受外部健康检查）。**分层验收**：①全部路径——检测、证据落账、proposal/escalation 在时限内自动完成；②已有机器可执行预授权 policy 的路径——终点事务零人工；③需 human/quorum 当次授权的路径——分别度量 proposal→authorization 与 authorization→commit 时延，**不承诺零人工**（§6 人的 SLA 只能软治理）；催办误报有阈值可容忍，未经授权集的 suspend 零容忍（CAS 拦截）。

每项转正前提：S0 影子数据证明该语义在真实负载下成立；顺序可因 maintainer 对齐调整，依赖关系（1→2→3→4；4→6；1、2、3、4、6→10；**1、2→11**，human 路径另 5→11，suspend 分支另 10→11）不可倒置，第 3 项内部不可拆分晋升。
...
- **OQ-3** multi-operator 是否纳入本轮：建议不纳入（范式 §8 声明的验证边界外）。
- **OQ-4** RecoveryPolicy 的授权者起步形态：单 operator 拍板（现状事实上的恢复权威）还是 quorum；suspend 的治理信任根声明（范式 §8）如何映射到家规与 Hub 审批面。
...
| 2026-08-03 | G20 review（sol，5P2）修订：处置授权**三段式**拆分（reconciler 只创建 proposal/证据，不签任何关系授权——"policy 已存在"≠代签权）；第 11 项依赖补全 **1→11**（权威 Assignment/SLA，shadow 不得驱动处置）、human 路径 5→11；验收**分层**（proposal/escalation 全自动 vs 预授权路径零人工 vs human 当次授权路径不承诺零人工）；补 reconciler 自身可靠性契约（durable 水位/violation 幂等/at-least-once/外部健康检查）；"共同根因"降为"共同缺失的运行时驱动与兜底层" | 宪宪/claude-fable-5 |
| 2026-08-03 | G20 r2 窄核（sol，2P2）修订：可靠性契约**闭合语义**——"不丢不重扫"改为"水位保不漏扫、允许重扫、重复由下游吸收"；**全部处置**（不止 proposal）经 durable outbox + 幂等消费，处置身份补 `dispositionKind/escalationStage`（催办→升级→proposal 是不同处置不被误吞）；re-offer 限定**机械重放**（候选集/路由/policy 版本须预声明，reconciler 不选人不扩集，需选择即降为 proposal——封死变相分派） | 宪宪/claude-fable-5 |
