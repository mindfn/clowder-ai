...
provenance: >
  ...
  r22（lang 发布边界补充 + sol 分工指令）：§5.3 从"四层记忆分工"
  改为"三类 durable 语义"（责任/职权/进度真相、交接四要素之原文或
  稳定引用、effect 准入与终态证据）；存储拓扑与记忆分层降为实现
  选择、非范式内容。article 同步：⑨ 删四层实现句、§8 压成四级
  证据边界（失效已观测/局部机制已验证/统一本体未实现/组合效果
...
- **耦合式（TeamAct 的选择）**：把"给职权"和"给上下文"绑成一个可审计的事务（§3.1）。前任在场就用上他的合作：接手者只在满足声明的就绪门槛并留下绑定证据后才获权（消除"无就绪证据便接权"的窗口，§3.2），责任没有空窗，还没回来的外部调用被列成清单随交接继承，人类接手者拿到的是整理好的上下文并按门槛显式确认。
...
选择判据（§1.2）回答"何时选 TeamAct"；本节回答它在设计空间中的坐标——与相邻传统各自最强形式的真实差异。（读者提示：本节是写给熟悉 multi-agent 文献的读者的精确划界，每个名词是一支研究传统，表格第一列附一句它是什么。不需要这层精度的读者，带走一句结论即可直接跳到 §2：**TeamAct 的 offer/accept 复用 Contract Net 形态，承诺概念承袭联合意图传统，一致性底座复用分布式系统成果；本文聚焦的组合是把职权续接、状态连续与审计耦合成一等事务**。）比较轴：**分配**（谁接活）、**承诺**（义务的表示与维持）、**排他**（副作用互斥）、**状态连续**（换手时信息如何延续）、**故障恢复**（参与者失效怎么办）、**拓扑**（控制权在哪）。每个传统按其**最强形式**呈现，不做"被本文吸收"式降格：
...
**恢复变体 ≠ 解耦式 reclaim**：失联恢复仍走 §3.1 的事务形态，只是各输入**显式退化**——RecoveryPolicy 授权替代前任签名；快照退化为最后 durable 检查点；在途清单退化为按账本准入记录推导（继任者背悲观对账义务）；context.ack 退化为继任者按锚定 ReadinessPolicy 对**重建上下文**留下的绑定证据——退化的是证据的合作来源（无前任配合），三元组绑定与门槛校验不变；`readinessPolicyVersion` 仍必须等于 WorkUnit 契约当前版本，恢复场景没有第二套门槛来源。"就绪先于激活"的门槛保留。TeamAct **有意不提供**"先认领、后重建"的 reclaim 原语：宁可让工作经 `suspend` 进入可稳定保持的悬置态（见下）等待能满足就绪门槛的继任者或人工升级，也不接受"持权但无法行动"的接管——代价是恢复接管可能更慢，这是显式取舍。纯 fence-and-reclaim（就绪与获权分离）是模型外部的基线设计（§1.2、§8），不是本节路径的别名。
...
从安全续接反推，三类语义必须 durable 可获得——前任不在场时仍可用：**其一**，责任、职权、进度与全部迁移真相（协调账本，回放即重建）；**其二**，交接所需的事实、意图、边界、行动（持久化原文，或指向权威存储的稳定引用）；**其三**，高风险 effect 的准入记录、在途清单与可信终态证据（§3.1 / §3.4）。三类齐备后存储拓扑自由：各域各有权威存储、以稳定 ID 互联，账本不是数据湖；具体的记忆分层属实现选择，不是范式内容。铁律不变：**任何决定 Assignment / Grant 状态的信息不得只存在于易失的工作上下文**。会话更替后的重建三源：交接快照或最后检查点（意图与进度）+ 账本回放（责任与职权状态）+ 知识检索与历史回读（细节）。
