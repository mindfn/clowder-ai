...
**触发条件（问题域声明）。** TeamAct 上场的条件就两条，缺一条都轮不到它：**一件没做完的工作真的换了人**，而且**接手的人必须依赖前任留下的状态**——包括前任脑子里还没写下来的意图、已经发出去还没回来的外部调用。没有换人，就没有跨 Actor 续接问题；换了人但不依赖前任状态，重新分配或认领就够了。

**两个必要问题。** 进入这个域，系统必须同时解决两个问题——必要性来自问题域本身，与后面选哪族解法无关：
...
**这个问题域出现在什么环境（A1–A5）。** 下面五条描述让触发条件高频出现的系统环境。它们是适用场景的画像，**不是能推出机制的公理**——处在这个环境里的系统，按上面的判据选解耦式同样可以活得很好：
...
**全文论证地图。** 选定耦合式之后，本文的任务是给出它的一个完整构造，并给出可检验的验收标准：§2 定义状态空间——最小本体：两个实体、两个版本化关系、一本账；§3 构造核心事务——Handoff 闭环，及其失效恢复与悬置变体；§4 收拢为参与者视角的最小循环；§5 是实现与可靠性策略层，其中 §5.4 回答"账本不会自己推进"（reconciler）；§6 把人类纳入同一本体。验收在 §7：六条可机器检验的不变量 I1–I6，并主张**在声明的环境假设下，I1–I6 成立即达成 O1–O4**——这个蕴含本身是可检验的主张，反例即有效批评。§8 给出反驳规则（Pareto 支配判定）、不适用域与局限。

### 1.3 五类失效 = 闭环破损的五种形态

§1.2 声明的问题域不是纸上推演，本节给它的实证证据。数月生产实践反复出现的失效（实测记录见 gap 文档），在本模型中获得统一解释——**每一类都是职权线或上下文线的断点**：
...
选择判据（§1.2）回答"何时选 TeamAct"；本节回答它在设计空间中的坐标——与相邻传统各自最强形式的真实差异。比较轴：**分配**（谁接活）、**承诺**（义务的表示与维持）、**排他**（副作用互斥）、**状态连续**（换手时信息如何延续）、**故障恢复**（参与者失效怎么办）、**拓扑**（控制权在哪）。每个传统按其**最强形式**呈现，不做"被本文吸收"式降格：
