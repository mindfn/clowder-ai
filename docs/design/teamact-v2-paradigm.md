...
单 agent 的行为可以用"思考—行动—观察"的内循环刻画（ReAct）。当多个**长命的、有身份的** agent 与人类组成持续协作的团队时，出现了内循环之外的一层问题：工作由谁认领、义务算在谁头上、上下文如何在交接中不失真、记忆如何跨越单次会话的生灭、认领者悄悄消失时谁来发现。本文提出 **TeamAct**：一个以 *WorkUnit / Claim / 协调账本（coordination ledger）* 为核心的**责任协调模型**。它定义六个协调实体、一个八步协作回合、上下文传递与记忆管理的分层设计、职责交接协议，以及六条不变量。模型的立场是：**执行状态回答"程序怎么继续跑"，责任状态回答"每项职责是否始终有明确承担者"**——后者需要独立于任何单个 agent 的显式结构。

**阅读路线**：第一次阅读先看 §1–§3，建立“为什么需要责任层、六个实体怎样进入一个回合”的心智模型；需要实现消息与上下文边界时读 §4，需要处理职责转移与过期执行实例时读 §6–§7；§5、§8–§9 与附录是记忆、存储和运行时语义的参考层，不必线性通读。
...
- **A2 — 人是团队成员**：人不只是发起者与消费者——人拍板、审批、也承接工作；因此**人的承诺同样可能逾期或失去跟进**。
...
| **F4 人工责任悬置** | 人工审批或决策长期未处理；系统对 agent 有超时机制，对人的职责却没有等价保护 | 人的待办的认领状态、SLA 与超时探测 |
| **F5 执行静默失联** | 执行者因外部故障停止活动，无失败信号，靠人偶然发现 | 执行的生命迹象（心跳/租约）与探测 |
...
- **顺序移交（handoff）**：当前 WorkUnit 完成后创建后继 WorkUnit，不改变当前单元的 holder。
...
| **Claim** | 执行者对 WorkUnit 的排他认领，1:1 | 由 CAS 建立；持有 lease（心跳续约）与 generation（职责转移计数）；**跨 attempt 存续** |
...
- **capability**：擅长什么（辅助工作分派与职责移交的依据）；
...
| **Acquire / Validate Claim** | 认领或续接，三分支；**成功即原子产生 `attempt.started` 并旋转 attempt generation** | ①首次执行：CAS acquire（失败 = 别人先到，安静退出）；②**原 holder 续接**：校验并续租自己既有的 claim（同 claim 下 attempt+1 的合法入口——新 attempt 持新 attemptGeneration，**分区后复活的旧 attempt 因 attemptGeneration 过期被 fence**，§7 N2）；③职责转移接收：transfer accept（§6.1） |
...
**检查点是执行静默失联场景的必要补充。** 正常收尾会留下契约或收尾记录，但执行静默失联（F5）没有收尾——因此执行中必须在关键点落 **durable 检查点（continuation capsule）**：当前进度、未完成的副作用清单（已准入未观测的 intent）、下一恢复点。没有检查点的三源重建只对可控中断有效；检查点频率是策略（按步骤、按时间、按副作用准入前），存在性是规范要求（附录 B7）。
...
| 形态 | 场景 | 协议 |
|------|------|------|
| **顺序移交（handoff）** | 正常协作流：**当前单元完成，后继工作形成新的责任** | 当前 WorkUnit → complete；同时创建（或早已 split 出）**带依赖关系的后继 WorkUnit** + 交接契约 + 定向 offer → 后继执行者对**新单元**走完整回合（Inspect → Acquire）。**handoff 不在同一 WorkUnit 上更换 holder**——那是 transfer 的语义，两者混用会撞 Claim 排他不变量 |
| **职责转移（transfer）** | 认领者中断或被更换，**同一 WorkUnit 保留进度并更换承担者** | 双条件缺一不可：**授权**（定向 transfer offer；签发者必须是当前认领者或有授权的调度者——否则任何人可自签自抢）+ **并发安全**（携带期望的 fencing token 原子接受转移）。接收者同时承接职责：custody 连续转移，并确认全部进行中副作用 |
| **释放认领（release）** | 认领者主动放弃且无指定接收者 | 显式释放 → WorkUnit 回到可认领状态 → 义务回落 offer 级 SLA（**无主阶段不是无监督**：offer 发起方承担催办责任） |

![动图 1：同一 WorkUnit 的状态变化与安全职责转移](./assets/teamact/animation-custody-transfer.gif)

*动图 1：W-42 从 Offer、A 的 Claim/Attempt、静默失联，经过授权的 TransferOffer 将职责原子转移给 B，再从检查点恢复并完成。WorkUnit 身份不变；变化的是 Claim holder、Attempt 与 fencing token。*
...
| `transfer.offered` | active | 有权签发者授权 B 接收职责；A 仍是当前 holder | #1 stalled（待处置） | offer 携期望 token、签发者和有效期，只提供授权，不自行改 holder |
...
- **只记录终态永远探测不到执行静默失联**（F5）——探测的对象是"生命迹象的缺失"，不是"失败的出现"。
...
| **N2 Fenced effects** | token 是 `{工作纪元, 认领代数, 尝试代数}`；取消、职责转移、Attempt 激活分别旋转对应分量。完整 token 覆盖账本、可变状态、检查点/Outcome 与外部副作用准入；过期动作要么被拒，要么成为已记账、可对账的进行中义务 | 旧 Claim 或分区复活的旧 Attempt 继续写入、重复发消息或制造静默副作用 |
| **N3 迁移全部落账** | 认领、交接、Attempt 起止/心跳、产出坐标全部 append-only，可回放 | 责任只能靠聊天猜、执行静默失联没有可观测起点 |
...
- **账本只做一件事**：append-only 地记录责任状态迁移事件——工作单元生命周期、offer、认领与职责转移、尝试起止与心跳、副作用准入、产出坐标、迁移。事件集必须**闭合且含生命周期起点**——没有"开始"事件就探测不到执行静默失联。
...
可逆的、不越权限边界的决策由执行者自决（事后可审计）；不可逆操作、愿景级变更、跨执行者僵局升级给治理者（人）。升级本身是一个 HumanGate WorkUnit——带 SLA 与探测，不是把职责交给一个不可追踪的通道（§2.3）。
...
- **协议遵守不是硬约束。** LLM 执行者靠提示词约定 + 运行时门禁兜底，仍会漏。账本让职责无人承接或执行失联**可见**，却不能使这些失效**不可能发生**——本模型是可观测性与恢复力的设计，不是形式化正确性的证明。
- **人的 SLA 是社会约定。** 对人只能催促与升级，不能强制。
- **协调有开销。** Orient 的检索、落账、探测都消耗资源——本模型只适合价值密度高、职责转移真实存在的工作（适用判据展开见 [tech-article](./teamact-v2-tech-article.md)）。
...
| D2 | offer 1:N，claim 1:1 CAS，split-before-claim；fencing token = {工作纪元, 认领代数, 尝试代数}；fencing 覆盖落账/状态/副作用准入三层；transfer = 授权 + CAS 双条件 | 否决：无限制并行 claim；不可参数化的普适 Single Writer；只 fence 落账（副作用漏防）；release-then-reclaim（留无主窗口）；token 少任一分量（取消、职责转移或过期 attempt 漏防）；CAS 当授权（可自签自抢） |
...
- **B2 副作用准入**：外部副作用两阶段——①准入：意向（携**完整三分量 token**{纪元, 认领代数, 尝试代数} + 幂等键）与认领迁移、尝试激活在协调账本同一串行化域内提交，token 校验发生于此；②执行：对已准入意向执行并记录观测结果。准入成功即不可撤销的派发义务，其后的职责转移/取消必须承认这些进行中副作用。能力允许时校验可下推至外部系统（须能持久化并比较完整 token——普通条件写只能 fence 外部资源版本，fence 不了工作归属）；两者皆不可时诚实降级为检测 + 对账。
- **B3 职责转移细则**：transfer offer 结构含 {工作、双方、期望 token、签发者、有效期}；接受是单次原子提交——校验未过期、未消费、签发者权限，然后消费 offer 并完成认领转移（代数 +1）。
