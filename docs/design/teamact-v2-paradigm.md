...
- **解耦式（fence-and-reclaim）**：succession 走 lease 失效 + fence + 重新认领；readiness 走共享状态自行重建。代价：继任者持权但未就绪的缺口、custody 出现无主窗口、放弃前任在场的合作价值（封存在途副作用、移交未外化意图）、人类继任者难以"回放重建"；
- **耦合式（TeamAct 的设计主张）**：把两者耦合为一个可审计事务（§3.1），利用前任在场的合作，保证获权即就绪、custody 无缝、在途副作用显式随交接继承、人类继任者以推送式策展上下文 + 显式接受完成。

**耦合是设计选择，不是逻辑必然**。选择判据——计划性易主为主、存在需前任封存的在途外部副作用、审计/SLA 要求 custody 无缝、继任者含人类 → 耦合式；崩溃主导、共享状态即是全部上下文、认领 churn 便宜 → 解耦式足够。TeamAct 同时内含两者：合作式 Handoff 是一等路径，RecoveryPolicy 失联恢复（§3.4）正是受治理的 fence-and-reclaim——完整系统两条都要，本文的贡献是把合作式路径一等公民化并给出闭环判据。
...
|---|---|---|
| **Contract Net**（Smith 1980） | 分配轴的成熟答案，且不止首次分配：支持**递归转包**，award 可携带执行所需数据（状态连续轴的早期形态） | 不处理职权版本化 fence、在途副作用继承、换手期 custody 审计。本文 offer/accept 是 CNP 形态的特例；CNP 不要求也不提供交接闭环 |
| **联合意图/SharedPlans/STEAM**（Cohen–Levesque、Grosz–Kraus、Tambe） | 承诺轴 + **故障恢复轴**的强答案：含失败监测、团队重组、任务再分配 | 承诺与修复位于团队心智模型层，不以持久账本、版本 fence、可审计 custody 为对象。Responsibility 是承诺的账本化、可迁移化——差异在表示层与可验证性，不在"谁先想到修复" |
| **Blackboard**（Nii 1986） | 状态连续轴的强答案；**控制模型本就多样**（控制可位于知识源、黑板或独立控制器），可实现调度与互斥 | 其互斥是调度级，非带版本 fence 的职权级；贡献者更替时的责任归属与审计不是其主题。"黑板 = 无排他职责"是错误刻画（r7 已纠）；正确差异是：黑板对 succession/custody 无一等语义 |
| **FIPA ACL** | 通信语义轴：communicative acts 对信念/目标/意图有**形式语义**，不是纯传输 | 语义效果作用于心智状态，不落为持久、可 fence 的职权状态。纪律 1（消息到达 ≠ 职权改变）针对后者；对 ACL 的正确表述是"语义必要，但不落账" |
...
- 职权持有者失联（F5）→ 生命迹象断供被探测 → 经授权的 transfer 把职权线接到新 Actor——该路径即受治理的 **fence-and-reclaim**（§1.2 解耦式）：前任缺席，无法利用其合作，只能接受更弱的就绪保证与悲观对账；
...
- **论断的自指风险（显式对冲）**：本文诞生于一个已按交接方式运转的团队，失效目录也采自它——存在"把现有协议的特征写进假设，再由假设推出现有协议"的循环风险。对冲方式：不作普适性主张（Abstract）、问题域与触发条件显式声明（§1.2）、承认域内存在解耦式替代并给出选择判据（§1.2）、相邻传统按最强形式对照（§1.4）。剩余自指以可反驳形式暴露：若有系统在触发条件成立时以更低代价满足 I1–I6，即构成有效反驳。
- **不适用域与不必选域**：A5 不成立（工作近幂等、重复无代价）→ 黑板/群体智能一族；工作可全程分解为独立小包扇出汇聚 → 编排模式；未发生易主、或继任不依赖前任状态 → 纯任务分配即可；处在域内但崩溃主导、共享状态即是全部上下文、认领 churn 便宜 → 解耦式 fence-and-reclaim 足够（§1.2），无需耦合事务。
