...
Multi-agent 系统有两种基本形态。一种是**中心化编排**：一个编排者拆任务、派工作、收结果，worker 用完即回收。另一种是**去中心化协作**：一群长期存在的 agent 和人像团队一样共事，工作在成员之间流动，职权的创建与迁移分散在各成员手里，没有谁掌握全局。成员的身份靠稳定锚点维系——运行它的 session、模型甚至供应商都可以换，"这还是同一个成员"不变。中心化编排已经有成熟的框架；去中心化协作才是本文的对象。

去中心化协作最危险的时刻是**工作换手**。换手时系统必须回答两个问题：**接手者有没有合法的职权？他是否已经知道接着该干什么？**——前者是职权续接，后者是上下文就绪。

**TeamAct 是针对这个问题域的协调范式。** 它不声称自己是去中心化协作的普适答案；它只针对一类明确的场景：**一件没做完的工作要换人，而且接手者必须依赖前任留下的状态才能安全继续**。这类场景有两族解法：**解耦式**——旧租约过期、新人重新认领、自己从共享状态重建，很多任务队列系统就是这么做的；**耦合式**——把"给职权"和"给上下文"绑成同一个事务，缺一个就不生效。TeamAct 选择耦合式。这是一个**带判据、可反驳的设计选择**，不是逻辑必然；什么时候该选哪族，§1.2 给出判据，§8 给出本文明确不适用的场景。
...
- **A3**：工作比会话长。一件事横跨多次进程重启、上下文压缩、供应商中断，干到一半换人是常态而非例外；
- **A4**：责任要经得起审计。谁在什么时候对什么负责，事后必须能查；验证的人不能是干活的人；
- **A5**：关键副作用不能重复做。部署、花钱、合并、对外承诺——这些动作做两遍有真实代价。

**先把话说诚实：A1–A5 描述的是适用场景，不是充分条件。** 五条全部成立，也推不出"必须有耦合的 Handoff 事务"。反例很容易造：进度持续写进一个复制账本；不能重复的副作用用带租约的凭据保护；有人失联就作废旧租约；接手的人自己重新认领、自己从共享状态重建。这个系统满足全部五条假设，却没有任何点对点交接。所以在这个场景里，真正**必要**的只有两个问题：

1. **职权续接安全**（succession）：不能重复做的事，它的执行权从一个人转到下一个人，全程不出现"两个人同时有权"、不出现伪造，事后能查；
2. **继任者上下文就绪**（readiness）：接手的人动手之前，得知道的都知道了。

做没做到，用一组**不偏向任何方案的结果性质**来衡量——下面四条不引用 TeamAct 的任何机制，评价解耦式基线和其他传统同样适用：
...
TeamAct 真正上场的条件比 A1–A5 更窄，就两条，缺一条都轮不到它：**一件没做完的工作真的换了人**，而且**接手的人必须依赖前任留下的状态**——包括前任脑子里还没写下来的意图、已经发出去还没回来的外部调用。不换人，那是纯分配问题；接手不依赖前任，那是纯恢复问题。

满足条件之后，仍然有**两族解法**可选：

- **解耦式（fence-and-reclaim）**：职权靠"旧租约失效 + 作废旧凭据 + 重新认领"续接，上下文靠接手者自己从共享状态重建。代价：接手的人拿到权了但可能还没准备好；中间有一段谁都不负责的空窗；前任明明在场却帮不上忙——他知道哪些外部调用还没回来、他脑子里的意图没写下来，这些都传不过去；换成人来接手，更没法靠"回放账本"重建现场；
...
**怎么选？** 换手多数是计划内的（前任在场能配合）、有发出去没回来的外部调用需要前任封存、审计要求责任一刻不能没人管、接手的可能是人——这四条占得越多，越该选耦合式。反过来，换手多数因为崩溃、共享状态就是全部上下文、重新认领很便宜——解耦式就够了，别为它付事务的成本。**TeamAct 从头到尾用耦合形态**：正常换手走合作式 Handoff（§3.1）；失联恢复也不例外，只是各项输入降级（§3.4——"就绪了才给权"的门槛不放松）。纯解耦式是**模型外部的合法基线**：它站在本文的本体之外，用来对照、也用来反驳本文（O1–O4、§8）——我们不把它收编进来。
