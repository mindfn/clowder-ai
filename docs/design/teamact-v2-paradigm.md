...
**两个必要问题。** 这个域里的"工作"还带着两个前提约束，先说破：关键副作用不能重复或并发执行（做两遍有真实代价），且责任归属要经得起事后追查——它们正是下文 A5/A4 描述的环境特征，在此作为问题域前提显式声明。带着这两个约束换手，系统必须同时解决两个问题——必要性来自触发条件加上述前提，与后面选哪族解法无关：
...
**这个问题域出现在什么环境（A1–A5）。** 下面五条描述这个问题域的典型环境：前三条（长命、人参与、跨会话）让换手频繁发生，后两条（审计、排他副作用）让换手失败的后果显著。它们是适用场景的画像，**不是能推出机制的公理**——处在这个环境里的系统，按上面的判据选解耦式同样可以活得很好：
