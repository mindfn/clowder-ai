...
TeamAct 真正上场的条件比 A1–A5 更窄，就两条，缺一条都轮不到它：**一件没做完的工作真的换了人**，而且**接手的人必须依赖前任留下的状态**——包括前任脑子里还没写下来的意图、已经发出去还没回来的外部调用。没有换人，就没有跨 Actor 续接问题；换了人但不依赖前任状态，重新分配或认领就够了。

满足条件之后，仍然有**两族解法**可选：

- **解耦式（fence-and-reclaim）**：职权靠"旧租约失效 + 作废旧凭据 + 重新认领"续接，上下文靠接手者自己从共享状态重建。代价：接手的人拿到权了但可能还没准备好——就绪与获权不绑定，就绪与否要另行证明；中间有一段责任空窗；前任在场的合作价值（封存在途副作用、移交未外化意图）**不是该方案原生保证的**——可以另行补建上下文通道，但那是额外工程；人类接手者同样可以被服务，只是方案本身不提供推送式策展与显式确认的保证；
