...
Multi-agent 系统构成一条光谱。**中心化**端（orchestrator-workers / agent team）：职权的创建与迁移集中于编排者，worker 间通常无需点对点交接。**去中心化**端：以稳定身份锚点跨 Run/session 存续的逻辑 Actor（agent 与人）组成团队，工作在参与者之间流动，职权的创建与迁移分布于对等参与者，没有任何一个上下文拥有全局。前者已被成熟的编排模式充分覆盖；后者在工作换手时暴露出两个必要问题——**职权续接是否安全？继任者上下文是否就绪？**
...
持久化义务状态仍不等于义务会被持续推进：在消息触发、回合结束即休眠的运行时里，失联者无法负责报告自己的失联。因而 TeamAct 还需要一个无决策与分派职权的常驻 **reconciler**，跨 Run、session 与 Actor 对账义务及生命迹象，把不变量落实为有界探测与处置；它不同于单个 Run 内围绕目标行动的 goal loop（§5.4）。
...
这不是先进性排序，而是权力与状态边界的选择：若一个编排者能合法拥有全局分派权并掌握足够上下文，中心化更简单；若责任、职权与上下文确实分散，才需要 peer coordination。现实系统通常混合两者——一个 WorkUnit 内可中心化编排临时 worker，WorkUnit 之间及长期 Actor 之间用 TeamAct 保持责任连续。诚实边界：TeamAct **不冒称覆盖全部 multi-agent**；纯中心化场景请直接使用编排模式，本模型与它的连续性仅在 §2.2 的 delegation 配置处说明。

### 1.2 系统形态假设

- **A1**：逻辑参与者跨任务存续，并有稳定身份锚点（Actor ID；Run、session、模型或供应商可更换），能力档案与责任记录通过该锚点连续关联；
...
- **Actor**：由稳定身份锚点指认的逻辑参与者。**Agent 是本命形态，human 是扩展形态**（§6）——同一抽象，不同接入。锚点只回答“跨 Run/session 如何确认仍是同一逻辑参与者”；模型、供应商、会话及 profile 都可变化。profile 三维：capability（擅长什么）、relation（与谁同源，独立验证的回避依据）、authority class（可持有哪些权限范围），它们是挂在锚点上的可演化属性，不是锚点本身。
