...
**TeamAct 是去中心化一族的协调范式**，中心化仅作对照边界。它的核心论断是一个**有条件的推导，不是公理**：当副作用排他且责任需审计（不能安全地重复执行）、工作跨越执行者生命期（必须换手）、且没有常设中心（换手只能对等完成）时，协作的枢纽事务必然是 **Handoff——职权迁移（authority line）与上下文交接（context line）的耦合闭环**（推导见 §1.2，与替代"核心"的批判性对照见 §1.4，不适用域见 §8）。本文给出支撑这一事务的最小本体（两个实体、两个版本化关系——责任指派与 per-scope 职权授予）、两阶段交接事务、最小协作循环（三步）与六条可检验不变量；push/pull、执行心跳、检查点、fencing、消息确认等此前的机制成果全部归位为闭环的**实现与可靠性策略**，不再占据内核。
...
**从假设到核心（推导，非断言）**：A5 + A4 ⟹ 任一时刻"谁可以行动"必须有排他且可追溯的答案——这就是职权；A3 ⟹ 工作必然跨越单个执行者的生命期——换手不可避免，且换手必须携带"足以继续的上下文"；A1 + 无常设中心 ⟹ 以上两件事只能在对等参与者之间、以可审计的方式完成。三者合取的枢纽事务就是 Handoff。**这是从任务结构推出的必然，不依赖"我们恰好以交接方式协作"**；任一假设不成立时结论失效，替代范式更优（§1.4、§8）。
...
**证据地位的诚实声明**：F1–F5 采集自我们自己以交接为形态的实践。它们是"该形态下闭环破损真实发生且有代价"的**存在性证据**，不是"交接是普适核心"的证明——一个按黑板形态运转的团队会产出黑板形状的失效目录。普适性主张只由 §1.2 的条件推导承担；条件不成立的域，本文明确不适用（§1.4、§8）。

### 1.4 批判性对照：替代"核心"候选与本文的位置

"去中心化协作的核心是什么"在文献与实践中有多个成体系的答案。本文的论断必须与它们对照后才有资格成立：

| 候选核心 | 主张 | 与本文的关系 |
|---|---|---|
| **任务分配**（Contract Net、市场/拍卖） | 核心是"谁最初接活"（announce–bid–award） | 被吸收为 Establish/Bind——offer→accept 就是合同网形态。分配理论对**中途换手**（部分完成状态 + 职权 fence + 上下文连续）基本沉默，那正是本文的对象 |
| **通信协议/言语行为**（ACL 一族） | 核心是标准化消息语义 | 本文显式降层：消息是传输；"消息到达 ≠ 职权改变"（§3.1 纪律 1） |
| **共同意图/联合承诺**（joint intentions、SharedPlans、commitments & conventions） | 核心是共享承诺结构 | 最强的经典竞争者，也是本文的直系血统：Responsibility ≈ 版本化、可迁移、可审计的 commitment；不变量 ≈ conventions。经典理论对**执行者退出后承诺如何易主**着墨极少——A3 现实中这是主应力 |
| **黑板/环境自组织**（blackboard、stigmergy、swarm） | 核心是经共享环境自组织，**无排他职责** | **真正否定本文前提的一族**：A5 不成立（工作近幂等、重复无代价、验证便宜）时它确实更优、根本不需要 Handoff——本文不适用；A5+A4 成立时，自组织产生重复/冲突副作用且无法归责 |
| **共识/分布式一致**（Paxos/Raft、CRDT、2PC、fencing token） | 核心是对状态达成一致 | 层次不同：它是本文账本/CAS/fence 的**基座**。两阶段事务机械显式承袭 2PC + fencing；本文新增的是被保证一致的**对象**——责任、职权与含在途副作用的上下文，且同时跨 agent 与 human |
| **组织结构/角色/规范** | 核心是稳定的角色结构 | 角色 = 常设 Grant 模板 + 默认指派策略；组织答"平时如何"，Handoff 答"运行时如何安全变更"。大而稳的组织角色主导，小而流动的团队运行时迁移主导 |
| **编排扇出**（orchestrator-workers；及"上下文无法可靠共享，勿建多 agent"一派） | 核心是分解与汇聚，或干脆**回避交接** | 光谱另一端（§1.1）：工作可分解为独立、读多写少的小包时它更优，交接是边缘问题。该派"上下文交接极难"的论断恰从反面确认了本问题的真实性——他们选择回避，本文选择闭环工程化。同一系统可以两态并用：串行责任流用 Handoff，可并行子问题用扇出 |

对照后的定位一句话：**多数候选与本文互补或分层（分配=Bind、共识=基座、角色=常设配置、承诺=血统）；真正互斥的是黑板一族与纯编排扇出——二者各自在 A5 或 A3/去中心化前提失效的域里更优**。本文只在 §1.2 假设合取的域内主张核心地位。
...
在 §1.2 假设域内，去中心化协作的协调复杂性集中在这一个事务上。**Handoff = 职权迁移与上下文交接的耦合闭环**：
...
- **论断的自指风险（显式对冲）**：核心论断诞生于一个已按交接方式运转的团队，失效目录也采自它——存在"用自己的实践证明自己的形态"的循环风险。本文以三件事对冲：条件化推导（§1.2，论断只在 A1–A5 合取时成立）、与替代核心的批判性对照（§1.4）、明确不适用域（本节）。读者若发现推导之外的隐含依赖，即为有效反驳。
- **不适用域**：A5 不成立（工作近幂等、重复执行无代价）→ 用黑板/群体智能一族；工作可全程分解为独立小包扇出汇聚 → 用编排模式；任务短于单个执行者生命期且无需审计 → 纯任务分配即可，无需交接闭环。
...
6. **文献锚点精确化**：§1.4 的对照基于 canonical 常识记忆做粗粒度定位（Contract Net、joint intentions/SharedPlans、commitments & conventions、blackboard/stigmergy、2PC/fencing、近期 LLM-MAS 编排与"勿建多 agent"讨论），未逐一 source-audit；对外发表前须补精确引文、年份与逐条核对。
