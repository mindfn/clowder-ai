...
| 三条真实故事（重构核心） | 别人真的怎么用 | 并行案例（冷读 spawn 侧写）/ 独立质疑（冷读问答压短）/ 跨天责任（候选 a） | 每故事五问结构 | 并行=案例框或小 fan-out 图；责任=时间线泳道（通往第二篇的桥）；冷读泳道保留 | 不做协议细节 |
| 从故事抽四个拆分面 | 差异模型成形 | 回指三故事 | 四拆分面 | 拆分面图（重做，替代旧四层图）+ 表（只留负载/责任落点/成本/不解决什么，与图零重复） | 不重复故事叙述 |
...
| 前任已死怎么办 | 无签字时材料从哪来 | 候选 a 事件（账本回放打捞侧写） | 心跳/悬置/检查点/悲观对账 | **崩溃恢复图（与交接图并排）**，标哪些是替代物、哪些保证降级 | 不重复两阶段定义 |
...
| 动手前七问 | 带走可操作检查 | 与前文小节一一对应 | 无新概念 | checklist | 不冒新概念 |
...
| 一 · 责任时间线 | 跨天换手断点 | 候选 a 叙述的时序部分 | 候选 a 坐标 | 新绘 |
...
1. ~~另两条故事坐标~~ → 已答：并行 = 冷读 spawn 侧写（坐标已核）；跨天责任 = 候选 a（坐标已核，本产线自身事件链）；候选 b 待复核作备选。
2. ~~SVG 复用评估~~ → 已答：复用 3（时间地图/冷读泳道/推导矩阵重定位），重做 1（四层图→拆分面图），新绘 7（上限内取舍）。
3. **发布渠道与目标宽度**（归 lang）：公众号正文 ≈677px、手机 ≈380-420 CSS px——若确定走公众号，图字号验收档从 440 收紧至 ~380，表格列数需限 4 列内。渠道定了我按实际宽度定验收。
4. **可折叠块**（归 lang）：公众号不支持折叠语法；若网页发布可用"实现细节/证据"折叠块分层。渠道决定信息分层方式。

## 验收链

brief v3.1 对齐（sol + lang，含 Open Questions 3/4 拍板）→ 候选 b 坐标复核 → 第二篇重构（六小节 + 图上限 5）→ 第一篇重构（三故事）→ 段落动作 pass（7 病表 + 每节三遍检查）→ sol 全量 review（含逐节契约与视觉账本逐项核）→ 单篇冷读重跑 + 双篇合读 → lang 终审。
