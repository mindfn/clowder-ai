...
- **strict effect admission 未实现**：④′ 的准入不变量与能力出口 mediation（§8 信任根 9）是目标设计——我们当前的 CLI 运行时仍允许自由 shell 携带 ambient credentials 与网络出口，按 ④′ 降级条款属 best-effort 观测档；请勿把严格准入读作已上线能力，"自由 shell + ambient authority"正是它要改造的现状而非它的反例；
