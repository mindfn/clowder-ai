...
<!-- FIGURE A（codex 绘制）：两层定位图。上层：执行编排（orchestrator / graph / crew / handoff——
     现有框架的领地，标注"任务怎么被执行"）；下层：责任协调（WorkUnit / Claim / 协调账本——本文，
     标注"团队怎么对工作负责"）；人在两层都出现但角色不同：上层是 HITL 暂停点，下层是有 SLA 的一等执行者。
...
**⑦ push 与 pull 是策略，责任结构是不变量。** 消息与工作的分发有两种模式：**push（定向传球）**——上一棒交接时指名下一棒，适合专长匹配明确、上下文连续性重要的工作；**pull（工作池）**——offer 广播进池，空闲执行者评估后竞争认领（CAS 保证唯一赢家），适合可并行、执行者可互换的工作。两种模式可以在同一团队共存，因为它们共享同一套认领 / 义务 / 探测语义——分发模式怎么选是策略，"认领排他、义务明确、掉球可探测"是不变量。
...
**⑨ 失忆是常态，记忆分四层管理。** 上下文窗口会压缩、会话会重启、执行者会更换——记忆设计的目标不是避免失忆，而是**让失忆不致命**。四层：**工作记忆**（会话内推理状态，易失）、**团队知识**（跨 agent 共享可检索——写入是 Commit 的副产品而非额外义务，检索是接手工作的标准步骤而非可选优化）、**agent 私有记忆**（身份/关系/偏好，不共享但影响行为）、**责任记忆**（协调账本——注意"我们决定了什么"是知识，"谁负责做的这个决定"是责任，两者分开存、以稳定 ID 互相引用）。会话更替后的恢复走**三源重建**：交接契约给意图、账本回放给责任状态、知识检索与历史回读给细节——三源缺一不可。铁律：**任何决定协作状态的信息不得只存在于工作记忆**，"我记得我答应过"在下一个会话里不存在。

<!-- FIGURE B（codex 绘制）：协作回合 + 上下文双通道。中：八步回合环（Wake/Discover→Inspect→Claim→
     Orient→Execute→Verify→Commit→Transition）；交接处两条通道：推送（交接契约四要素，窄而结构化）
     与拉取（回读历史+检索知识，宽而按需）；旁注记忆四层小图例。可整合 paradigm 图 2/图 4 为一张对外简化版。 -->
> *图 B：协作回合与上下文双通道（待插图）*
