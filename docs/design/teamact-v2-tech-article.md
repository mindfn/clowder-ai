...
下文严格区分故障状态与合法迁移：**职责悬置**指义务在 SLA 内没有被认领、处理或升级；**执行失联**指 active Attempt 失去心跳与租约；**职责失去有效承接**指既没有有效 Claim，也没有受监督的后续处理路径。与这些故障不同，**职责转移**是同一 WorkUnit 经授权更换承担者，**顺序移交**则是当前 WorkUnit 完成后创建后继单元。
