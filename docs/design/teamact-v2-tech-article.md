...
**⑨ 失忆是常态，恢复是多源重建。** 与 ⑧ 同一个推导的另一半：⑧ 的三类持久语义齐了，新会话或新 holder 的恢复就不需要读一份"大摘要"，而是从**交接快照/最后检查点 + 账本回放 + 知识检索/历史回读**多源重建：前者给意图与未完成进度，账本给责任真相，知识与历史给细节（④′ 的崩溃路径正是这条链的退化形态——快照退化为检查点，清单退化为准入记录的推导）。我们自己的实现把记忆分四层（工作/团队/私有/责任）、靠稳定 Actor 锚点跨 Run 关联——那是这个推导的一种实现，不是范式内容。

![图 D：四层记忆与恢复路径](./assets/teamact/figure-d-memory-recovery.svg)

*图 D：我们当前实现的四层记忆划分——⑧ 三类持久语义的**一种非规范实现示例**，不是范式内容；范式要求的是那三类语义 durable 可获得，不是这个分层本身。*
