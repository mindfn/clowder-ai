...
8. **协议保证 provenance，不保证 semantic truth。** 账本能证明"谁依据什么声明了完成 / 就绪"，不能证明声明在语义上为真；保证强度取决于所选 completion / readiness policy 与验证故障域（独立验证者若与产出者共享同一个坏适配层，独立性名存实亡）。高保证工作需要跨故障域验证、认证回执或人类审批。
