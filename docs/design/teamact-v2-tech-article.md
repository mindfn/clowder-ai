...
现代编排框架在 durable execution 上已经走了很远（LangGraph 的 checkpointing 与跨 session 状态、OpenAI Agents SDK 的可序列化 RunState、CrewAI Flows 的持久化恢复——见 §5）。但它们持久化的对象是**执行状态**（图走到哪个节点、run 等待什么输入）。我们的生产 incident 反复指向另一类状态的缺失：**责任状态**——这个工作单元被谁认领、义务算谁的、认领者失联了谁来发现、人类成员的承诺怎么被跟踪。执行状态回答"程序怎么继续跑"，责任状态回答"团队怎么不掉球"。

## 2. 一次实测暴露的结构缺陷

某天我们做了一次消息可见性实测，加上既往的诊断，得到五个 incident：

| # | 发生了什么 | 结构根因 |
|---|-----------|---------|
| 1 | 一条没有指名的消息（"1+1=?"）按"最近回复者"规则**正确**路由给了 A 猫；B 猫随后因另一条显式点名被唤醒——但系统把 A 猫名下的这条消息注入了 B 猫的执行上下文，还用"thread 里有未读消息"的门禁拦住了 B 猫的行动 | 路由没错——**上下文注入范围**和**处理义务**都按 thread 粒度计算，不区分"是谁的消息、在谁的执行范围里" |
| 2 | UI 消息序混乱：并行回复按壁钟到达序交错渲染，上下文断裂 | 审计时间线、会话因果树、执行泳道三种视图混在一根时间线里 |
| 3 | Session 断片后协作状态不可靠（重影、冗余触发） | 会话续接机制存在，但续接与"哪个工作、谁的认领、第几次尝试"没有关联 |
| 4 | 升级给 operator 的事项无限期搁置——诊断结论令人印象深刻：**"整个系统里唯一没有掉球保护的 agent，是人"** | 人不在协调模型内：人的待办无认领状态、无 SLA、无探测 |
| 5 | 讨论期间一只猫因供应商 API 中断**两次静默掉球**，靠人工发现并手动唤醒 | 失败的唤醒没有尝试记录与心跳，静默死亡无信号可探测 |

五个 incident 指向同一件事：**我们有消息系统，也有局部的执行可靠性机制，但没有统一的责任协调模型**。消息（说了什么）、执行（跑到哪了）、责任（谁该做什么）耦合在同一根管道里，粒度各自都不对。
...
| **Claim** | 排他认领（1:1，CAS；带租约心跳和防僵尸的 fencing 代数）；**认领跨 session 存续** |
...
几个最有用的设计判断（每个都对应一个实测 incident）：

**① 三维分离：唤醒 ≠ 义务 ≠ 可读。** incident 1 的病根是三个正交维度被耦合成一个"可见性"。唤醒（谁被这条消息叫起来）和义务（谁背处理责任、注入谁的上下文）必须 per-recipient 收窄；但**投递状态不决定阅读权限**——可读性由独立的 ACL 管，协作域默认可读。隔离的是注意力和义务，不是信息：事后接手他人工作、审计复盘，都依赖"义务之外仍可读"。

**② 人是一等执行者。** "等人批准"不是把球扔进虚空，而是一个执行者为 human 的 WorkUnit——因此它有认领前/认领后两层 SLA、超时提醒与升级路径，被掉球探测统一覆盖（修复 incident 4：不把人建模进协调层，就没有地方挂人的超时策略）。唯一的硬边界：审批超时只能提醒/升级/搁置/取消，**系统永远不能替人批准**。

**③ 静默死亡靠"生命迹象"探测，不靠终态。** 只记录"任务失败了"永远探测不到"任务没了动静"。attempt 有 started 事件和心跳；心跳断供 + 租约过期 + SLA 超时 = 判定掉球 → 探测、唤醒、升级或转移（修复 incident 5）。
...
**具体收益**（每条对应 §2 的一个实测 incident）：掉球可探测且**人与 agent 统一覆盖**；球权有 chain of custody 可复盘；中断可恢复且易主安全（attempt 链 + fencing）；并发安全（claim CAS 消灭"都以为对方在做"）；注意力隔离而不牺牲协作感知；治理约束（跨家族 review、决策边界）从"团队约定"变成可校验的协议。
...
1. **协议遵守不是硬约束。** LLM agent 靠提示词约定 + 运行时门禁兜底，仍会漏——本文讨论期间就实测到一只猫因 API 中断静默掉球、靠人工唤醒。账本让掉球**可见**，不让掉球**不可能**。
