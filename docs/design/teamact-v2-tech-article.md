...
- **逻辑 Actor 是长命的**：运行它的 session、模型乃至供应商可以更换，但稳定的 Actor 锚点不变；记忆、能力画像和责任记录都通过这个锚点连续关联。锚点不是花名——它是**由身份权威管理的注册 principal**：ID 不可复用、注册与重绑不能由 Actor 自己声明、身份由运行时从受信认证绑定注入而非自报（物理构成见 §3，担保边界见 §8 信任根）。异构模型混编（Claude / GPT / Gemini / Kimi 同队），围绕一个长期软件产品持续协作与迭代。
- **人是团队成员，不是调用者**：operator 拍板愿景、审批不可逆操作、也**承接工作**（配环境、做决策）——因此人工职责同样可能逾期或失去跟进。
- **工作跨 session**：一个功能以天计，横跨多次进程重启、上下文压缩，乃至模型供应商的额度与服务中断。
- **结果要审计**：谁在什么时候对什么负责，事后必须可追溯（跨模型家族 code review 是我们的铁律——"家族"指行动时刻实际绑定的模型家族，不是花名册上的角色身份，两者可以不同）。
...
| 概念 | 一句话 |
|------|--------|
| **Actor** | 由注册 principal 指认的逻辑参与者——principal 经身份权威的 enrollment 事务建立（ID 不复用、凭据可轮换、注册非自报），agent 是本命形态，**人是扩展形态**；Run/session/模型可更换，责任与审计仍通过该 principal 连续关联。身份只回答"你是谁"：有无推进义务由 ResponsibilityAssignment 回答，能否决策/提交副作用由 AuthorityGrant 回答——**Actor ID 不是 capability** |
...
| 概念 | 一句话 |
|------|--------|
| **Run** | Assignment 下的一次实际执行（一次 session）；由可信 runtime 固化一份不可变的**执行绑定快照**（provider / 模型 / 版本等独立性维度——不由 agent 自报）；中断 = 记终态，**Assignment 不动**，恢复 = 同一 Assignment 下新 Run。**`Run.completed` 只是执行观测，不自动产生 `resolve(complete)`**——完成要走 ⑤ 的门槛 |
...
**④ 职责转移 = 授权 → 就绪 → 原子生效，三步不并作一步。** 先说清适用面：这是**计划性易主**的主路径——前任在场、能配合签字和封存现场。执行者猝死走的是它的恢复变体，④′ 单独讲。同一件工作从 A 转到 B，走两阶段事务。第一步集齐授权：责任易主由当前承担者签字，每个随迁的职权由它的持有者**分别**签字——执行者不能替人转走审批权。第二步 prepare：冻结随迁职权、定版上下文快照、封存一份**在途副作用清单**——A 已经发出去、还没返回的外部调用。B 必须知道这份清单，否则会把同样的事再做一遍。B 确认过快照与清单，commit 才原子生效：职权迁移，旧凭据作废。说准这里保证了什么：**职权只在 B 满足声明的就绪门槛并留下绑定证据后生效——协议消除的是"无就绪证据便接权"的窗口**；它不能保证 B 永远不会误判自己的理解（确认记录是 provenance，不是理解完备的证明）。就绪门槛与 ⑤ 的完成门槛对称、按风险分级：低风险显式 ack 快照与清单即可（接受"自信却没理解"的残余风险）；高风险可要求结构校验（必需引用可访问、版本与清单逐项核对）乃至外化的就绪证据（复述在途副作用、回答从快照可导出的检查题）。中途失败就 abort，一切原样恢复。
...
4. **在途清单从哪来：不是从 A 的记忆里抢救，而是从账本水位上算出来。** 这里有一条 ④ 没有明说、崩溃路径却完全依赖的前提：**高风险副作用在真正离开受控边界之前，必须已经有 durable 的准入记录**（effect ID、幂等键、请求摘要、准入状态）。有了它，A 猝死后的在途清单就是一个查询：**已准入 ∧ 尚未观测到可信终态**的 effect 集合。终态证据也不能只依赖可能死掉的 Run——回执要有可验证的来源；durable outbox、供应商签名回调、独立轮询器、外部系统原生的事务语义，都是合法实现，**要求的是行为性质，不是某种固定拓扑**。降级条款覆盖**任一 effect surface**——既包括给不出可信回执的外部系统，也包括 agent 侧自身的执行通道（允许自由 shell 携带任意网络出口与 ambient credentials 的运行时，准入拦截天然不完备）：只要准入覆盖或可信观测无法保证，就诚实降级——只承诺检测与事后对账，不声称在途清单完备或严格防重，恢复交接必须携带"不完备 effect coverage"的风险标记，默认悲观对账。
...
6. **继任者确认后才获权。** B 收到的是一份退化的交接：恢复政策的授权替代 A 的签字，最后一个 durable 检查点替代合作式快照，从准入记录推导的清单替代 A 亲手封存的清单，外加一份悲观对账义务。B 必须**显式确认**这份重建结果，commit 才原子迁移责任与职权；B 无法满足声明的就绪门槛，工作就**继续悬置**——宁可停着，不接受"先抢权再慢慢猜"。
...
- **接手者动手之前必须知道什么？** 四要素：**事实**（做了什么 + 产出坐标）、**意图**（为什么 + 放弃的权衡）、**边界**（开放问题 + 风险）、**行动**（期望下一棒做什么）。这是交接快照的最小完备集——外加一份**在途副作用清单**（④/④′）：不知道 A 发出去了什么，B 就会把同样的事再做一遍。链条的收口是**接收者按声明门槛留下就绪证据**：快照送达不算，B 按风险档位满足从显式 ack 到证据化检查的门槛，才算就绪（④ 把它做成 commit 的前置条件）。
...
把责任状态持久化只是第一步。我们的运行时仍是事件驱动的：消息到达才唤醒参与者，回合结束后执行实例就休眠；若唯一推进者停止，就不能仍依赖它完成责任状态迁移——独立巡检必须依据心跳、lease 与 SLA 推动既定 policy。因此系统还需要一个常驻 **reconciler**，持续对账账本上的 Assignment/SLA/处置时限与实际心跳、进度及处置记录，在时限内触发催办、唤醒、升级或授权提案。
...
reconciler 的两条边界必须说破。**其一，它查的是义务与证据是否到位，不判断证据内容真假**：它能发现验证逾期、证据缺失、收口悬空，发现不了一个已满足低强度完成策略、但内容为假的完成声明——执行终态造假属于执行层完整性故障，不在 F1–F5 的失联分类里，防它的机制在 ⑤ 的完成门槛。**其二，它自己是这套机制的 liveness 根**：巡检水位与处置投递必须持久化（重启不漏扫、重复安全），它自身的存活要暴露给外部健康检查——**守护者的失效必须比被守护者的失效更容易被发现**。因此"时限内推进"的承诺有前提：监测器存活、调度公平。reconciler 停摆期间，安全性质（fence、版本、唯一记录）不受影响，逾期事实仍持久在账本上，但探测与处置暂停——真实的处置延迟上界 = 外部发现并恢复 reconciler 的时长 + 补扫延迟 + policy 处置时长（§8 把这一点作为独立的信任根披露）。
...
**两层按 §3-⑥ 的递归边界组合**：orchestrator-worker、evaluator-optimizer 等 patterns 可以完整活在一个 WorkUnit 的执行内部（临时 helper），也可以升格为 child WorkUnits（独立责任）。Anthropic 的 when-not-to 判据我们完全采纳：单任务能单 agent 做就不要多 agent。他们记录的失败模式与我们机制的对应也能交叉验证：telephone game ↔ 结构化 handoff 契约 + **原始证据可回读**（接手者主动回看原始上下文的访问权不因路由收窄，不依赖转述——① 三分法里的"可读"）；early victory ↔ 独立验证 WorkUnit + 否定约束 + 不可变坐标绑定；context pollution ↔ **默认注入的工作上下文按 recipient 隔离**（① 三分法里的"义务注入"侧。收窄说明：Anthropic 该失效讲的是上下文内容质量，只能为注入隔离背书；"义务归属错误"是 TeamAct 额外提出的责任语义，由 F1 实例与账本原则自行论证，不借它站台）。
...
还有一条**约束之外的诚实边界：即使五条约束全部成立，耦合式交接也不是唯一解**。解耦式 fence-and-reclaim（§3 诚实声明）在同样约束下完全合法。我们选耦合式的具体判据：**计划性易主为主**（前任在场，可合作封存在途副作用、移交未外化意图）、**审计要求责任连续无空窗**、**继任者含人类**（人需要推送式策展上下文与显式接受，难以"回放重建"）。反向负载（崩溃主导、共享状态即是全部上下文、承接竞争便宜）应选解耦式。这个选择判据本身**可以被推翻**，反驳类事先固定。前提三个"同"：同一工作负载、同一环境假设、同一结果门槛（副作用不重复、责任可归属、接手者能安全继续、失效有界处置——探测与处置的时限阈值也要一样）。代价按**封闭的四个维度**比较：上下文不可恢复率、恢复延迟、协调开销、误判处置率——维度集合就此声明封闭，不允许事后追加维度化解反例。反驳分两层：**偏好无关的硬反驳**——在判据说"该选耦合式"的负载上，解耦方案在四维上不劣于预注册的非劣界限、且至少一维以超过最小效应阈值的幅度严格更好（带统计容差的 Pareto 支配，不要求测量值恰好相等）；**决策级反驳**——结果是混合向量，但按该负载在**看到结果之前**登记的决策规则（加权、词典序或约束优化，三选一事先声明）以超阈值幅度胜出，同样推翻判据。预注册纪律闭合在登记时刻：预测选耦合式或解耦式的评测**必须**同时登记决策规则——没有登记决策规则的评测，只能在**开始之前**就把预测记为 abstain（或显式标为"只测机制、不检验选择判据"），不存在"看到混合结果后再降级为弃权"的路径。弃权记为选择判据弃权——**不得继续宣称"该负载应选耦合式"**。覆盖率同样事前可判定：评测套件预注册目标负载集合与最大弃权率；套件跑完弃权率超限，即便从未出现 Pareto 碾压，也构成另一类失败——覆盖率不足，不能承担工程选择器的职责。裁判用的标准是结果性质，不是本文自己的机制。

## 7. 有什么用 & 适用判据

**具体收益**（每条回应 §2 的一类失效）：职责悬置与执行失联可探测且**人与 agent 统一覆盖**；职责归属链全程可复盘；中断可恢复且职责转移安全（Run 链 + 两阶段事务 + fencing）；并发安全（账本 CAS 消灭"都以为对方在做"）；注意力隔离而不牺牲协作感知；治理约束（跨模型家族 review、决策边界）**可以**从"团队约定"变成可校验的协议——这是目标能力：兑现它需要 ⑤ 的独立性 policy 加上 Run 的执行绑定快照（按行动时刻的实际绑定判独立，而非花名册身份；我们当前系统仍是花名册级调度门禁，尚非账本级模型独立性证明）。
...
| 有，且需要知道“谁该做、做到哪” | 中高 | 至少引入 WorkUnit + offer/accept 责任指派与超时探测（此档完成多为自证——承担残余的假完成风险） |
...
4. **范式收敛自我们自身的实践，存在自指风险。** §2 的失效目录采自一个已按交接方式运转的团队——它证明该形态下协调破损真实且有代价，**不证明**交接是普适核心。对冲方式：问题域显式声明（§3 诚实声明）、承认解耦式替代并给出选择判据（§6）、反驳标准使用模型中立的结果性质而非本文自己的机制——分偏好无关硬反驳与决策级反驳两层，并带**选择器覆盖率**约束：判据无法事前给出可检验预测的负载必须弃权，弃权不得事后解释为判据成功（§6 末段）。
5. **恢复治理有信任根。** 失联恢复与悬置的授权者集合是协议的信任根：合法授权者恶意停工无法由协议消除，只能由 quorum / 职责分离缓解；协议保证的是滥用全程落账、可见可追溯。
6. **失效探测有自己的信任根。** reconciler 是逻辑 liveness 根（§3.5）：它停摆期间安全性质不受影响、逾期事实仍持久在账，但探测与处置暂停；"谁盯守护者"最终收口在外部健康检查与平台监控——一个不在本体内的依赖。多副本能消除单进程故障，消除不了这个逻辑依赖。
7. **substrate provenance 有自己的信任根。** 执行绑定快照由可信 runtime 固化（agent 自报无效）——独立性 policy 的一切校验以 runtime 诚实为前提；runtime 被攻破时，账本无法自证。
8. **身份有自己的信任根。** Actor 锚点由**身份权威 + Actor registry** 维护：enrollment 事务建立、ID 不复用、凭据可轮换、注册与重绑不能由 Actor 自报。身份权威被攻破时，攻击者可伪造 enrollment 或重绑 principal，账本无法靠自身证明 Actor 未被冒用——责任、职权与 Run provenance 全部立在这个信任根之上。它与 substrate provenance 相邻但不同：一个担保"这个成员是谁"，一个证明"这次实际由什么在运行"。
9. **effect 准入的完备性有自己的信任根。** 账本只能 fence 经过受控出口的动作；受控边界放在**能力出口**——网络 egress、凭据、工具宿主——而不是命令文本分类上（分类器只能做 telemetry，不能做安全证明）。运行时若允许未受控出口或 ambient credentials，账本无法证明没有旁路副作用：对应执行域必须显式标注 best-effort / partial coverage，并按 ④′ 的降级条款行事。它与身份权威、substrate provenance 各管一段：谁是成员 / 这次谁在跑 / 它能通过哪些出口影响现实——即使物理上由同一 runtime 实现，逻辑上分别披露。
10. **协议保证 provenance，不保证 semantic truth。** 账本能证明"谁依据什么声明了完成 / 就绪"，不能证明声明在语义上为真；保证强度取决于所选 completion / readiness policy 与验证故障域（独立验证者若与产出者共享同一个坏适配层，独立性名存实亡）。高保证工作需要跨故障域验证、认证回执或人类审批。
...
- **统一本体未实现**：本文核心——协调账本与 WorkUnit 本体——代码改造未启动。其中 **strict effect admission** 值得单独强调：④′ 的准入不变量与能力出口 mediation（§8 信任根 9）同属目标设计——我们当前的 CLI 运行时仍允许自由 shell 携带 ambient credentials 与网络出口，按 ④′ 降级条款属 best-effort 观测档；请勿把严格准入读作已上线能力，"自由 shell + ambient authority"正是它要改造的现状而非它的反例；
