...
开头那次静默失联不是孤例，只是五类失效里最戏剧性的一类。下面五个场景都是我们数月生产实践中反复出现的失效模式的抽象（具体案例数据属于内部工程记录，此处只保留结构）。其中几类（并行时序混乱、静默失联）在一次性的 orchestrator + subagents 里同样可能发生——但那里责任边界通常收在单次任务及其编排者之内；agent **长命、跨会话、与人混编**时，同类问题跨任务累积，暴露成责任连续性问题：
