...
外部等待、action successor 与 callback principal 可以从各自上游协议产生一条已经定型的 `private_input` 来源消息。owner 可以只提交自身状态变更而不唤起 Agent；只有 owner 已经决定“需要 exact target 继续处理”并签发 envelope 时，Queue 才出现一条 `private_input`。普通 A2A terminal 不属于这组 producer：它只更新 History，dispatching member 在下一次自然 invocation 通过 exact source×target observation 反查结果。主生命周期不从 payload 反推新的 work，也不复制 owner 状态或等待记录形成第二条 Queue / 平行 lifecycle。
...
sequenceDiagram
    ...
    D->>D: resolve targets that can dispatch now
    D->>H: materialize M1 once on first actual dispatch
    D->>H: append exact dispatchRef(s) + response bubble(s)
    D->>Q: remove only admitted targets; keep pending siblings
    ...
    and independent run C
        ...
        Note over T,Q: ordinary failure creates no predecessor Queue work
        T->>D: release C run and request drain
    end
```

初始 enqueue 把完整 target set 一次写入同一 Queue row，因此 admission intent 是 all-or-none；这不要求所有 targets 同时 start。B 已可引导而 C 仍需等待时，B 可以先离开 pending set，C 保留。cutover 后每个 target 拥有独立 invocation、response bubble 与 terminal，某个 sibling 失败不会回滚另一个已经接受的运行。

兼容的连续 public inputs 可以共用一次 client dispatch，但每条 input 仍有独立 source、Queue Entry、History message 与顺序；batch 绝不把消息身份或正文合并。
...
主链的 Agent Client terminal 只有 `completed / failed / canceled` 三类 outcome；`interrupted` 是 startup recovery 在没有 live client callback 时合成的 failure-like terminal，沿 failed 分支收敛，不是第四条正常 callback 路径。只有 completed final 中新的显式 target 可以追加 message ref；failed、canceled 与 interrupted 都不隐式创建 predecessor work。dispatching member 在后续自然 invocation 从 History 观察当前 source×target terminal；需要自动续跑时由既有 continuation owner 显式创建 work。
...
- target 相同是共用一次 dispatch 的必要条件，但不是“把消息合成一条”的许可。只有队首开始的兼容输入，或已被同一次未读投影精确覆盖的 wake ref，才可一并接管；每条消息与 entry 的身份、正文和顺序仍然独立。
...
Queue 只保存 `private_input` 的 inline payload，不理解这段正文为何产生，也不解析 generation、predecessor 或成员 session 状态。action-successor 的 `actionSuccessorFence`、event-wait 的 `waitContinuationCarrier` 与 registered callback 自己的 carrier 继续由各自 owner store 解释；owner 若需要 terminal disposition，就在 QueueEntry 之外按 exact entry/invocation 建立自己的 binding。普通 A2A failure 不生成 `private_input`；它由 dispatching member 后续反查 canonical History。是否存在 owner binding 是上游协议事实，不是 QueueEntry 字段或 private payload subtype。
...
多目标仍是**同一条** Queue Entry：`targets=[B,C]`。B 被 dispatch 后 entry 原位变为 `targets=[C]`；C 也被 dispatch 后 entry 删除。Queue 不为 B/C 复制 source row，不保存已投递成员数组，也不把 response terminal 写回该 entry。
...
Queue Panel 是 public conversation input 在排队阶段的唯一用户可见位置，聊天面板只展示已经发生至少一次 actual dispatch 的消息。entry 仍只有 pending targets 时，输入既不属于 Chat History，也不进入任何 Agent 普通上下文；第一次 actual target 被原子取走时，admission 复用 `sourceRecordId` materialize 一条公开 History message，并为该 target 创建 response bubble 与 `dispatched` ref。尚未投递的 sibling targets 继续留在**同一条** Queue Entry；后续每次 actual dispatch 只从 `targets[]` 删除对应 target，并向同一 History message 追加该 target 的 ref。`targets[]` 为空时 entry 删除。`private_input` 是同一 durable Queue 中的私有 entry，但不在用户 Queue Panel 或聊天面板展示，只在被投递目标的 exact input 中可见，因此也没有公开头像锚点。
...
入口解析结构化 mention 并把当时有效的 exact `targets` 写入 Queue Entry；这只形成 pending target intent，不在 enqueue 时猜默认成员。唯一的提前绑定例外是作者显式选择「立即发送，引导回复」：targetless entry 可以只读调用队首同一个 fallback resolver，但只有 resolver 返回的成员同时拥有 exact current reply 与静态 guide capability 时，才原子绑定该 target 并尝试引导；没有精确可引导对象、resolver 读取失败或 dispatch fence 变化时仍保留普通排队语义，不改用另一成员。entry 成为队首后先得到 actual target set，再按 `kind` 处理：
...
sequenceDiagram
    ...
    Note over S,H: failed/canceled/interrupted only settle exact History refs; no predecessor wake
```

单 target 与 multi-target 共用这条主链：一条 `@B @C` 消息在 Queue 中始终只有一条 source entry，`targets=[B,C]` 保存尚未投递的成员。初始 enqueue 对完整 target set 原子写入；之后 B/C 可以各自离开 pending set，并分别拥有自己的 run 与 response bubble。某个 target 失败不会回滚已经被 sibling 接受的运行。
...
因此连续三条 `M1/M2/M3 → B` 可以一次拉起 B，但 History 中仍是三条输入，Queue UI 仍可在 admission 前分别重排、删除或 Steer。batch 是一次 client 调用的输入集合，不是新领域对象，也不合并各 source Queue Entry 的身份。
...
- targetless head 通常只等到最后一个 Active Run 删除，同一个终局事件会立即重新触发；显式「引导回复」若精确命中 fallback 的 current reply，则先绑定并 Append，不再作为 targetless head 等待；
...
一条 `@B @C` 在 Queue 中始终是一条 entry，`targets=[B,C]` 只表示尚未实际投递的目标。初始 enqueue 对完整 target set 原子写入，不产生半条 source 或 ghost target。此后 coordinator 可以在一次 cutover 中从同一 entry 取走当下可投递的 B，写入 B 的 History ref、Active Run 与响应气泡，同时把 entry 原位更新为 `targets=[C]`；C 稍后独立投递，最后一个 target 离开时才删除 entry。

all-or-none 只约束**初始 source admission**，不要求 sibling 同时 start，也不存在执行失败后的整批回滚。每个 actual target 独立拥有 invocation、响应气泡与 terminal；某个 target 启动或执行失败不会取消已经被 sibling 接受的运行。
...
私有 entry 走同一个 exact take/CAS 边界，只是结果写 internal diagnostic 而不写 History。需要 terminal disposition 的 source owner 从自己的 invocation binding 找回 typed carrier。普通 A2A target failure 不生成 private input；两条路径仍然只有 Queue membership 与 Chat History 两个业务真相源；`DeliveryFailureResult` 是 History message 的一种，不是新 lifecycle ledger。

### 7.2 Admission 是唯一运行 cutover

`admitExactBatch` 在 per-thread coordinator 内完成：

1. 再次确认 exact entry 列表仍是从当前队首开始的兼容前缀，并以实时 Queue `targets[]` × History `dispatchRefs` 计算仍待投递的 exact targets；
...
5. 一个持久事务原子完成：existing owners CAS 验证 preflight candidates 仍 current，并提交 immutable admission bindings；从每条 source entry 的 `targets[]` 删除本次 actual targets，空数组时删除 entry；每条 conversation input 在第一次 actual dispatch 时复用 `sourceRecordId` 为公开 `messageId` 并分配 `orderKey`，后续 sibling dispatch 复用同一 History message；每条 message wake 验证 `payload.messageId`；随后为每个 actual target 写一条引用完整 `inputEntryIds/inputMessageIds` 的 `processing` ResponseBubble，并直接创建指向该 bubble 的 `dispatched` ref；`private_input` 不写 History message/ref，只返回 exact 私有输入；最后激活 callback principals；
...
callback principal 只有在第 5 步 admission commit 成功时才激活。若 exact-target mutation 或任一 owner binding CAS 失败，整个事务没有 Queue/History/owner 副作用，尚未激活的 principal 可以直接丢弃；它从未授权 client callback，也不算一次 admitted run。正常 Steer 并发不把整批用户操作报成 stale：服务端重读 live `targets[]` 与 History refs，已经投递的 target 直接跳过，只对仍 pending 且合法的选择提交 cutover。structured owner fence 不匹配仍按 typed conflict/fail-closed 处理。任何分支都不能从 carrier/current identity 选择新 generation，也不能把 entry 留成未受约束的可重放工作。
...
response bubble terminal、typed owner dispositions 与 completed final 的显式-target follow-up 必须在同一个持久事务中提交；否则进程可能在几次写入之间退出，留下“用户看到了 completed final，但显式目标没有 wake”的半终局。failed/interrupted/canceled 没有隐式 follow-up，dispatching member 之后从 History 反查结果。这笔 durable transaction 与随后删除 in-memory exact Active Run 共同组成 run closure，但不能颠倒：事务成功后才执行“删除 exact Active Run → requestDrain”。若提前释放 run，owner CAS 失败时会丢掉 outstanding work；若先触发 drain 再释放 run，drain 会看到 busy 后退出且可能再也没有信号。
...
disposition（若有），删除 run，再 requestDrain。普通 A2A caller 不由这条路径自动回唤，而在自己的下一次自然
invocation 反查该 failed response；结果不会因为 client 没有输出而静默消失。
...
普通 A2A terminal 只改写 canonical History response 与 source 上的 exact target ref，不从正文、author、predecessor 或 owner disposition 推导新 Queue work。dispatching member 的进程内 outbound dispatch view 以 `(owner, thread, caller, sourceMessageId, targetId)` 聚合自己提交的 Queue targets、已提交 Steer 增删和 actual-result 变化。item 保存 monotonic revision/presentedRevision，以及 Queue/History 已无法反查的 firstAddedBy、最新 selectionChange 与 selectionChangedBy（initial/steer/queue_withdrawal/unknown）；当前进程首次看到遗留 remove/actual 事件且无法证明最初来源时，firstAddedBy=unknown。这些最小解释事实不参与执行裁决。view 不改写 source `dispatchRefs`，也不持久化为新账本。

这条链按 source 隔离、按 target 结算：`M1→B` 与 `M2→B` 各自拥有 dispatchRef 与 view item，即使 B 通过同一次 provider invocation batch 消费 M1/M2；一个 response 可以引用两条 input source，但不能把两条 source 的 caller 状态合成一个 attempt。反之，`M1→B/C` 只有一条 Queue row，B/C 仍分别拥有自己的 ref/result。dispatchRef 关联已经发生的投递或投递失败事实，不表示 target 预期；仅凭 ref 存在不能断言 provider 已收到消息。Queue mutation 成功后只 touch 相应 view item；actual admission/terminal 不复制结果到 view，而在 caller 下一次自然 invocation 时用 canonical ref/response fingerprint 与上次呈现基线比较，发现变化才分配新 revision 并注入。
...
例如 A→B、C→B 的两条消息都已公开，B 的本轮未读投影同时包含两者时，两条 wake 都可被同一次运行覆盖，B 不重复启动；两条 History message 仍然独立，且 B 终局时两条 source×B ref 都指向同一个 canonical response。A/C 分别在自己的下一次自然 invocation 反查该结果，不生成 predecessor route。窗口外或未被所有 target 覆盖的 wake 继续留在原队列位置，不能凭“可能读过”猜测清理。
...
| 立即发送，引导回复 | coordinator 从 source entry 取出选中的 pending target | 静态 client capability 支持且存在 exact current reply 时追加到该 run；默认设置遇到不支持/无 current reply时保留为 `next_work`，显式 Steer 则禁用该选项 |
...
- targetless entry 的首目标绑定与新增 siblings 必须在同一 `targets[]` mutation 完成，不能留下无目标 ghost entry；
...
4. 调整 public conversation 入口为 Queue-first；独立 Agent `post_message` / completed final 直接复用其 History message并原子写入唯一 message-ref wake；actual dispatch 时才为目标创建 History ref；targetless fallback 只留给 public head；
...
| 根据 predecessor/owner 是否存在改写消息 kind | 把 owner disposition 结果误当成来源消息用途 | entry kind 由生产者给定；普通 A2A terminal 不从 owner disposition 推导新 work |
...
| targetless 消息错误投给忙碌成员 | ingest 时自行猜 fallback，或把任意 active member 当 fallback | entry 默认保持 `targets=[]`；到队首且 thread idle 后才由 head-time resolver 选择最近 completed response target/default。作者显式选择「立即发送，引导回复」时只能提前只读调用同一 resolver，且其结果必须正好有 exact current reply 才绑定并引导；否则仍排队。Web 只读同源投影，不把发言新近性叫作执行活跃 |
...
| A6 | 一条用户消息 `@B @C` | Queue 中始终只有一条 source entry、`targets=[B,C]`；B 首次 actual dispatch 时 materialize 这一条 input并删除 B，C 仍留在同一 entry；C 后续投递时复用同一 History message。两个独立 response bubble/ref/头像各自终局 |
...
| A11 | 用户消息无 target，thread 有 Active Run | 默认 targetless head 等待，后续显式 target entry 不越过；若作者选择「立即发送，引导回复」，仅当同源 fallback resolver 返回的成员正有 exact current reply 时绑定并 Append，否则仍等待 |
...
| A33 | B run 失败，A 之后因新消息自然唤起 | 公开 failed response 是唯一终态；A 的 exact source×B observation 在 prompt 中读到该 response；不生成 predecessor Queue work |
...
| A48 | failed/interrupted bubble 提交后、dispatching member 观察前进程退出 | History terminal 保持；新 process generation 不自动重建旧 observation，只在下一自然 invocation 给出有界 process-start 说明，结果可按需从 History 读取；不生成半截 predecessor work |
...
- targetless input 在其他 Active Run 尚存时绕过同源 resolver 猜成员，或在 resolver 目标没有 exact current reply 时仍提前绑定；
...
- multi-target source 的 target mapping 在同一 source cutover 只落半组、targetless 原行仍未绑定，或一个 target 的 post-cutover failure 错误回滚 sibling；
...
- 普通 A2A terminal 另建 hidden predecessor wake、private result carrier 或递归责任树；
