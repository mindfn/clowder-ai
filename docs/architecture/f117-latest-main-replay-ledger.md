...
| User, connector, plugin, and system public inputs enter the same canonical Queue/History path; only authenticated `from` / source category and producer fences differ. | RFC audit §§5.1–5.2 and L1–L3. | All producers call `InvocationQueue.enqueueExistingMessageDurable`; connector continuation additionally validates the typed wait/action owner before admission. | `connector-invoke-trigger.test.js` proves deterministic rows plus delivered-outcome/obsolete-generation rejection; message-lifecycle ingress tests prove the same author-intent envelope for user input. |
