...
cited_by:
  ...
  - {feature: F117-reminder-retirement, date: 2026-09-17, delta: retire the manual Queue reminder button, endpoint and attempt projection; Queue owns only pending targets/order while Steer owns the explicit delivery-mode choice}
---
...
(`sourceId` plus `sourceRecordId`), ordering, immutable launch fences, per-target author intent,
revision, and a short reversible claim. It does not own response processing, terminal outcomes,
...
decision may emit a user-visible receipt. When a cat-authored A2A response fails, that response is the only
terminal History fact. The dispatching member observes it on a later natural invocation through an exact
source×target History lookup; failure does not synthesize a predecessor Queue row or hidden wake.
...
path, so ordinary failure propagation runs (source dispatchRef settlement and `delivery_failure` for pre-start;
the dispatching member later observes either fact through History); no confirmation dialog exists, and
`force-reset` remains only an internal thread-scoped reconciler. The
...
multi-select, and enable guide from static member capability rather than a local liveness guess. Confirmation
...
- Changing the Queue-to-History dispatch cutover, unread adoption, or source materialization.
...
- Do not render fail-open routing warnings as chat messages, synthesize a predecessor failure carrier, or enqueue a
