...
F292 keeps official-plugin trust policy and published catalog metadata on opposite
sides of the boundary. `clowder-ai-plugins` owns the machine-readable list of
published packages, versions, manifests and digests. Clowder AI owns configured
catalog origins, allowed registries/channels, package identity admission, grants,
provenance validation and quarantine. Catalog refresh is discovery only, never
installation or runtime truth. Generic update/repair is not a terminal public
Manager action; internal integrity recovery stays narrow and typed.
