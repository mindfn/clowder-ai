...
- 复用 F190 Phase C #1/#4 hardening pattern：
  ...
  - capability writes 只支持无登录本地应用的 direct localhost UI（loopback peer + localhost Host/Origin + no proxy forwarding headers）；LAN / reverse proxy / non-local origin 不作为 capability write 支持场景
