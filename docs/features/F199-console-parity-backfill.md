...
- 复用 F190 Phase C #1/#4 hardening pattern：
  ...
  - capability writes 在无登录本地应用中允许 direct localhost UI（loopback peer + localhost Host/Origin + no proxy forwarding headers）；LAN / reverse proxy / non-local origin 仍要求 `DEFAULT_OWNER_USER_ID`
