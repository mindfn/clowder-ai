...
这一行带四条硬约束，每条对应一堵墙；标注"少了这条会打开哪道缝"，因为一条大约束
红了还要重新解剖才知道哪堵塌了：

- **旧批对状态是 no-op**：一条全部落在 frontier 之下的批，事件被"已见过滤"逐个挡掉，
  因此对状态零写入。这堵墙删掉的是 **read-modify-write 这个模式本身**——旧设计里
  "迟到的旧写覆盖新状态"之所以可能，是因为存在"读快照 → 改 → CAS 写回"，generation
  正是在给这个模式站岗。新链子里状态只能被通过 filter 的事件推进，失败模式从**覆盖**
  变成**丢弃**，方向是安全的（frontier 只退不进，旧批想退也退不了）。
  *少了它*：旧批重新获得写状态的能力，generation 站的那班岗又需要有人站。
- **同一 subject 同一 surface 的批次 FIFO**：粒度是 **subject × surface**，不是只到
  subject。跨 surface 乱序无害（source 不相交，旧批全被 filter）；危险的是**同一 surface
  两次 poll 重叠**（慢轮询未结束、下一 tick 准时启动）把一个回合的 open 与 close 拆进
  两个乱序批。`botTurns` 的 open（评论 @bot）和 close（review 提交）同住 review-feedback
  这一个采集面，所以这道缝是真的。
  *少了它*：close 先于 open 被消费 → close 找不到回合被丢 → **回合永不关闭**，
  正是"盖假章"换个地方借尸还魂。
- **seq 由入队点盖，不由抓取器自报**：并发抓取器同刻产批时，"观察顺序"没有上帝视角。
  §3.2 那句"写入必须单点"是承重墙，落地成 per-subject 单写者。
  *少了它*：FIFO 依据的是抓取器自报的时间戳，而那不是关于任何东西的事实。
- **一批是一个原子单元，且零通过批零写入**：批内事件不得被拆散或与别批交错——回合重放
  是**批内**性质，同秒 close 必须先于 open，拆散批等于删掉这条规则。同时，回合的开闭
  只由**通过 filter 的事件**驱动：一条零通过的旧批，不得触发回合评估或超时退休。
  *少了后半句*：旧批拿到一个"不写 frontier 也能动回合状态"的侧门。
- **注册不入队**：- **注册不入队**：注册是同步契约（§4 A45 的原子拒绝、409/422/503 即期返回、响应带
