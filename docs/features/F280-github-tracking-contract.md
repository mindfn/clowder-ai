...
**最后一行是这次改造的全部。** 现在的实现在这里做的不是"推进"而是**重建**：把作者身份、
每条游标、开着的 bot 回合重新搬进新的一代。搬漏一个就静默丢一类通知——§3.1b 记的那些疤痕
全部出自这个动作。改成原地推进之后，"手工三方合并"这件事本身就不存在了，**不需要任何机制
替代它**。自动续期这个能力仍然满足：轮询不停就是续期，它不该有专门机制。

**"每个来源一条游标"不等于所有面都比大小。** 评论 ID 单调可比；CI / HEAD / conflict /
base 是**状态转移**（比指纹、比同 HEAD 变化）；review dismissal 会**原地修改同一个 ID**。
代码本来就区分这三类。统一的是**处理入口**，不是强迫所有事实都满足 `id > frontier`。

**这条链子之外不需要新机制，尤其不需要一层队列。** 并发抓取不是缺陷；而 FIFO 也给不了
正确性——请求的发出顺序、完成顺序、GitHub 的事实顺序是三件不同的事，串行消费替代不了
新旧判断与 CAS。已有的竞争保护（注册路径的 revision 围栏、写入的 generation CAS）留在
原地即可。看到"维护旧等待模型"的代价之后，下一个坑是把它换成"维护新队列模型"。

**待投递记录不属于续期机制，不跟它一起删。** TaskStore、event log、MessageStore 是三个
写入点：消息写了而前沿未推 → 重放；先推前沿 → 漏发。持久 outbox、稳定通知 ID、原通知的
路由与 metadata 承担的是多存储原子性，是必要职责，与"重建状态"无关。

**每个保留下来的机制都要能回答这四问之一**，答不上来的就是多余的：
