...
4. caller 下一次自然 invocation 从 view 反查 Queue/History：仍 pending、已实际投递、已终局、投递前失败或由 Steer 移除分别诚实呈现；不靠 target 回复语义或隐藏 wake。
...
`M1×B`、`M2×B` 两个 item，两条 source 也各自拥有一条 dispatchRef。B 可以把 M1/M2 batch 到一次 provider
invocation；该 response 可以同时引用两条 input，但不能把两条 source 或两条 caller observation 合并。反过来，
...
3. **actual admission**：先建立持久 processing response receiver，再给每条实际输入 source 写 exact target
   dispatchRef，最后从 Queue 删除该 target。batch 中每条 source 分别写 ref；view 只 touch 对应 source×target；
...
6. **无隐式续跑**：target terminal 不为 caller 新建 Queue row，也不发送隐藏 result wake。caller 只有因用户消息、
   普通成员消息或显式 continuation owner 自然启动时才收到这份变化投影。
...
- **成员唤起的 dispatch 失败**：failed response 原位终局，source 消息上该成员头像转静止（失败终态）。
  不为 source 成员创建隐式 predecessor Queue row、隐藏 wake 或第二条失败通知；dispatching member 在下一次
  自然 invocation 通过 Phase H 的 exact source×target observation 读取同一 failed response。它随后重分发给
  其他成员、放弃或上报用户，都是新的显式普通消息。
...
| E5 | 失败正文与系统提示 / 恢复 continuation 重复成两个气泡；dispatching member 后续不知道 target 终态 | 实现 | 失败细节合并进唯一 terminal failed response；成功的 `runtime_replacement` 不再续排 source-less continuation；Phase H 在 dispatching member 下一次自然 invocation 反查 exact source×target History 终态，不另造 failure carrier |
...
普通 A2A dispatch 只建立一条 source→target delivery lifecycle，不隐式承诺「target 终局后自动再次运行 caller」。
terminal observation 不依赖 target 回复正文的 `@caller`、完成声明或传球措辞；合法的显式 `@caller` 仍按普通
dispatch 路由。新的 Queue work 只能来自用户/成员的显式消息、已有 Queue work，或 `hold_ball` / eventWait /
action-successor 等显式 continuation owner。
...
7. 普通 terminal（completed/failed/canceled/interrupted）只刷新 History lifecycle 与 caller 的下次观察结果，
   不创建隐藏 `a2a_result` wake、第二条 system message或隐式 predecessor Queue row。若 target 主动 `@caller`，
   那是一条新的普通 dispatch；若 caller 要求结果到达后自动续跑，必须显式注册 continuation；
...
- [x] AC-E5: 失败 response 只呈现一次；正文逐字采用实际 client/provider error message，同一 logical dispatch 的多次错误按发生顺序合入同一气泡，不由 lifecycle 层追加成员/source/error-code/建议；另一只猫的 cursor 读能读到同一 failed response 并可把它作为下游 source，无需用户转达；live 与 hydration 不生成第二条 system/provider error；成功的 `runtime_replacement` 不生成 source-less continuation；普通失败不创建 predecessor Queue row，dispatching member 的后续观察由 Phase H exact History 反查提供
- [x] AC-E6: QueuePanel 横幅、浮窗轨迹按钮移除；轨迹 chip 位置符合验收描述
- [x] AC-E7: 对已确认死亡的 exact execution，Stop 返回 200 `reconciled` 而非 409；进程快照不完整时服务端有界重试后按 failed（reason `control_plane_unavailable`）终局并返回 200，失败沿 Phase C/Phase H 收敛（源 dispatchRef settle、dispatching member 后续 History observation、pre-start 走 `delivery_failure`）；不做平台兼容分支，Windows 子进程不可观测时同样走 fail 收敛；确认无 owner 的 read-repair 使用 `execution_owner_lost`，pre-start processing 超时使用 `prestart_timeout`；单个 child 失败不终局仍有 tracker/process-owner 见证的 sibling parent；`ForceResetDialog` 退役，`ThreadExecutionBar` 无常驻/卡死触发的强制重置入口、无「运行状态待确认」横幅；投影 read-repair 落地，pre-start 预留 TTL 收窄到 create→startAll 窗口
...
- [ ] AC-H6: active-runs 共享快照与 caller 私有 dispatch view 职责分离；自动 continuation 只来自显式 owner，普通 terminal 不创建第二条 status message或 predecessor Queue row
...
- **Phase H in scope**: caller 自有 outbound dispatch 的进程内 revision view、预期 target 与 Steer 增删更新、下一次自然 invocation 对 Queue/History 的反查/注入/compare-and-clear，以及有界 process-start 说明；不改变 source `dispatchRefs` 语义，不自动重建旧 view，不新增普通 A2A 自动回唤
...
| KD-16 | 普通 A2A terminal 不自动回唤 caller；caller 只在自己的下一次自然 invocation 反查尚待观察的 outbound dispatch | dispatch lifecycle 已确定性记录 source×target×response 结果；把每个 terminal 变成隐藏 Queue wake 会混淆结果可观察与 continuation。客户端单 runtime 用可丢弃指针优化连续体验，重启后回到持久 History | 2026-09-14 |
...
- Phase H: Astra 先复核「source dispatchRefs 语义不变 + caller runtime view + revision compare-and-clear + 无隐式 wake」边界；实现后由 Fable/opus 逐条核验 AC-H1..H8，再交 co-creator 做初始多目标、Steer 增删、不可用目标、连续新 source、正常终局与 runtime restart 的 worktree UAT
