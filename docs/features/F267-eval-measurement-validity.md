...
- Git bootstrap boundary：新 eval 域首次写入 executable census 是唯一保留的 Git 路径，且只允许纯新增 entry：`committedVerdictArtifactCount: 0`、`unmigrated`、certificate/result/replay refs 全为 `null`、`keep_observe_only`；既有 entry 修改及任何 verdict/bundle 证据仍一律拒绝 Git 发布，必须走 `cat_cafe_publish_verdict` 的 durable artifact store。
