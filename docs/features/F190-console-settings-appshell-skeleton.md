...
| KL-2 | capability install/update/delete/env patch 只支持无登录本地应用的 direct localhost UI；多用户/LAN/反向代理不是 capability write 支持场景 | 避免让本机用户为自己编 owner id，也避免把 owner id 伪装成远程写授权；session-only + loopback Host/Origin + no proxy forwarding headers 仍是写边界 | 若未来要支持 LAN/多用户，先设计真实认证模型，不复用 `DEFAULT_OWNER_USER_ID` 当登录系统 |
