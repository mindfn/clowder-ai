...
- [x] AC-B3: 涉及 high-risk 文件（route 注册、auth/callback、env registry、allowlist、service lifecycle）时必须走 manual-port review。Proof: MCP write / Service Manifest / refAudio / IM connector write 分别经 #1651/#1652/#1654/#1655 独立 review + remote review。
...
| operator experience / 关切 | 当前实际状态（证据） | 匹配？ |
|-------------------|----------------------|--------|
...
> **Status**: ✅ merged (PR #1720, `94c9cee49`) | **Trigger**: operator 2026-05-14 post-F199-close 实测发现多处入口重复 + 视觉不一致
> **方向**: F190 follow-up fix PR，不开新 Feature（operator: "禁止新开feat了 原本你们的f199就不应该存在 就是f190的follow up"）
...
#### 1. ChatContainerHeader.tsx（🔴 红区文件 — 修改需 operator override）

对比 clowder-ai 开源 main，本地多出 7 项：

| # | 组件 | 来源 | 处置建议 |
|---|------|------|----------|
| 1 | CatCafeLogo | 本地品牌 | ✅ **保留**（operator 确认） |
| 2 | DaemonActiveIndicator | F198 | ✅ **保留**（operator 确认，本地独有） |
| 3 | ThreadCatPill | F154 | ✅ **保留**（operator 确认，本地独有） |
...
**布局待决策**：两种风格哪个保留？需 operator 拍板。

#### 9. Tailwind 原色硬编码全局审计（2026-05-16 operator 拍板）
...
#### 11. Visual Design Pattern Gaps — Thread Sidebar + 全局 Line Divider（operator 2026-05-15）

operator 对比开源截图后确认以下设计模式需要跟进：
...
operator 要求：改为 button 或 card 样式，提供可点击视觉暗示。

##### 11e. Tag Label Visual Distinction

ThreadItem 内 `LabelDots` 渲染 `w-1.5 h-1.5` 色点堆叠，视觉区分度低。operator 要求优化为可辨识标签。
...
> **Status**: ✅ merged (PR #1712) | **Trigger**: 2026-05-16 三猫对比审计收敛 + operator 拍板
...
### PR 规划：1 个 PR 收敛（operator: "别给我搞成六个PR"）
...
| # | 区域 | 问题 | 修改内容 | 优先级 |
|---|------|------|----------|--------|
| D-7 | AppShell / ChatContainer（🔴 红区） | desktop sidebar owner 错位 | AppShell 接管 desktop ThreadSidebar | **Deferred**（operator 2026-05-17: "先不做，记录一下"） |
...
#### P1 — Visual Design Pattern 跟进（operator 2026-05-15 确认）
...
> 参与：opus（提案）+ codex（review 意见）+ opus-47（独立观点）。operator 授权猫猫自决，约束："别太夸张"、gap-4 保留。
