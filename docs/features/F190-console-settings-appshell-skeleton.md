...
| KL-2 | capability install/update/delete/env patch 在无登录本地应用中允许 direct localhost UI；多用户/LAN/反向代理部署仍要求 `DEFAULT_OWNER_USER_ID` | 避免让本机用户为自己编 owner id；session-only + loopback Host/Origin + no proxy forwarding headers 仍是写边界 | UI/ops docs 明确多用户部署必须配置 `DEFAULT_OWNER_USER_ID`；可加 telemetry warning |
