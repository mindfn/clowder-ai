...
- [x] Steer 弹窗始终显示“立即发送，引导回复”与“立即发送，中断回复”；静态 client capability 不满足时禁用引导并说明原因，当前活跃快照不参与弹窗能力判定
...
- [x] “引导回复”在服务端确认时重读 exact active parent invocation；父 invocation 存在时，`exact_active_turn` 可证明当前可见轮次读取，`queued_internal_turn` 则追加到同一运行的下一内部轮次；父 invocation 不存在或不能引导时保留普通 Queue 语义且绝不自动 cancel
