...
>
> **重新打开原因**：2026-03-12 声称 done 并通过愿景守护，但operator 2026-03-14 实际启动 dev 点开狼人杀后发现：(1) 无关闭/返回按钮，用户被困在全屏游戏界面；(2) 无大厅/配置流程，7 只猫自动塞入无法选择；(3) 猫猫不会自动行动，游戏永远卡在 night_guard 等待中；(4) 整体不可用。92 个单元测试全绿但零 E2E 真实验证。教训见 LL-032。
...
现有 mode（brainstorm/debate/dev-loop）已被 skill 流程吸收，几乎没人使用。Mode 应重新定位为**强机制游戏系统容器**，第一个目标是狼人杀。operator可选择当玩家、上帝视角观战、或法官。
...
- 规则基准：**网易狼人杀**（大众熟悉的版本）
- 角色配置：可自定义（operator开局时选角色组合），默认 7 人局
...
**B3. operator参与模式（v1 支持 player + god-view）**
...
### Phase D: 狼人杀重做 — operator 1v1 采访定案（2026-03-14）

基于operator 1v1 采访（2026-03-14 22:30），Phase D 是对 Phase A-C 的体验重做。
...
- [x] AC-B2: operator可选 player 或 god-view 参与
...
- [ ] AC-C4: **E2E 验收标准** — codex 或 gpt52 启动 dev 环境，operator能真正进入并完成一局游戏

### Phase D（狼人杀重做 — operator采访定案）✅
...
- [x] AC-E1: 上帝推理模式（Detective Mode）— 观战者开局选定一只猫，只能看到该玩家的身份和信息权限，其余座位只看到公开信息。operator experience："只能选择一只猫看他身份，狼人杀观战模式那种"
...
- [x] AC-F8: operator在 god-view 能清楚理解"正在发生什么"（不再一脸懵逼）
...
operator 2026-03-20 批评：当前 `GameAutoPlayer` + `LlmAIProvider` 只是裸调 LLM API，猫猫根本不知道自己在玩游戏。三猫（金渐层诊断 + Ragdoll架构 + Maine Coon审查）一致同意重做驱动层。
...
- [x] AC-G3: operator开局后 API 重启，游戏自动恢复推进（不卡在"全员等待"）
...
operator 2026-03-16 实测发现的核心体验 bug。先调研 GitHub agent 狼人杀项目（AIWolf 等），再设计修复方案。
...
| R2 | "operator可以选择当你们的玩家" | AC-B2 | manual | [x] |
...
| KD-6 | 旧三 mode **直接删除**，不做兼容 | operator拍板：面向终态开发，垃圾清掉 | 2026-03-11 |
| KD-7 | 角色配置可自定义 | operator开局选角色组合，默认 7 人局 | 2026-03-11 |
| KD-8 | 超时 3-5 分钟，全员提交可提前进入下阶段 | 猫猫推理慢（几秒不够），但全员完成不用空等 | 2026-03-11 |
| KD-9 | 网易狼人杀规则，无警长竞选 | 大家都熟悉的规则 | 2026-03-11 |
| KD-10 | 有遗言阶段 | operator确认 | 2026-03-11 |
...
| KD-15 | 同一 thread 单局，不做多局并发 | operator拍板：一个 thread 只跑一局游戏，想开新局就新 thread | 2026-03-11 |
| KD-16 | 游戏战绩对接 Leaderboard（F075） | 所有游戏模式（狼人杀/三国杀/猜猜我是谁等）统一接入现有排行榜系统，历史战绩通过排行榜查看 | 2026-03-11 |
| KD-17 | 技术细节（断线重连/AI策略等）找 gpt52 讨论，不找operator | operator："涉及技术你找 GPT-5.4 讨论都比我靠谱" | 2026-03-11 |
| KD-18 | 游戏在独立 thread 运行，归档分类 `游戏-狼人杀` | operator希望游戏和日常聊天完全隔离 | 2026-03-14 |
| KD-19 | 猫猫保留咖啡馆身份，不需要新 persona | operator："还是猫猫咖啡的猫猫！！！" | 2026-03-14 |
| KD-20 | 上帝面板：发牌+暂停+跳过，**不要踢人** | operator："太过分了 猫猫做错什么了" | 2026-03-14 |
| KD-21 | 展示真实 ready 状态，不做假动画 | operator担心猫猫卡住看不到 | 2026-03-14 |
| KD-22 | 狼人猫猫风 UX = 可爱+暗色调+猫猫 cosplay 狼人 | operator："猫猫装狼人那种可爱的带点黑色的风格" | 2026-03-14 |
| KD-23 | 结算画面：胜负+统计+MVP | operator确认 | 2026-03-14 |
| KD-24 | 上帝推理模式（Detective Mode）列入 Phase E | 观战者绑定单座位视角，增加悬念和代入感，不在 Phase D scope | 2026-03-15 |
| KD-25 | 平票 = no_kill（空刀），默认保守 | operator："no_kill 好像确实？一般是这样！" | 2026-03-16 |
| KD-26 | 白天投票实名公开（实时可见） | operator："要公开吧？这是推理的重要信息" | 2026-03-16 |
| KD-27 | 狼队 faction channel 讨论 — 只在夜间，讨论时间需考虑猫猫 LLM 响应速度 | operator确认要做，担心猫猫"大屁股太慢了"讨论不完 | 2026-03-16 |
| KD-28 | 狼队讨论 30s + 投票在同一阶段；首回合 grace：Ragdoll +6s / Maine Coon +12s / Siamese +30s | operator确认 30s 可以，"走起" | 2026-03-16 |
| KD-29 | 猫猫通过 A2A mention 协议参与游戏，不再裸调 HTTP API | operator批评"猫猫 agent 都没接入"——`LlmAIProvider` 只是无状态 HTTP 调 LLM，猫猫根本不知道自己在玩游戏。三猫（金渐层诊断 + Ragdoll架构 + Maine Coon审查）一致同意 | 2026-03-20 |
...
| KD-33 | 复用现有 `invoke-single-cat.ts` session 管理，同 thread = 同 session chain（自动 resume） | operator提醒"CLI new session vs resume 别搞错"——游戏在独立 thread，`sessionManager.get(userId, catId, threadId)` 天然按 thread 隔离 session | 2026-03-20 |
...
| KD-35 | Resume capsule = 导航指引 + 关键摘要 + 搜索提示，不做全量状态 dump | operator指出猫猫有 MCP 搜索 thread 能力（search_evidence / get_thread_context / read_session_events）。Resume 时给关键信息（身份/阶段/存活）+ 提示猫猫主动搜索 thread 历史恢复策略记忆。这考验每只猫的搜索和上下文恢复能力——更像人类凭记忆+回忆玩游戏 | 2026-03-20 |
| KD-36 | 信息隔离 = 心里话模式（`thinkingMode: 'play'`），不需要额外 MCP 权限层 | operator指出：CLI 内 = 心里话（`origin: 'stream'`，play 模式不 broadcast），`post_message` = 说话（`origin: 'callback'`，进入 thread）。游戏 thread 全程 play 模式，猫猫内心推理天然私密，只有 post_message 发出的才是公开/定向消息。比"三层 MCP 过滤"优雅得多 | 2026-03-20 |
| KD-37 | 游戏 thread 不入 evidence 索引 | operator指出：写代码的猫搜狼人杀搜出游戏内容很奇怪。`threadListFn` 应过滤 `projectPath.startsWith('games/')` 的 thread，不送入 IndexBuilder | 2026-03-20 |
...
> 2026-03-11 调查，operator指出 @ 弹出面板已有完整头像映射
...
| antigravity | `antigravity.png` | Bengal（豹纹+棱镜吊坠） |
| owner | `owner.jpg` | operator（`You.png` 在 assets/avatars/ 也有一份海豚版） |

### 游戏集成方案

GameView 的 `SeatView` 只需携带 `actorId`（= catId），前端直接用 `<CatAvatar catId={seat.actorId} />` 渲染，**零额外开发**。operator的 seat 用 `owner` 作为 actorId，fallback 到 `/avatars/owner.jpg`。
