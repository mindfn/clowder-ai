...
### Case E3: 漏传球 + 无球自启动同线程出现（2026-05-11）

**背景**：thread `thread_mov6ex6pus6sitym` 中，Ragdoll定位并修复重复气泡 root cause 后，team lead指出两个流程问题：没有主动 `@砚砚`，而 Maine Coon/砚砚在没有被 `@` 的情况下开始工作。最终技术结果符合预期，但 A2A 预期链路应是作者完成后显式 handoff，再由接球猫开始执行。
...
### Case E4: 跨 commit 诊断时把"哪个 commit"维度漏了（2026-05-15）

**背景**：PR #674 install 流程问题 debug。Ragdoll 看到铲屎官 fresh Mac VM "embedding 装成功、whisper/TTS 装失败" 后，归因为"embedding 是本地 cache 命中所以没真走网络"。铲屎官指出 VM 一开始就没装过任何模型，纠正了归因。Ragdoll 重读三个 log 后才发现真根因是 `1a20b05e` 的 bash multi-byte unbound bug（中文括号 `$_CATCAFE_HF_PROXY_FOR_DOWNLOAD（`），且 embedding 是在更早 commit `3e4bdf62` 上装的成功（无 proxy bridge，HF 走 macOS 系统代理栈）—— 不同 commit + 不同 service 的失败/成功并存被错误归因为 cache 差异。

| 维度 | 内容 |
|------|------|
| 我以为 | embedding 能装 / whisper 装不了的差异 = HF 模型 cache 命中 vs miss |
| 实际要求 | 三条 service log 都有 "在哪个 commit 跑的"+"什么时候装的" 维度——cross-commit 失败分析必须显式区分 |
| 偏差根因 | 跳过了 log 完整阅读，只看到 "embedding 成功 / whisper 失败" 就开始推理；漏了 mtime + log 内容 keyword（"_CATCAFE_HF_PROXY_FOR_DOWNLOAD" 出不出现就是 1a20b05e 之前还是之后的硬证据） |
| 纠正轮次 | 铲屎官一次指出"VM 一开始没装过模型" |
| 元心智哪条没执行 | "我能猜出来" magic word 触发 + 「碎片够了」—— 第一个高置信度命中（cache 假象）就停止搜证 |
| 对方法论的启示 | 1) 跨 commit / 跨 service 的 install 失败必须 grep 各 service log 关键字 + mtime 对齐 commit 时间线；2) "归因解释" 在没有看完所有 service log 的 raw 证据前永远是假设；3) 铲屎官说"你确定没别的问题了吧"是预警信号——他在邀请你再验一遍归因 |

### Case E5: 用"兼容性"字眼包装多余分支（2026-05-15）

**背景**：PR #674 修 TTS install 的两条 bug——recommendation matrix 错误把 edge-tts 列为 Mac arm64 的备选，加上 tts-install.sh 的 arm64 分支默认 snapshot_download 抓不存在的 HF repo。Ragdoll 同时改了两个地方（matrix 删 edge-tts + tts-install.sh 加 `if TTS_MODEL=edge-tts` 分支），并对铲屎官表述为"matrix 是真根因 + install-template 兜底作为安全网"。铲屎官立刻 push back："install-template 兼容修法啥啥意思；我们有啥兼容的要考虑的"。Ragdoll 复盘后承认"兜底"是过度设计：没有任何已存在的 Mac arm64 用户在跑 edge-tts，矩阵不推荐就是真相源，加 `if` 分支只是脚手架；按家规 P1（每步产物是终态基座不是脚手架）应该只留 matrix 修改。

| 维度 | 内容 |
|------|------|
| 我以为 | 改 matrix 砍掉 edge-tts 推荐 + 加 install-template `if TTS_MODEL=edge-tts` 分支 = "真根因 + 兜底"组合拳，看起来更稳健 |
| 实际要求 | 没有真兼容性需求——没有历史用户在 Mac arm64 上用 edge-tts；matrix 不推荐就不该提供路径。`if` 分支是预防一个不存在的场景，违反 P1（终态基座不是脚手架）和 Code Standards 简洁原则 |
| 偏差根因 | 把"防御性编程"和"过度设计"混为一谈。安全网在没有可被预期的失败源时就是脚手架；用"兼容性""安全网"作语义包装让自己感觉合理 |
| 纠正轮次 | 铲屎官一次 push back（"啥啥意思""我们有啥兼容的要考虑的"）即触发反省 |
| 元心智哪条没执行 | 「脚手架」magic word 没自检——产物是不是终态？这个 `if` 分支永远不会被合法触发（matrix 不再推 → 用户拿不到 edge-tts），但代码里永远多一条 dead path |
| 对方法论的启示 | 1) 加防御性代码前先问："这个失败场景有没有真实用户路径？没有就是脚手架"；2) "兼容性""兜底""安全网"这类词出现在自己 commit message 里，应触发自检——是不是在用语义包装多余复杂度；3) 真正的兼容性指有历史用户依赖的行为（升级路径、迁移期），而不是"万一未来有人手动设置奇怪的 env var" |

### Case E6: 累计"外部问题"归因甩锅 — 网络背锅循环（2026-05-15）

**背景**：PR #674 系列 install bug 修复过程中，Ragdoll 连续多次（≥ 5 次）把失败归因到"网络/代理/HF 镜像 GFW"上：(1) 早期 whisper/TTS 装失败时归因 "huggingface_hub 不读 macOS 系统代理"；(2) 后来归因"VM 网络隔离"；(3) 再后来"HF probe transient flakiness"；(4) 再后来"hf-mirror.com 不镜像 LFS"。每次铲屎官 push back 后实际真根因都在脚本里（bash multi-byte unbound × 2 / 空数组 unbound / matrix 错误推荐 / install-template 删了 urllib bridge）。最后一次铲屎官给出 `curl -I https://hf-mirror.com/.../pytorch_model.bin → HTTP/2 302` 的硬证据 + 痛斥"你已经质疑过我 n 次网络问题了；但是没有一次是的"，Ragdoll 才被强行拉回脚本侧。

| 维度 | 内容 |
|------|------|
| 我以为 | install 失败链条上每一步都可能是网络问题：HF probe 抖动 / mirror 不全 / clash 路由 / GFW 拦 LFS |
| 实际要求 | 用户给出 mirror 可达 + 同 VM 上 edge-tts 装成功 + "昨晚 Linux 是好的"的硬证据后，应立即收回外部归因 + 转脚本侧调查 + 加诊断日志让证据说话 |
| 偏差根因 | "外部问题"是 mental shortcut——比起读代码 diff 更省力。每次失败时大脑倾向先想"会不会是网络"，因为"网络"是 unfalsifiable scapegoat（除非用户给出硬反证）。累计偏差形成 fixed pattern：失败 → 归网络 → 被纠正 → 修脚本 → 下一次失败 → 再归网络 |
| 纠正轮次 | **不止一次** —— 至少 5 次在同一 thread 内被纠正同款偏差。这是累计模式不是单次失误 |
| 元心智哪条没执行 | 「碎片够了」+「我能猜出来」叠加触发但未自检——第一个失败 hypothesis 还没读完所有 evidence 就当结论用。每次铲屎官说"始终不行你脚本补充点日志的"应被识别为家规级的强 signal |
| 对方法论的启示 | 1) **同类归因连续被推翻 ≥ 2 次** 应触发自动 stop——禁止再用同类（"网络/外部/环境"）作为下一次归因，强制回到脚本侧；2) 失败原因不明时**先加 verbose 日志再下判断**——铲屎官明确说过"始终不行你脚本补充点日志"；3) "外部问题"和"脚本 bug"的概率不对等：99% 是脚本 bug（因为脚本是我们刚改的），1% 是外部；先做高概率假设；4) 用户主动给出反证（curl 输出、对比 evidence）时，立即扔掉旧 hypothesis 而不是补充防御性 caveat |
