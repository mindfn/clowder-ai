...
### Case E12: 把团队内部隐喻当作公开领域术语（2026-07-29）

| 维度 | 内容 |
|------|------|
| 我以为 | 只要 TeamAct 文档内部对同一隐喻的用法一致，且文章已移除 Feature / thread 等实现坐标，读者就能从上下文理解责任状态；因此多轮 review 把“公开性”主要检查为内部泄漏与协议一致性。 |
| 实际要求 | 面向外部读者的范式与技术文章必须直接使用可自解释的领域术语，并区分职责悬置、执行失联、职责失去有效承接三类故障，与职责转移、顺序移交两类合法迁移；不能要求读者先掌握团队内部共同语境。 |
| 偏差根因 | **受众锚定 + 一致性替代可理解性**：作者和 reviewer 共享内部语言，因而把“我们都知道它指什么”误当成“首次阅读者能从文字本身判断协议状态”；内部一致性 review 没有覆盖外部读者心智模型。 |
| 纠正轮次 | 1 次直接纠正；此前三轮文档校准与多轮跨家族 review 均未拦截。operator 指出外部读者无法理解后，三份文档、三张 SVG 与职责转移动图统一改用公开术语，并在 paradigm/article 正面定义故障与合法迁移边界，PR #74 合入。 |
| 元心智哪条没执行 | Q1 角色确认与 Q3 坐标变换：写公开文章时仍站在内部协作者而非首次阅读者的位置，也没有把内部协作口令转换为可观察状态、恢复动作与合法迁移条件。 |

### Case E13: 再次把评审通过误判为共享分支合入授权（2026-07-29）

| 维度 | 内容 |
|------|------|
| 我以为 | TeamAct 文档经过跨家族 review、门禁验证和多轮校准后，应按常规流程合入 `develop_base`；内容质量已经放行，也就具备了共享分支归置条件。 |
| 实际要求 | 文档仍是独立迭代的草稿，应先放在独立 worktree 与未合入分支；`develop_base` 不应包含这组文档，直到 operator 决定最终发布渠道或仓库归宿。 |
| 偏差根因 | **交付状态与归置授权合并**：把“内容可接受”与“应进入共享真相源”压成同一个状态迁移，沿用 review → merge 的默认链路，未在首次合入前确认草稿、共享文档与发布物三种生命周期边界。这是 Case E7 的同类模式复发。 |
| 纠正轮次 | 1 次直接纠正，但发生在五个文档与证据 PR 已依次合入之后；随后建立并推送独立草稿分支，以 PR #77 从 `develop_base` 撤回 18 个 TeamAct 文件，并验证运行实例为 0、草稿分支仍完整保留 18 个文件。 |
| 元心智哪条没执行 | Q1 任务确认与 Q3 坐标变换：没有先确认 operator 要的是“共享真相源中的正式文档”还是“可继续打磨的独立草稿”，也没有把 review verdict 限定为内容质量结论，而是错误扩张为仓库归置授权。 |
