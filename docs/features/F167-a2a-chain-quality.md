...
### Case E8: 把可重复的本地验收配置暴露成人工启动步骤（2026-07-13）

| 维度 | 内容 |
|------|------|
| 我以为 | 告诉 operator 在两个终端分别填写端口、数据目录和代理环境变量，再手工激活 workspace，就足以交付本地验收环境。 |
| 实际要求 | 确定性的端口、隔离数据、示例材料、workspace 激活与进程清理都属于项目启动契约；operator 应只需运行 `pnpm dev` / `pnpm start` 一类的一条项目命令。 |
| 偏差根因 | **实现细节外泄 + 行动偏好偏差**：把“我能用命令拼出环境”误当成“用户获得了可用启动体验”，没有把重复步骤识别为应由代码承担的开发 harness。 |
| 纠正轮次 | 1 次；纠正后新增一键 acceptance launcher、红绿测试、真实 API/UI/source dogfood 与统一退出处理。 |
| 元心智哪条没执行 | Q1 任务确认与 P1 终态思考：目标是交付可启动的本地首版，不是交付一份内部运维操作清单。 |
