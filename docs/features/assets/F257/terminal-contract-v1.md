...
| # | 偏离 | 证据 | 违约条款 | Owner | 状态 |
|---|---|---|---|---|---|
| D-1 | snapshot 内嵌整份 corpus | D1 pending snapshot 6,148,472 bytes（200 episode 正文） | TC-4 | sol · §14 S1（删 snapshot corpus → CycleRecord） | **S1 done @ `11729f614`**（Fable gate approved 09-02；唯一 PR 待六片后开） |
...
| D-7 | Console 仍有"待分类"，两组命名不对 | `SegmentTraceTheater.tsx` L54/65/100 | TC-12 | sol · §14 S4；输入 = **只 cherry-pick `88cc67154`**（7 files +41/−46；对 develop_base@635acbc97 dry-run merge 零冲突），**勿 merge 整条 `fix/f257-tracing-two-groups`**（含 485 个 rebuild 前恢复文件） | **S4 done @ `6d5ff56ac`**（Fable gate：隔离栈 F-2 / F-6 + 真实浏览器 D1 / D14 三面；首轮 `096a9ec46` 一条 P1"同名两口径"→ `6d5ff56ac` 只改名词不改数） |
| D-8 | Objective trace 归属把段 `status === 'observed'` 当**证据准入门**：readiness 计数与评估语料**同读**这一过滤索引，窗口内 `absent` 的 episode 被整体排除出池 | `ObjectiveTraceIndex.ts:118` `if (segment.status !== 'observed') continue;`；`CycleEvaluationEvidence.ts:20` 语料同源 `objectiveTraces.invocationIds(...)`；运行实例实测 4 个 Objective（capability-boundary / collaboration-loop-boundedness / critical-analysis-quality / world-state-fidelity）trace=0、`cycleStart=2026-08-24`、`evalStatus=idle` 11 天未触发；D10 在 400 episode 抽样中 present 305 / observed 0 | TC-1（预分类后才入池）/ TC-2（把标记当证据准入门）/ TC-15（首周期永远 not-ready）；spec `2026-08-04` §0.1 estimator「冻结窗口内全部 eligible TraceEpisode（**含 observed/absent**）」、AC-3、AC-13 | 未分配 | **待修**（opus 2026-09-04 实查登记） |
...
| F-5 | 评估回写后 ≤ 5 分钟，同一 Objective thread 收到 governance assignment（含历史周期摘要，≤32 KB，无正文）且 `CycleRecord.governanceAssignedAt` 写入；`submit_cycle_governance`：keep 直接归档进下一周期、不产卡；rollback / evolve → durable `HarnessGovernanceProposal`（`HGP-<hash(cycleId, cardOrdinal)>`，Redis `harness-governance-proposal`）→ F276 pending 卡（`decisionMode: approve-skip-reject`，detail 含 header / conclusions / history / changes(before+after 全文) / evidenceRefs / cardOrdinal 五段）；readonly / immutable 段的 modify / disable 草案被拒（`*_forbidden`）；reject 必附理由 → 同一冻结 `windows` 重评（assignment 带 `rejectReasons`）→ 下一张卡 `cardOrdinal+1`；approve → override 应用（modify / rollback / enable / disable）或目录写入（add）→ `resetPipelineSingleton + refreshOverrideSnapshot` → 周期归档 approved 并以新版本引用开启下一周期；skip → 归档 skipped、版本不变；无卡且 thread 静默 → 每天一次提醒（TC-16） | CycleRecord + Objective thread 消息 + `GET /api/approval-hub/pending` + `POST /api/harness-governance-candidates/:id/{approve,skip,reject}` + `GET /api/prompt-hooks/:hookId/versions` + 真实浏览器卡片（Needs Me 抽屉） | TC-8/9/10/16/17 | S3 |
