...
| OQ-7 | **Verdict 发布改为运行时 artifact（KD-26）之后，仍只读产品仓库 bundles 的消费者**（2026-09-15 同类审计实测）：F266 闭环三处——Eval Hub lifecycle 投影、`reeval-closure-task-spec`、`eval-repair-case-action-resolver` 都经 `loadLifecycleRootsWithLegacyCases`/`scanLifecycleRootArtifacts` 只扫 `docs/harness-feedback/bundles`；friction `EvalDomainAdapter`（eval-domain 通道）同样只读仓库 snapshot；`TrajectoryInspectorRepoEvidenceSource` 只认 committed bundle。三者都看不到 artifact store。闭环部分当前**潜伏**：measurement census 15 个域全部 `keep_observe_only`，actionable verdict 发不出去；**任一域解锁 `certificate_actions_allowed` 之前，这些读者必须先按 owner 读 artifact store**，否则 runtime 的 actionable verdict 永远进不了 F266 闭环。friction / trajectory-inspector 两个证据输入是**当下就缺**新产出。 | ⏳ open |
...
| KD-26 | **Verdict 发布 = 按 owner 分区的运行时 artifact（2026-09-15）**：`local-artifact-publisher` 写入产品仓库之外的 artifact store，布局 `owners/<sha256(ownerUserId)>/<domainSlug>/<artifactId>/`，由 `artifact-store-layout.ts` 唯一定义、写入方与所有读取方共用；publisher 只确认真实落在规范坐标上的文件（rename 前后各验一次，afterPublish 之前）；Eval Hub 只读当前 session owner 的分区，证据文件经 owner 作用域路由按坐标 + 封闭 file key 打开，不向客户端暴露服务端路径。Git worktree publisher 仅保留给 F311 measurement issuer。刷新时的选择 A（thread `000444`）被 KD-25 同源原则取代（`000722`，`001098`/`001107` 重申）。 | verdict 是运行时演进产出，按 KD-25 默认属于非基线，提交进产品源码树会让安装包用户拿到某个实例对自己的评估；owner 进地址而不是读后过滤，因为 verdict 由 owner 作用域证据生成——共享命名空间就是跨 owner 读证据（sol review @`01d818c8e` P1）。遗留：fork 运行实例里旧布局 `<root>/<domain>/<id>` 的 artifact 不会自动出现在新布局下，重建 develop_base 时需一次显式迁移；未改造的仓库读者见 OQ-7 | 2026-09-15 |
