...
| KD-23 | **终态契约落盘为唯一真相源**：`assets/F257/terminal-contract-v1.md`（TC-1~13 条款 + 偏离台账 D-1~7 + 真实实例 falsifier F-1~6）。核心纠正：冻结只冻结时间窗不复制数据（TC-4）；单一线性池、反例只是高置信度标记（TC-1/2）；系统三路触发 → 固定评估 thread → 工具回写 → 自动 governance → 仅回退/演进发审批卡 → 跳过不改起点（TC-3~10）；版本独立评估无对比（TC-11）；sweep 不得是前置门（TC-13）。**PR 描述必须逐条映射 TC-#，对话里的"理解了"不作数。** | operator 2026-09-02 第 ≥4 次纠正同一预期后要求"不要停留在对话中"：两个月未闭环的根因是完成定义停在组件级绿灯、契约散落对话；本行起以文件为准。台账证据来自当日运行实例只读实查（D1 1436/200 无 judgment、snapshot 6 MB、drain generation 1899）。 | 2026-09-02 |
