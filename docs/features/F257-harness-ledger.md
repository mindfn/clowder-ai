...
2. **两条高置信通道写同一种标记**：MCP pending marker 与结构化规则都在 terminal 后绑定 exact episode；Semantic Sweep 生产路径已移除，历史 annotation 只审计。自标工具上工具面留待下一轮，不在本轮偷接。
3. **一段一 Objective，粗目标细指标**：每个段恰好归属一个 Objective，一个 Objective 可挂多个段；L4 归入 `iron-law-compliance` 并以五个 Metric 表达五条铁律。Objective 静态定义与 CycleRecord 的有效策略/生命周期分离。
...
5. **调度和判断分离**：checker 对 owner 线性池累计、高置信反例与 cadence 做三路 `anyOf`，CycleRecord 只冻结时间窗和内容寻址版本引用；eval cat 按窗读全量后回写，governance 确定性调整 N/D 并对 whole hook 产动作列表。失败不影响原 invocation。
...
| OQ-7 | **Verdict 发布改为运行时 artifact（KD-26）之后，仍只读产品仓库 bundles 的消费者**（2026-09-15 同类审计实测）：F266 闭环三处（Eval Hub lifecycle 投影、`reeval-closure-task-spec`、`eval-repair-case-action-resolver`）已按 lifecycle space 接通（KD-26 第三段，sol review @`54b50ecd2` P1）。仍未改造的两个证据输入：friction `EvalDomainAdapter`（eval-domain 通道）只读仓库 snapshot；`TrajectoryInspectorRepoEvidenceSource` 只认 committed bundle——两者都看不到 artifact store，**当下就缺**新产出，需按 owner 读 artifact store 后才算闭合。 | ⏳ open |
...
| KD-21 | **raw-first Unit 评估证据契约（历史决策，2026-09-04 收紧）**：非结构化 `TraceEpisode` 是共享 canonical fact pool；高置信反例是检索锚点而非 evidence gate。原文中的 segment eligibility、snapshot/receipt 与 Semantic Sweep 已被 TC-1/4/13 覆盖：当前只按 owner+时间窗准入、CycleRecord 不复制 corpus、Sweep 已移除。 | co-creator 2026-08-20 对“annotation 才能归属评估证据”的纠正；当前唯一契约真相源：`assets/F257/terminal-contract-v1.md`。 | 2026-08-21；2026-09-04 收紧 |
| KD-22 | **Objective conclusion + PatchTrial 使用 judgment schema v2**：trigger readiness 与 verdict rule 分离；counter/replay/semantic-label 保留 count，只有天然 rate 才投影为 lower-is-better `rate-badness`；PatchTrial before/after 固定同 metric + 同 measurement kind + 同 evaluator model version，并以 trace hash 证明 injection state 真改变。Candidate §3 保持 v1 冻结。 | objective redesign INV-13 禁止 counter 虚构 denominator/rate；PR #143 conclusion→governance 闭环若继续写 v1 `violationRate` 会让行为差分建立在假坐标上。精确契约见 `assets/F257/judgment-schema-v2.md`。 | 2026-08-31 |
| KD-23 | **终态契约落盘为唯一真相源**：`assets/F257/terminal-contract-v1.md`（现 TC-1~19、DV-1~12、F-1~11）。核心纠正：owner 单一线性池不过滤；冻结只记时间窗；高置信反例 source 谓词全消费者一致；N/D 自适应而 M 冻结；一段一 Objective；whole-hook condition/content/enablement 治理；Objective 显式 lifecycle + 内容寻址版本；Semantic Sweep 已移除；提案卡按 TC-19 五段且整卡原子审批。**PR 描述必须逐条映射 TC-#，对话里的"理解了"不作数。** | operator 2026-09-02 至 09-04 连续纠偏后要求不留口头；本行随契约收紧同步。 | 2026-09-02；2026-09-04 |
| KD-24 | **治理面只够得着提示词段，观测面早已覆盖 MCP 与 skill（2026-09-11 实测）**：`EvaluationUnitRef.unitType` 是单成员字面量 `'segment'`；而 `tool-access-correct-use`（实机 19 周期，全库最多）与 `capability-guide-wakeup` 两个 Objective 一直在测 MCP 调用与 skill 唤醒。治理侧搜 `SkillOverride|McpOverride|ToolDescriptionOverride` **零命中**，只有 `HookOverrideStore`。结论：**观测得到病灶、动不了病灶**——当工具类 Objective 得出「用得不对」，唯一杠杆是改一段提示词。泛化成本不在放宽类型，在每个 unit type 要凑齐「内容可版本化 / 观测面 / 治理杠杆」三元组：skill 缺第三项（override store 与段高度同构，可能是最短路径），MCP 的第一项（tool description）不是我们的资产，须先回答归属。并记 sol 的定性：**自进化系统应与核心解耦、独立演进，但通过宿主定义的窄契约相连**——这与 F257 当前「往 core 里堆」的姿态有张力，`catalog` 110 处引用 / 28 个构造点即该代价的具象。 | KD-19 写明「段，后续 skill/MCP 复用同组件」但从未落地；不记下这条，下次会以为泛化只是改个联合类型 | 2026-09-11 |
| KD-25 | **基线范围与本地演进是两个作用域，在 manifest 层面强制区分（operator 2026-09-14 定调）**：`unitEvaluationManifest.origin` 取 `baseline`（缺省，即安装包出厂的 46 个 `CANONICAL_UNIT_IDS`）或 `local`（本线之后新增，治理卡产物 D22 是第一个）。双向强制：基线单元不得声明 local；**非基线单元必须显式声明 local，不能靠缺省混入冒充出厂内容**。local → baseline 的提升只能是显式动作（往上游提 PR 扩 `CANONICAL_UNIT_IDS`），不能是「往 manifest 追加一行」的副作用。operator 原话：正常新增默认应在非基线范围，这样才能保证安装包下体验一致。 | 此前基线与当前状态在 manifest 里是同一个扁平列表，三处测试写死"恰好 46 / 恰好 24"，直到 D22 出现才炸——说明系统当时没有"基线"这个概念，只有"当前有多少个"；sol 对 `>=N` 的反驳同源，因为基线是必须逐个盯住的集合而非一个数字 | 2026-09-14 |
| KD-26 | **Verdict 发布 = 按 owner 分区的运行时 artifact（2026-09-15）**：`local-artifact-publisher` 写入产品仓库之外的 artifact store，布局 `owners/<sha256(ownerUserId)>/<domainSlug>/<artifactId>/`，由 `artifact-store-layout.ts` 唯一定义、写入方与所有读取方共用；publisher 只确认真实落在规范坐标上的文件（rename 前后各验一次，afterPublish 之前）；Eval Hub 只读当前 session owner 的分区，证据文件经 owner 作用域路由按坐标 + 封闭 file key 打开，不向客户端暴露服务端路径。Git worktree publisher 仅保留给 F311 measurement issuer。刷新时的选择 A（thread `000444`）被 KD-25 同源原则取代（`000722`，`001098`/`001107` 重申）。**artifact 是容器不是 verdict**：一次发布可生成多个 verdict（friction breakout 的 child verdict 与聚合 verdict 同容器），verdict 坐标是 (artifactId, verdictId)，读者枚举容器内全部 `verdicts/*.md`，证据路由为 `/api/eval-hub/artifacts/:domainSlug/:artifactId/verdicts/:verdictId/files/:fileKey`。**verdict id 在 owner 的 store 内唯一**：publisher 在容器可见之前为 parent 与全部 child 原子预留 id（`artifact-store/verdict-id-reservations.ts`：按 id 摘要命名、写完整后 hard link 落位、永不释放——失败发布的 id 仍归原容器，只有同容器重试能复用），同一次 generation 重复 id 算 generator 失败，已提交进仓库的 verdict id 对 child 同样占用；reader 遇到两个容器持有同一 id 报 `verdict_id_conflict`，不按目录序取第一个。**lifecycle 按 owner 分空间，配置 owner 的空间即 install 空间**（`lifecycle-space.ts`）：install space = 仓库提交的 roots（含 legacy case migration 与 capability-wakeup 导入）+ 配置 owner 的运行时 artifact roots，沿用原全局 Redis 键；legacy migration 描述的是 verdict 谱系而非存储位置，对其中的运行时 v1 root 同样生效；同一 id 同时有仓库 root 与运行时 root 时拒绝加载。其它 owner 的 owner space = 该 owner 分区里的 roots + `eval:verdict-lifecycle:owners:<ownerKey>:*` 键（log / 去重集 / 索引全部按 owner）。Hub 只投影读者自己空间里的条目（空间外的可操作条目标 `verdict lifecycle belongs to another lifecycle space`），命令只落在调用方 owner 的空间；reconciler 与 F266/F313 eval repair 审批运行时只跑 install space。 | verdict 是运行时演进产出，按 KD-25 默认属于非基线，提交进产品源码树会让安装包用户拿到某个实例对自己的评估；owner 进地址而不是读后过滤，因为 verdict 由 owner 作用域证据生成——共享命名空间就是跨 owner 读证据（sol review @`01d818c8e` P1）。lifecycle id（verdictId、由 domain + finding 派生的 caseId）跨 owner 不唯一，所以 owner 是事件日志的地址而非事件字段，与 artifact 分区同一个 owner key（sol review @`54b50ecd2` P1）。但 stable case 的身份是 domain + finding，存储位置不能拆开它：配置 owner 在仓库里的旧周期与运行时的新周期若分属两个空间，Hub 出两张卡、事件链一分为二、两个 reconciler 各推一半（sol review @`f886006b6` P1-1）。仓库里的 lifecycle 历史本来就归配置 owner——git publisher 时代的 verdict 由它的评估产生，reconciler、责任 / 复评服务与 CVO 治理门也只替它工作，回环会话同样绑定它。verdict id 唯一性恢复的是仓库单一 `verdicts/` 目录天然具备、换成多容器后丢掉的不变量；改用完整坐标作身份会波及事件 schema、命令路由与 web，是另立一套身份模型（sol review @`f886006b6` P1-2）。遗留：fork 运行实例里旧布局 `<root>/<domain>/<id>` 的 artifact 不会自动出现在新布局下，重建 develop_base 时需一次显式迁移；未改造的仓库读者见 OQ-7 | 2026-09-15 |
