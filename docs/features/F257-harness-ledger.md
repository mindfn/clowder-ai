...
| 2026-07-27 | **Console 判据④ true-scene replay 合入 develop_base（PR #65，merge `e33d4e7b`，reviewed head `c4400641`）**：segment lifeline 确定性回放、原子 Lua 删除生命周期、completeness gap 贯穿、v0 null version 与 native-L0 null vars 合法化；terra R1→R7 review 收敛。 |
| 2026-07-28 | **Console 判据⑤ 变量段呈现合入 develop_base（PR #66，merge `53082a4f`，reviewed head `375a2829`）**：TEMPLATE_FILES runtime 占位符与 hook.yaml canonical variables parity、source/preview/replay 三界分离、restore-backup placeholder guard；terra R1→R3 review 收敛。 |
| 2026-07-29 | **Console 判据⑥ 启禁用矩阵合入 develop_base（PR #71，merge `e3b5b1cb`，reviewed head `7e6017a3`）**：localOverlay / runtimeOverride 双平面、manifest safetyTier 服务端门控、VersionActions 组件；terra R1→R2 review 收敛。post-merge acceptance：shared/API/web build 全绿，segment-enablement 12/12、enablement-matrix 10/10、VersionActions 6/6 通过。 |
