...
| KD-26 | **Verdict 发布 = 按 owner 分区的运行时 artifact（2026-09-15）**：`local-artifact-publisher` 写入产品仓库之外的 artifact store，布局 `owners/<sha256(ownerUserId)>/<domainSlug>/<artifactId>/`，由 `artifact-store-layout.ts` 唯一定义、写入方与所有读取方共用；publisher 只确认真实落在规范坐标上的文件（rename 前后各验一次，afterPublish 之前）；Eval Hub 只读当前 session owner 的分区，证据文件经 owner 作用域路由按坐标 + 封闭 file key 打开，不向客户端暴露服务端路径。Git worktree publisher 仅保留给 F311 measurement issuer。刷新时的选择 A（thread `000444`）被 KD-25 同源原则取代（`000722`，`001098`/`001107` 重申）。**artifact 是容器不是 verdict**：一次发布可生成多个 verdict（friction breakout 的 child verdict 与聚合 verdict 同容器），verdict 坐标是 (artifactId, verdictId)，读者枚举容器内全部 `verdicts/*.md`，证据路由为 `/api/eval-hub/artifacts/:domainSlug/:artifactId/verdicts/:verdictId/files/:fileKey`。**lifecycle 同样按 owner 分空间**（`lifecycle-space.ts`）：repository space = 仓库提交的 roots + 原全局 Redis 键（旧状态原样可读）；owner space = 该 owner 分区里的 lifecycle roots + `eval:verdict-lifecycle:owners:<ownerKey>:*` 键（log / 去重集 / 索引全部按 owner）。Hub 投影、命令路由各在条目所属空间内读写；reconciler 每个空间注册一个（repository + 配置 owner）；F266/F313 eval repair 审批运行时服务配置 owner 的空间。 | verdict 是运行时演进产出，按 KD-25 默认属于非基线，提交进产品源码树会让安装包用户拿到某个实例对自己的评估；owner 进地址而不是读后过滤，因为 verdict 由 owner 作用域证据生成——共享命名空间就是跨 owner 读证据（sol review @`01d818c8e` P1）。lifecycle id（verdictId、由 domain + finding 派生的 caseId）跨 owner 不唯一，所以 owner 是事件日志的地址而非事件字段，与 artifact 分区同一个 owner key（sol review @`54b50ecd2` P1）。审批运行时只服务配置 owner：F266 服务本就绑定配置 owner（任务 / lease tenantScope），且仓库 roots 自 verdict 不再提交后不会再产生 friction finding、也从未记录过审批。遗留：fork 运行实例里旧布局 `<root>/<domain>/<id>` 的 artifact 不会自动出现在新布局下，重建 develop_base 时需一次显式迁移；未改造的仓库读者见 OQ-7 | 2026-09-15 |
