...
### Scope Reset 2026-04-30

This feature has two and only two product goals for the current thread:

1. **Console full restructure**: replace the old Hub/modal-centered console surface with route-based console flows. This is primarily information architecture, page flow, layout, entry-point cleanup, and F056 front-end design-debt cleanup. It must preserve existing member/account/skill/MCP/plugin/IM functionality unless an acceptance item explicitly says otherwise.
2. **Service Manifest support**: the only new user-visible functional capability in this feature. Service Manifest covers known external service declarations, health/status APIs, owner-gated lifecycle operations, logs, and inline console status surfaces.

Out of scope for this feature:

- Opening or replacing the GitHub PR. PR #576 was intentionally closed and is not the next action.
- Adding unrelated new console product features beyond Service Manifest.
- Keeping the old `CatCafeHub` modal as a primary or hidden console entry after `/settings` becomes the canonical management surface.
- Treating implementation screenshots from an old branch/runtime as acceptance evidence. Acceptance must be against this worktree and current `origin/main`.

Current branch reality check:

- `ServiceManifest` work has already started in code (`packages/api/src/domains/services/*`, `packages/api/src/routes/services.ts`, `packages/web/src/components/settings/ServiceStatusPanel.tsx`). It must be reviewed as the feature's new capability, not hidden inside "console layout".
- `CatCafeHub` is still mounted from `ChatContainer`, so old-console removal is not complete.
- `ActivityBar` currently includes Mission as a primary icon and places Settings in the lower utility area; that does not match the intended four primary entries: Chat / Signals / Memory / Settings.

Detailed acceptance control lives in [F179 Acceptance Table](./acceptance/F179-acceptance-table.md).
