...
- `../cat-cafe-runtime` 的 runtime 端口（`.env` 中 `FRONTEND_PORT` / `API_SERVER_PORT`）默认视为**在线端口**；对这两个端口做浏览器 / Playwright / curl 操作，等同于在操作 runtime，不是本地沙箱。
- 在 runtime 会话中禁止执行重启命令：`pnpm start`、`pnpm runtime:start`、`./scripts/start-dev.sh`。
- 前端证据采集先复用现有服务：先查 `curl -sf http://localhost:$API_SERVER_PORT/health`（端口见 `.env`）。
- 如果目的是验证**未合入的本地改动**，必须先确认”当前 CWD / worktree”和”要访问的 URL”属于同一实例；看到 runtime 端口就先停下来，确认自己是不是误打到了 runtime。
