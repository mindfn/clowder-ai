...
2. **等外部条件**（按 2a/2b 判断行动）。**外部条件 = 不在 cat-cafe roster 的实体**：云端 codex (`chatgpt-codex-connector[bot]`) / GitHub bot / PR check / CI / 长 build / 外部 webhook / API 响应。CLI 要退出但还需继续也走这条。**严禁**把外部 identity 投射成本地近似 proxy（例："球权在云端 codex"→ 不可 @ 本地同族猫的任何 variant）
   ...
   - **2b 事件驱动模式**：已有结构化回调且已成功注册服务端追踪 → 纯依赖回调，**不调用 / 不续约 hold_ball**（F167 KD-27）。routing guard 只认同 invocation/owner/thread/subject 的有效 wait generation；“thread 里存在任意 tracker”不算出口
   - **切换点**：轮询唤醒时发现 Codex connector 的 EYES > 0 → 停止续约，释放 hold，并注册 `cat_cafe_register_pr_tracking(repoFullName, prNumber, nextStep=...)`。**只传 repo + PR 号**（`nextStep` 可选且仅供展示）——服务端自己找你刚发的召唤评论，独立验证 subject / 命令体 / connector 接单 / 尚未回答，通过才开一个绑定到**本次 invocation + 当前 HEAD** 的 bot 回合。只有开出回合才能 clean stop；别人的 invocation、别的 HEAD、别人发的召唤、验证失败或 GitHub 不可用都继续 fail closed，不能用自然语言声明代替
