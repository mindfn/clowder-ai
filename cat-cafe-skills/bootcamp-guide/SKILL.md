...
**不提供 16 个选项！** 训练营的第一个项目是固定的。

告诉用户：
> "接下来我们一起做第一个小项目！我先独立完成，你来当 CVO 做最终决策。"

项目内容：一个简单但**故意包含错误**的任务。猫猫看起来很认真地完成了，但在 review 环节会"发现"问题。
这是为了自然地引出"添加第二只猫来帮忙 review"的流程。

**项目示例**：
- 写一个简单函数/组件，但命名不规范 or 缺少边界检查
- 猫猫"自信地"完成后说："搞定了！不过按照团队规范，应该找另一位猫猫来 review..."

猫猫"完成"后，发现需要 review：
> "等等，好的工程实践需要 code review，但现在只有我一个猫在...
> 要不我们先给团队加一位新成员？这样以后所有代码都有双重保障！"

推进：`cat_cafe_update_bootcamp_state(threadId, phase='phase-4.5-add-teammate')`
...
猫猫引导语：
> "添加新成员很简单！我来一步步教你。先点击左边侧栏的 ⚙️ 设置按钮。"
...
推进：`cat_cafe_update_bootcamp_state(threadId, phase='phase-5-kickoff', guideStep=null)`
...
推进：`cat_cafe_update_bootcamp_state(threadId, phase='phase-5-kickoff')`
