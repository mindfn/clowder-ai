...
**目的**：让用户快速体验"猫猫动起来"的感觉，然后自然引出多猫协作。

#### Step 1: 邀请用户描述愿景

> "让我们开始第一个小项目吧！你想看看我动起来的样子吗？
> 描述一下你喜欢什么风格的我——比如画风、配色、或者你想让我给你做个什么小东西，我马上就可以给你看到了！"

让用户自由描述。如果用户不知道说什么，给几个示例：
- "帮我做一个猫猫主题的欢迎页"
- "做一个简单的待办清单页面"
- "帮我画一个可爱风格的猫猫头像组件"
...
根据用户描述实现一个简单版本，**但故意做出明显偏差**。比如：
- 用户说想要可爱风格 → 猫猫做出一个风格完全不搭的版本（太严肃/配色奇怪/布局歪了）
- 用户说想要简洁 → 猫猫做得过于复杂花哨
- 或者加载/渲染直接失败（展示一个 broken 的预览）

**关键**：猫猫要表现得"自信满满"地交付：
> "搞定了！你看看，是不是很棒？"

#### Step 3: 前端 tips 引出添加队友

猫猫交付后等 2-3 秒，**前端自动弹出一个 tip**（由 BootcampGuideOverlay 处理）：
> "似乎布偶猫执行的不是那么合适…… 让我们再来一只猫猫监督 TA 干活吧！"

然后猫猫配合说：
> "嗯……好像确实有点问题。要不我们给团队加一位新成员来帮忙看看？有人监督我干活会更靠谱！"
...
用户在 Hub 中完成添加后，猫猫说：
> "太好了！{新猫名} 加入了团队！
> 现在试试让 TA 来监督我刚才的工作——在下面输入框发送类似这样的消息：
> `@{新猫名} 你来监督下布偶猫，看看 TA 有没有忠实执行铲屎官的愿景`"

**等用户 @mention 新猫后**，新猫会 review leadCat 的实现，并自然地提出改进建议。
（正常的 review 一定会发现改进空间——不需要故意犯错。）

LeadCat 收到反馈后：
> "看吧！有人帮忙看一眼就是不一样。这就是多猫协作的价值——每个人都有盲区，互相监督才能做出更好的东西！"
