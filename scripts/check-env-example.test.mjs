...
const EXAMPLE_ALLOWLIST = new Map([
  ...
  ['TELEGRAM_BOT_TOKEN', 'F240 connector manifest/preconfigured gateway env, not global env registry'],
  ...
  ['WECOM_BOT_SECRET', 'F240 connector manifest/preconfigured gateway env, not global env registry'],
  ['WECOM_CORP_ID', 'F240 connector manifest/preconfigured gateway env, not global env registry'],
  ['WECOM_AGENT_ID', 'F240 connector manifest/preconfigured gateway env, not global env registry'],
  ...
  // #770: Vars removed from env-registry (internal/infra/credential — not for Settings UI)
  ['MCP_SERVER_PORT', '#770: internal — MCP server port, set by launcher'],
  ['NEXT_PUBLIC_API_URL', '#770: build-time — Next.js public API URL (build-time only)'],
  ['CLI_TIMEOUT_MS', '#770: internal tuning — CLI timeout'],
  ...
  ['ANTHROPIC_API_KEY', '#770: credential — managed via credentials.json'],
  ['OPENAI_API_KEY', '#770: credential — managed via credentials.json'],
  ['GOOGLE_API_KEY', '#770: credential — managed via credentials.json'],
  ...
  ['NEXT_PUBLIC_WHISPER_URL', '#770: build-time — Whisper URL for Next.js frontend'],
  ['TTS_URL', '#770: service URL — TTS (MlxAudio) service address'],
  ['TTS_CACHE_DIR', '#770: internal — TTS cache directory path'],
  ['NEXT_PUBLIC_LLM_POSTPROCESS_URL', '#770: build-time — LLM postprocess URL for Next.js frontend'],
  ['GITHUB_REVIEW_IMAP_USER', '#770: credential — GitHub review IMAP username'],
  ['GITHUB_REVIEW_IMAP_PASS', '#770: credential — GitHub review IMAP password'],
  ['GITHUB_REVIEW_IMAP_HOST', '#770: internal — GitHub review IMAP host'],
  ['GITHUB_REVIEW_IMAP_PORT', '#770: internal — GitHub review IMAP port'],
  ['LOG_LEVEL', '#770: internal — Pino log level'],
