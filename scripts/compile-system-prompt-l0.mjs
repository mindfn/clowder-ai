...
/**
 * Compile per-cat L0 and capture the exact L1-L7 sections before they are
 * flattened into the provider prompt. The manifest is the trace authority for
 * native-L0 delivery, so it must be produced by the same render pass.
 *
 * @param {Object} options - same options as compileL0
...
export async function compileL0WithManifest(options) {
  ...
  // Capture each rendered section before flattening. Reconstructing this from
  // the compiled prompt would be ambiguous because compiler-only labels are
  // intentionally stripped from the delivered bytes.
  let result = template;
  const lSegments = [];
...
//     → write the exact L1-L7 manifest (orthogonal to prompt output mode)
